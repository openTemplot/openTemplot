...
var
  ...
  function convert_tagged_string_to_html(cap_code:integer; tagged_str:string; for_alert:boolean):string; // 0.97.d
...
function convert_tagged_string_to_html(cap_code:integer; tagged_str:string; for_alert:boolean):string;
  ...
  with html_help_list do begin
                 ...
                 then begin
                        ...
                        if (cap_code=-2) or (cap_code=-3) or (cap_code=-4)
                           then Add('<HTML><HEAD><TITLE>help  notes</TITLE></HEAD><BODY BGCOLOR="#'+html_colour_str+'"'
                                   +' STYLE="FONT-WEIGHT:NORMAL; FONT-FAMILY:''Comic Sans MS''; FONT-SIZE:16PX; FONT-WEIGHT:BOLD; COLOR:#600000;">')   // template info.
    ...
    repeat
      ...
      if next_string=''                                                    // multiple CR.
         then {n:=}Add('<BR><SPAN STYLE="FONT-SIZE:10PX;">&nbsp;</SPAN>')  // init para spacing.
                ...
                repeat
                  ...
                  case style_index of
                    ...
                    '9': if for_alert=True
                            then add_str:=add_str+'<SPAN STYLE="font-weight:bold; font-family:''Arial''; font-size:'+IntToStr(help_font_size+2)+'px;">'+part_string+'</SPAN>'                                            // header for alerts
    ...
    case cap_code of
      ...
      -3,-2: Caption:='    template  information';
       ...
       //------------------------------------------------------
        ...
        else Caption:='    '+Application.Title+'  information';          // files.
       ...
       then begin     // html file name (possibly including a # target, so don't use ExtractFileExt, etc.)...
              ...
              if cap_code<-100     // plain HTML code..
                 then begin
                        ...
                        current_load_str:=convert_tagged_string_to_html(cap_code,full_path_or_msg,False);  // 0.97.d  extracted to separate routine
                    ...
                    with help_form do begin
                      ...
                      if yes_msg=''
                         then begin
                                ...
                                do_show_modal(help_form);  // 212a   ShowModal  // wait for user to respond.
