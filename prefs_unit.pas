...
const
  html_header_str: string = '<P CLASS="heading">Saved &nbsp;Program &nbsp;Preferences'
    + '<BR>&nbsp; &nbsp; &nbsp; &nbsp;  &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;  &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;  &nbsp; &nbsp; &nbsp;' + '<SPAN CLASS="alert">please scroll this page to read all of it&nbsp; &nbsp; &nbsp; click the <SPAN STYLE="color:blue;">blue links</SPAN> to make your settings</SPAN></P>' + '<HR NOSHADE>' + '<P STYLE="COLOR:#773300; FONT-STYLE:ITALIC; line-height:140%;">' + 'This function is intended for experienced Templot users only. There is a risk of losing some program functionality, making it impossible to follow the videos and tutorials.' + ' To cancel saving preferences, click <A HREF="abandon_prefs.85a">abandon preferences</A> , and then re-start Templot.</P>' + '<P><TABLE ALIGN="CENTER" WIDTH="100%" BORDER="0" CELLSPACING="12" CELLPADDING="0" STYLE="line-height:140%;">'   // outer table

    + '<TR><TD WIDTH="40%" VALIGN="TOP" STYLE="BACKGROUND-COLOR:#DDFFFF;">';    // left column

  html_left1_str: string =
    '<TABLE ALIGN="CENTER" WIDTH="100%" BORDER="0" CELLSPACING="0" CELLPADDING="12">'
    + '<TR><TD>You have no saved program preferences.</TD></TR>' +
    '<TR><TD>Click <A HREF="init_prefs.85a">save preferences</A> to save your current program preferences. You will have an option to reload them when you next start a Templot session.</TD></TR>' + '</TABLE>';

  html_left2_str: string =
    '<TABLE ALIGN="CENTER" WIDTH="100%" BORDER="0" CELLSPACING="0" CELLPADDING="12">'
    ...
    + '<TR><TD>Click <A HREF="change_prefs_file_and_load.85a">change file and reload</A> to choose a different existing file to be used instead, and reload the preferences from it now.</TD></TR>' + '<TR><TD>&nbsp;<BR>Click <A HREF="save_prefs.85a">save preferences</A> to update your preferences in the current preferences file now.</TD></TR>' + '<TR><TD>Click <A HREF="new_prefs_file_and_save.85a">new file and save</A> to create a new preferences file to be used instead, and save your current preferences to it now.</TD></TR>' + '<TR><TD>&nbsp;<BR>Click <A HREF="change_prefs_file_only.85a">change file only</A> to choose a different existing file to be used instead when preferences are next reloaded or saved.</TD></TR>' + '<TR><TD>&nbsp;<BR>Click <A HREF="prefs_include_gen.85a">include generator settings</A> to include the generator settings in your saved preferences the next time they are saved.' + ' Click <A HREF="prefs_exclude_gen.85a">exclude generator settings</A> to exclude the generator settings from your saved preferences the next time they are saved.</TD></TR>' + '<TR><TD>&nbsp;<BR>Click <A HREF="abandon_prefs.85a">abandon preferences</A> to abandon your saved preferences. You won''t see the option to use your preferences when you start the next Templot session.</TD></TR>' + '</TABLE>';

  html_right_str: string = '</TD><TD VALIGN="TOP">'    // right column
    + '<TABLE ALIGN="CENTER" WIDTH="100%" BORDER="0" CELLSPACING="0" CELLPADDING="12">'
    + '<TR><TD STYLE="PADDING-TOP:NONE;">Program preferences are your settings for the various options in Templot which affect its appearance on the screen and control the way the program operates and displays your work.' + '<BR><BR><SPAN STYLE="FONT-SIZE:16PX; COLOR:#A00000;">(In the current version of Templot only a limited number of program settings are saved in your preferences. More will be added in later versions.)</SPAN>' + '<BR><BR>' + 'Program preferences do not include matters related to track design such as your model scale and track gauge, timbering styles, etc. These are saved in your template data files.' + '<BR><BR>' + 'When you quit a Templot session, your program preferences can be saved, and they can be restored at the start of the next session.' + '<BR><BR>' + 'The controls for saving your program preferences are accessed from the <SPAN STYLE="COLOR:#773300; FONT-STYLE:ITALIC;">preferences</SPAN> menu on the <SPAN STYLE="COLOR:#773300; FONT-STYLE:ITALIC;">program panel</SPAN>:' + '<BR><BR>' + '<IMG SRC="~~~3">' + '<BR><BR>' + 'It is also possible to create several different preferences files and choose which one to use at any time.' + '<BR><BR>' + 'The template generator settings can be included in your saved preferences. Use this option with care.' + ' The generator settings are intended for short-term changes while working in Templot. Including them in your saved preferences may cause essential functions to be disabled or unavailable.' + ' Your generator settings are currently being ~~~4.' + '<BR><BR>' + 'Preferences files can have any name of your choosing but always have file extension <B><I>.sk1</I></B> and are always saved in the program folder: ~~~2' + '<BR><BR>' + 'Your preferences files can be shared with other Templot users, provided they are saved to the user''s program folder. If you do this, it is strongly recommended that you exclude the generator settings.' + '</TD></TR></TABLE>' + '</TD></TR></TABLE></P>';    // outer table - final closing of body tags is in help() routine;
...
begin
  if prefs_available = False then
    help_str := html_header_str + html_left1_str + html_right_str
  else begin
    ...
    current_prefs_str :=
      '<SPAN STYLE="FONT-WEIGHT:BOLD; COLOR:#3000D0; FONT-FAMILY:Courier New; FONT-SIZE:17PX;">' +
      get_prefs_file_name + '</SPAN>';
  ...
  if control_room_form.prefs_include_gen_settings_menu_entry.Checked = True then
    ...
    help_str := StringReplace(help_str, '~~~4', '<B><I>excluded</I></B>', [rfIgnoreCase]);
...
begin
  ...
  if help_showing = True then
    do_help(-200, prepare_prefs_html_str, False)  // update the contents
  else
    help(-200, prepare_prefs_html_str, '');       // shows modal
...
begin
  Result := str_prefix + StringReplace(str, '=', '%#%#%', [rfReplaceAll, rfIgnoreCase]);
  // no '=' in string
end;
//______________________________________________________________________________
...
begin
  ...
  if prefs_file_str = '' then begin
    prefs_file_str := get_prefs_file_name;
    if confirm_it = True then begin
      if alert(7, '      save  program  preferences ?',
        '||You are about to replace your previous program preferences in file: ' +
        prefs_file_str + '||OK?| ', '',
        '', '', '', 'no  -  cancel', 'yes  -  save  preferences    ', 0) = 5 then
        EXIT;
    end;
  end;
  ...
  with user_prefs_list do begin
    ...
    Values['templot_user_preferences'] :=
      str_prefix + 'from ' + Application.Title + ' version ' + GetVersionString(voFull);
    ...
    Values['00-85b'] := IntToStr(integer(
      control_room_form.blank_spaces_in_menus_menu_entry.Checked));
    // 213b
    ...
    Values['02-05i'] := IntToStr(startup_restore_pref);
    ...
    Values['03-41b'] := IntToStr(integer(pad_form.transparent_names_menu_entry.Checked));
    // 208a ...
    ...
    Values['03-70b'] := IntToStr(integer(pad_form.no_f7_snapping_menu_entry.Checked));
    // 218a radio item
    Values['03-71b'] := IntToStr(integer(pad_form.snap_on_background_templates_menu_entry.Checked));
    // 218a radio item
    Values['03-72b'] := IntToStr(integer(
      pad_form.snap_always_on_background_templates_menu_entry.Checked));  // 218a radio item
    ...
    Values['03-97b'] := IntToStr(integer(bgkeeps_form.bold_timber_outlines_checkbox.Checked));
    // 219a
    ...
    Values['05-20i'] := IntToStr(mouse_click_action);
    // -1 = either click-move-click or drag allowed.

    Values['05-30b'] := IntToStr(integer(control_room_form.fast_100_menu_entry.Checked));
    // allow_idle = False
    Values['05-31b'] := IntToStr(integer(control_room_form.mouse_100_menu_entry.Checked));
    // allow_idle = True
    ...
    if control_room_form.prefs_include_gen_settings_menu_entry.Checked = True then begin
      Add('--------  generator:  --------');
      Values['06-00b'] := '1';
      //control_room_form.prefs_include_gen_settings_menu_entry.Checked (so can save again).
      ...
      Values['06-65b'] :=
        IntToStr(integer(pad_form.gen_platforms_trackbed_edges_menu_entry.Checked));

      rail_prefs := -1; // init
      if (pad_form.both_edges_menu_entry.Checked = True) then
        rail_prefs := 0;
      if (pad_form.gauge_faces_only_menu_entry.Checked = True) then
        rail_prefs := 1;
      if (pad_form.outer_edges_only_menu_entry.Checked = True) then
        rail_prefs := 2;
      if (pad_form.rail_centrelines_only_menu_entry.Checked = True) then
        rail_prefs := 3;
    ...
    if grey_shade = True then
      print_colour_prefs := 1;
    if black_white = True then
      print_colour_prefs := 2;
    Values['08-15i'] := IntToStr(print_colour_prefs);

    Values['08-17b'] := IntToStr(integer(pad_form.printed_grid_solid_menu_entry.Checked));
    // 207a radio item
    Values['08-18b'] := IntToStr(integer(pad_form.printed_grid_dotted_menu_entry.Checked));

    Values['08-20b'] := IntToStr(integer(pad_form.thin_printed_lines_menu_entry.Checked));
    // 207a radio item
    Values['08-21b'] := IntToStr(integer(pad_form.normal_printed_lines_menu_entry.Checked));
    // 207a radio item
    Values['08-22b'] := IntToStr(integer(pad_form.thick_printed_lines_menu_entry.Checked));
    // 207a radio item
    Values['08-23b'] := IntToStr(integer(pad_form.set_line_thicknesses_menu_entry.Checked));
    // 207a radio item

    Values['08-25b'] := IntToStr(integer(pad_form.adjust_line_thickness_menu_entry.Checked));
    // 207a
    ...
    Values['08-45b'] := IntToStr(integer(
      pad_form.timber_numbering_on_plain_track_menu_entry.Checked));
    // 208a
    ...
    Add('--------  storage box:  --------');
    Values['10-00b'] := IntToStr(integer(keep_form.briefly_hide_on_store_menu_entry.Checked));
    // 205e now radio item.  205c
    Values['10-01b'] := IntToStr(integer(keep_form.hide_on_store_menu_entry.Checked));
    // 205e radio item
    Values['10-02b'] := IntToStr(integer(keep_form.dont_hide_on_store_menu_entry.Checked));
    // 205e radio item
    Values['10-03b'] := IntToStr(integer(keep_form.reveal_on_store_menu_entry.Checked));
    // 206a radio item
    Values['10-04b'] := IntToStr(integer(keep_form.alert_on_store_menu_entry.Checked));
    // 206c radio item
    ...
    with colour_form.colour_dialog do begin
      // !!! NOT .CustomColors  -- Values is the prefs list
  ...
  if user_prefs_list.Count > 0 then begin
    try
      ...
      if help_form.Showing = True then
        do_help(-200, prepare_prefs_html_str, False); // update details
    except
...
begin
  ...
  val_str := StringReplace(val_str, '%#%#%', '=', [rfReplaceAll, rfIgnoreCase]);
  // replace '=' in string

  Result := StringReplace(val_str, str_prefix, '', [rfReplaceAll, rfIgnoreCase]);
...
begin
  ...
  with prefs_font do begin  // temp result font
    ...
    if val_str = '' then
      Name := def_font.Name  // not in list, use default
    else
      Name := StringReplace(val_str, str_prefix, '', [rfReplaceAll, rfIgnoreCase]);   // remove string prefix

    val_str := user_prefs_list.Values[str + '1i'];
    if val_str = '' then
      Size := def_font.Size  // not in list, use default
    else begin
      try
        temp_int := StrToInt(val_str);
      except
    ...
    if val_str = '' then
      Color := def_font.Color      // not in list, use default
    else begin
      try
        temp_int := StrToInt(val_str);
      except
...
begin
  if get_file = True then begin
    if prefs_file_str = '' then
  ...
  if startup_msg_prefs = True then begin
    //-------- startup dialogs

    start_colours := get_pref_int('02-01i', start_colours);
    startup_restore_pref := get_pref_int('02-05i', startup_restore_pref);
    // 0=ask, 1=don't restore, 2=restore without asking
    ...
    try
      ...
      wheel_zoom_code := get_pref_int('00-80i', wheel_zoom_code);
      // mouse wheel zoom mode 0.97.d

      user_popup_X := get_pref_int('00-82i', user_popup_X);
      // 213a     pop-up menu position
      user_popup_Y := get_pref_int('00-83i', user_popup_Y);

      if control_room_form.blank_spaces_in_menus_menu_entry.Checked <>
        get_pref_bool('00-85b', control_room_form.blank_spaces_in_menus_menu_entry.Checked)  // 213b
      then
        control_room_form.blank_spaces_in_menus_menu_entry.Click;

      retain_prefix_pref := get_pref_bool('00-87b', retain_prefix_pref);  // 213b

      with cdvi do begin    // dummy vehicle dims  0.98.a

        dv_start := get_pref_float('00-90f', dv_start);
        // to first axle/bogie-pin from CTRL-0
      ...
      //-------- bgnd shapes
      with bgnd_form.pad_shapes_linewidth_1_radiobutton do
        Checked := get_pref_bool('01-01b', Checked);
      with bgnd_form.pad_shapes_linewidth_2_radiobutton do
        Checked := get_pref_bool('01-02b', Checked);
      with bgnd_form.pad_shapes_linewidth_3_radiobutton do
        Checked := get_pref_bool('01-03b', Checked);
      ...
      with pad_form.bgnd_keeps_font_label do
        Font.Assign(get_pref_font('03-3', Font));

      with pad_form.transparent_names_menu_entry do
        Checked := get_pref_bool('03-41b', Checked);
      with pad_form.boxed_over_names_menu_entry do
        Checked := get_pref_bool('03-42b', Checked);

      with pad_form.names_scaled_menu_entry do
        Checked := get_pref_bool('03-43b', Checked);
      with pad_form.names_fullsize_menu_entry do
        Checked := get_pref_bool('03-44b', Checked);


      //-------- trackpad control template
      with pad_form.peg_blank_menu_entry do
        Checked := get_pref_bool('03-60b', Checked);
      with pad_form.peg_solid_red_menu_entry do
        Checked := get_pref_bool('03-61b', Checked);
      with pad_form.peg_hollow_menu_entry do
        Checked := get_pref_bool('03-62b', Checked);


      with pad_form.no_f7_snapping_menu_entry do
        Checked := get_pref_bool('03-70b', Checked);     // 218a radio item
      with pad_form.snap_on_background_templates_menu_entry do
        Checked := get_pref_bool('03-71b', Checked);     // 218a radio item
      with pad_form.snap_always_on_background_templates_menu_entry do
        Checked := get_pref_bool('03-72b', Checked);     // 218a radio item


      //--------  trackpad background templates    // 208a ...

      with bgkeeps_form.timber_outlines_checkbox do
        Checked := get_pref_bool('03-80b', Checked);
      with bgkeeps_form.timber_centres_checkbox do
        Checked := get_pref_bool('03-81b', Checked);
      with bgkeeps_form.gauge_faces_checkbox do
        Checked := get_pref_bool('03-82b', Checked);
      with bgkeeps_form.marks_checkbox do
        Checked := get_pref_bool('03-83b', Checked);
      with bgkeeps_form.joints_checkbox do
        Checked := get_pref_bool('03-84b', Checked);
      with bgkeeps_form.centres_checkbox do
        Checked := get_pref_bool('03-85b', Checked);
      with bgkeeps_form.peg_checkbox do
        Checked := get_pref_bool('03-86b', Checked);
      with bgkeeps_form.outer_edges_checkbox do
        Checked := get_pref_bool('03-87b', Checked);
      with bgkeeps_form.template_number_checkbox do
        Checked := get_pref_bool('03-88b', Checked);
      with bgkeeps_form.template_name_checkbox do
        Checked := get_pref_bool('03-89b', Checked);
      with bgkeeps_form.reduced_ends_checkbox do
        Checked := get_pref_bool('03-90b', Checked);
      with bgkeeps_form.timber_infill_checkbox do
        Checked := get_pref_bool('03-91b', Checked);
      with bgkeeps_form.timber_numbering_checkbox do
        Checked := get_pref_bool('03-92b', Checked);
      with bgkeeps_form.platforms_checkbox do
        Checked := get_pref_bool('03-93b', Checked);
      with bgkeeps_form.trackbed_edges_checkbox do
        Checked := get_pref_bool('03-94b', Checked);
      with bgkeeps_form.template_id_checkbox do
        Checked := get_pref_bool('03-96b', Checked);
      with bgkeeps_form.bold_timber_outlines_checkbox do
        Checked := get_pref_bool('03-97b', Checked);    // 219a
      ...
      irreg_crossings_msg_pref := get_pref_bool('04-66b', irreg_crossings_msg_pref);
      // 0.93.a
      sb_rvf_add_msg_pref := get_pref_bool('04-68b', sb_rvf_add_msg_pref);
      // 0.97.a
      sb_no_select_msg_pref := get_pref_bool('04-70b', sb_no_select_msg_pref);
      // 0.98.b
      too_many_files_msg_pref := get_pref_bool('04-72b', too_many_files_msg_pref);
      // 208e
      retain_prefix_msg_pref := get_pref_bool('04-74b', retain_prefix_msg_pref);
      // 213b
      drop_msg_pref := get_pref_bool('04-76b', drop_msg_pref);
      // 214a
      yellow_msg_pref := get_pref_bool('04-78b', yellow_msg_pref);
      // 214a
      auto_fit_msg_pref := get_pref_bool('04-80b', auto_fit_msg_pref);
      // 214a
      screenshot_msg_pref := get_pref_bool('04-82b', screenshot_msg_pref);
      // 215a


      //--------  cpu / mouse actions / menus
      if get_pref_bool('05-05b', classic_templot) = True then
        control_room_form.classic_templot_mode_menu_entry.Click
      else
        control_room_form.make_on_click_mode_menu_entry.Click;
      full_draw_trans_pref := get_pref_bool('05-10b', full_draw_trans_pref);
      auto_dir := get_pref_bool('05-15b', auto_dir);

      mouse_click_action := get_pref_int('05-20i', mouse_click_action);
      // -1 = either click-move-click or drag allowed.

      with control_room_form.fast_100_menu_entry do
        Checked := get_pref_bool('05-30b', Checked);    // radio items
      with control_room_form.mouse_100_menu_entry do
        Checked := get_pref_bool('05-31b', Checked);
      ...
      with pad_form do begin
        if get_pref_bool('06-05b', timber_chairs_menu_entry.Checked) <>
          timber_chairs_menu_entry.Checked then
          timber_chairs_menu_entry.Click;
        if get_pref_bool('06-10b', centre_lines_menu_entry.Checked) <>
          centre_lines_menu_entry.Checked then
          centre_lines_menu_entry.Click;
        if get_pref_bool('06-15b', rad_end_marks_menu_entry.Checked) <>
          rad_end_marks_menu_entry.Checked then
          rad_end_marks_menu_entry.Click;
        if get_pref_bool('06-20b', timber_outlines_menu_entry.Checked) <>
          timber_outlines_menu_entry.Checked then
          timber_outlines_menu_entry.Click;
        if get_pref_bool('06-25b', outline_extensions_menu_entry.Checked) <>
          outline_extensions_menu_entry.Checked then
          outline_extensions_menu_entry.Click;
        if get_pref_bool('06-30b', timber_centres_menu_entry.Checked) <>
          timber_centres_menu_entry.Checked then
          timber_centres_menu_entry.Click;
        if get_pref_bool('06-35b', timber_numbers_menu_entry.Checked) <>
          timber_numbers_menu_entry.Checked then
          timber_numbers_menu_entry.Click;
        if get_pref_bool('06-40b', timbering_infill_menu_entry.Checked) <>
          timbering_infill_menu_entry.Checked then
          timbering_infill_menu_entry.Click;
        if get_pref_bool('06-45b', reduced_ends_menu_entry.Checked) <>
          reduced_ends_menu_entry.Checked then
          reduced_ends_menu_entry.Click;

        if get_pref_bool('06-50b', stock_rails_menu_entry.Checked) <>
          stock_rails_menu_entry.Checked then
          stock_rails_menu_entry.Click;
        if get_pref_bool('06-52b', crossing_rails_menu_entry.Checked) <>
          crossing_rails_menu_entry.Checked then
          crossing_rails_menu_entry.Click;
        if get_pref_bool('06-54b', check_rails_menu_entry.Checked) <>
          check_rails_menu_entry.Checked then
          check_rails_menu_entry.Click;

        if get_pref_bool('06-56b', overscale_joggles_menu_entry.Checked) <>
          overscale_joggles_menu_entry.Checked then
          overscale_joggles_menu_entry.Click;
        if get_pref_bool('06-58b', joint_marks_menu_entry.Checked) <>
          joint_marks_menu_entry.Checked then
          joint_marks_menu_entry.Click;
        if get_pref_bool('06-60b', guide_marks_menu_entry.Checked) <>
          guide_marks_menu_entry.Checked then
          guide_marks_menu_entry.Click;
        if get_pref_bool('06-62b', switch_drive_markx_menu_entry.Checked) <>
          switch_drive_markx_menu_entry.Checked then
          switch_drive_markx_menu_entry.Click;

        if get_pref_bool('06-65b', gen_platforms_trackbed_edges_menu_entry.Checked) <>
          gen_platforms_trackbed_edges_menu_entry.Checked then
          gen_platforms_trackbed_edges_menu_entry.Click;

        rail_prefs := get_pref_int('06-80i', -1); // -1 init, rail prefs not present

        case rail_prefs of
          0:
            both_edges_menu_entry.Click;
          1:
            gauge_faces_only_menu_entry.Click;
          2:
            outer_edges_only_menu_entry.Click;
          3:
            rail_centrelines_only_menu_entry.Click;
        end;//case
      ...
      then begin
        control_room_form.prefs_include_gen_settings_menu_entry.Checked := True;
        // radio item // so can save again.
        if any_bgnd > 0 then
          show_modal_message('Saved program preferences included generator settings.' +
            #13 + #13 + 'Click the "generator > rebuild all background" menu item' + #13 +
            'to apply the new settings to the background templates.');
      end
      else
      ...
      help_font_style := get_pref_int('07-02i', help_font_style);
      // 0=normal  1=bold  215a
      help_font_size := get_pref_int('07-04i', help_font_size);
      // px  215a

      help_client_width_as_user :=
        Round(get_pref_int('07-10i', Round(help_client_width_as_user / global_factor)) * global_factor);
      // user set sizes, adjust for any change in scaling
      help_client_height_as_user :=
        round(get_pref_int('07-20i', Round(help_client_height_as_user / global_factor)) * global_factor);
      ...
      print_corner_page_numbers_font.Assign(
        get_pref_font('08-0', print_corner_page_numbers_font));
      ...
      case print_colour_prefs of
        0: begin
          grey_shade := False;
          black_white := False;
          pad_form.colour_printing_menu_entry.Checked := True;
        end;           // radio item
        1: begin
          grey_shade := True;
          black_white := False;
          pad_form.grey_shade_printing_menu_entry.Checked := True;
        end;       // radio item
        2: begin
          grey_shade := False;
          black_white := True;
          pad_form.black_and_white_printing_menu_entry.Checked := True;
        end;  // radio item
      end;//case

      with pad_form.printed_grid_solid_menu_entry do
        Checked := get_pref_bool('08-17b', Checked); // 207a radio item
      with pad_form.printed_grid_dotted_menu_entry do
        Checked := get_pref_bool('08-18b', Checked); // 207a radio item

      with pad_form.thin_printed_lines_menu_entry do
        Checked := get_pref_bool('08-20b', Checked); // 207a radio item
      with pad_form.normal_printed_lines_menu_entry do
        Checked := get_pref_bool('08-21b', Checked); // 207a radio item
      with pad_form.thick_printed_lines_menu_entry do
        Checked := get_pref_bool('08-22b', Checked); // 207a radio item
      with pad_form.set_line_thicknesses_menu_entry do
        Checked := get_pref_bool('08-23b', Checked); // 207a radio item

      with pad_form.adjust_line_thickness_menu_entry do
        Checked := get_pref_bool('08-25b', Checked);   // 207a

      printgrid_thick := get_pref_float('08-31f', printgrid_thick);
      // mm 207a ...
      ...
      with pad_form.timber_numbering_on_plain_track_menu_entry do
        Checked := get_pref_bool('08-45b', Checked); // 208a
      ...
      //--------  storage box
      with keep_form.briefly_hide_on_store_menu_entry do
        Checked := get_pref_bool('10-00b', Checked); // 205e now radio item. 205c
      with keep_form.hide_on_store_menu_entry do
        Checked := get_pref_bool('10-01b', Checked); // 205e radio item
      with keep_form.dont_hide_on_store_menu_entry do
        Checked := get_pref_bool('10-02b', Checked); // 205e radio item
      with keep_form.reveal_on_store_menu_entry do
        Checked := get_pref_bool('10-03b', Checked); // 206a radio item
      with keep_form.alert_on_store_menu_entry do
        Checked := get_pref_bool('10-04b', Checked); // 206c radio item
...
begin
  ...
  then begin
    if (prefs_existed_on_startup = False) or (prefs_available = False) then
      Result := True                // beginner user, not using prefs, or prefs since abandoned.

    else begin
      // not running with available prefs, ask about saving them now
      repeat
        i := alert(4, '      save  program  preferences ?',
          ...
          +
          '||If you answer yes, your previously saved preferences will be replaced if you have not changed the current preferences file name.' + '||If you choose to abandon your preferences, you won''t see the option to use your preferences when you start the next Templot0 session.' + '||If you are unsure, click the green bar.| ', '', 'more  options  and  information', 'abandon  my  saved  preferences', 'yes  -  save  my  current  preferences', 'cancel  quit   -   continue  running', 'no  -  quit  without  changing  my  saved  preferences   ', 2);

        case i of
          2:
            control_room_form.saved_preferences_setup_menu_entry.Click;
          3: begin
            Result := True;
            abandon_prefs(False);
          end;
    ...
    Result := True;
    if (prefs_available = True) and
      (control_room_form.update_prefs_on_quit_menu_entry.Checked = True) then
      save_prefs('', False);
  end;
end;
//______________________________________________________________________________
...
begin
  if FileExists(Config.FilePath(cudiData, prefs_pointer_str)) then begin
    ...
    show_modal_message('Error - You have no saved preferences to abandon.');
  ...
  if help_form.Showing = True then
    do_help(-200, prepare_prefs_html_str, False)
  else
    help(-200, prepare_prefs_html_str, '');
end;
//______________________________________________________________________________
...
begin
  if load_file = True then
    prefs_form.Caption := '  reload  preferences  file ...'
  else
  ...
  try
    ...
    then begin
      ...
      if str = '' then begin
        show_modal_message('No file was selected - click a file in the list to select it.'
          + #13 + #13 + 'Action cancelled.');
        EXIT;
      end;
      ...
      prefs_file_name := str;
      if load_file = True then
        load_prefs(prefs_file_name, True, True, False);     // load file, show changes
...
begin
  with math_form do begin
    ...
    then begin
      ...
      if prefs_file_name = '' then
        prefs_file_name := get_prefs_file_name + '_new';  // default new name
...
begin
  ...
  if str = get_prefs_file_name then begin
    show_modal_message('The selected file is in current use and cannot be deleted.' +
      #13 + #13 + 'Please first change to using a different current file.');
    EXIT;
  end;
  ...
  try
    with cgs_list do begin
      ...
      for n := 0 to 3 do begin    // custom A to D ...
        ...
        Values['11-' + IntToStr(n) + '0s'] := prepare_string_for_prefs(gauge[index].name_str_glist);
        // gauge designation: 9 chars max

        Values['11-' + IntToStr(n) + '1s'] :=
          prepare_string_for_prefs(gauge_form.gauge_listbox.Items.Strings[index]);
        // 215a    full line in selection list.
        ...
        Values['11-' + IntToStr(n) + '5f'] := FloatToStr(gauge[index].fwe_glist);
        // mm flangeway end (flangeway+flare).
        Values['11-' + IntToStr(n) + '6f'] := FloatToStr(gauge[index].trtscent_glist);
        // mm track centres, turnout side.
        Values['11-' + IntToStr(n) + '7f'] := FloatToStr(gauge[index].trmscent_glist);
        // mm ditto, main side.
        Values['11-' + IntToStr(n) + '8f'] := FloatToStr(gauge[index].min_radius_glist);
    ...
    with cgs_list do begin
      ...
      for n := 0 to 3 do begin    // custom A to D ...
        ...
        with gauge[index] do begin
          ...
          name_str_glist := get_pref_string_cgs(cgs_list, '11-' + IntToStr(n) + '0s', name_str_glist);
          // gauge designation: 9 chars

          gauge_form.gauge_listbox.Items.Strings[index] :=
            get_pref_string_cgs(cgs_list, '11-' + IntToStr(n) + '1s', gauge_form.gauge_listbox.Items.Strings[index]);
          // 215a   full line in selector list.

          scale_glist := get_pref_float_cgs(cgs_list, '11-' + IntToStr(n) + '2f', scale_glist);
          // mm per ft.
          gauge_glist := get_pref_float_cgs(cgs_list, '11-' + IntToStr(n) + '3f', gauge_glist);
          // mm.
          fw_glist := get_pref_float_cgs(cgs_list, '11-' + IntToStr(n) + '4f', fw_glist);
          // mm flangeway.
          fwe_glist := get_pref_float_cgs(cgs_list, '11-' + IntToStr(n) + '5f', fwe_glist);
          // mm flangeway end (flangeway+flare).
          trtscent_glist := get_pref_float_cgs(cgs_list, '11-' + IntToStr(n) + '6f', trtscent_glist);
          // mm track centres, turnout side.
          trmscent_glist := get_pref_float_cgs(cgs_list, '11-' + IntToStr(n) + '7f', trmscent_glist);
          // mm ditto, main side.
          min_radius_glist := get_pref_float_cgs(cgs_list, '11-' + IntToStr(n) + '8f', min_radius_glist);
          // mm minimum radius for check.
