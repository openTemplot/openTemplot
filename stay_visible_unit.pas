...
begin
  ...
  if fast_button.Checked=True then help_scrollbox.VertScrollBar.Increment:=Canvas.TextHeight(' ') div 2;  // scrolling jumps 1/2 line at a time.
...
begin
  help_scrollbox.Color:=get_colour('choose  a  new  colour  for  the  stay-visible  text  panel',help_scrollbox.Color);
end;
...
begin
  text_label.Font.Assign(get_font('choose  a  new  font  and  text  colour  for  the  stay-visible  text',text_label.Font,True));
...
begin
  if (Showing=True) and (no_onresize=False)
     then begin
            ...
            if text_label.Width>1 then label_height:=limits_i(12000, 32000, 2400000 div text_label.Width)  // maintain nominal area 12000 x 200
                                  else label_height:=32000;
...
begin
  help_scrollbox.VertScrollBar.Increment:=Canvas.TextHeight(' ') div 2;  // scrolling jumps 1/2 line at a time.
end;
//________________________________________________________________________________________
...
begin
  ...
  // OT-FIRSTClientWidth:=288;
  // OT-FIRSTClientHeight:=376;
  AutoScroll:=True;
end;
