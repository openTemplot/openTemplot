...
const
  ...
  +'||Sorry, detailed help notes for platforms are not yet written. Please refer to the <A HREF="go_to_templot_club.85a">Templot&nbsp;Club</A> user forum for help.'
  +'||rp.gif If you are currently using the `0adjacent rails`3 option as a design guide, clicking the `0real > platforms...`1 menu item will cancel the adjacent rails.'
  +' It is not possible for Templot0 to show adjacent rails and platforms on the same template.'
...
begin
  ...
  with platform_form do begin
    ...
    if ts_platform_checkbox.Checked=True then ts_platform_checkbox.Color:=ts_panel.Color
                                         else ts_platform_checkbox.Color:=Color;

    if ms_platform_checkbox.Checked=True then ms_platform_checkbox.Color:=ms_panel.Color
                                         else ms_platform_checkbox.Color:=Color;

  end;//with form

  if (draw_ts_platform=True) or (draw_ms_platform=True) then pad_form.adjacent_trackbed_platforms_menu_entry.Click;  // switch adjacent tracks off if necessary

  if (save_ts=False) and (draw_ts_platform=True)   // just added a platform, set default lengths (leave widths).
     then begin
            platform_ts_start_mm:=0;
            platform_ts_length_mm:=def_req;        // def_req converted in strails(); to full template length.
          end;

  if (save_ms=False) and (draw_ms_platform=True)   // just added a platform, set default lengths (leave widths).
     then begin
            platform_ms_start_mm:=0;
            platform_ms_length_mm:=def_req;        // def_req converted in strails(); to full template length.
          end;
...
begin
  ...
  with platform_form do begin
    ...
    if draw_ms_platform=True then ms_platform_checkbox.Color:=ms_panel.Color
                             else ms_platform_checkbox.Color:=Color;
...
begin
  ...
  mouse_action_selected('    adjust  turnout - side  platform  start ...','turnout - side  platform  start',captext(platform_ts_start_mm)+' mm');
  plat_ts_start_mod:=1;
end;
//______________________________________________________________________________
...
begin
  ...
  if platform_ts_length_mm=def_req
     then platform_ts_length_mm:=turnoutx-platform_ts_start_mm;
  ...
  mouse_action_selected('    adjust  turnout - side  platform  length ...','turnout - side  platform  length',captext(platform_ts_length_mm)+' mm');
...
begin
  ...
  mouse_action_selected('    adjust  turnout - side  platform  starting  width ...','turnout - side  platform  starting  width',captext(platform_ts_start_width_ins)+'  inches full-size');
...
begin
  ...
  mouse_action_selected('    adjust  turnout - side  platform  ending  width ...','turnout - side  platform  ending  width',captext(platform_ts_end_width_ins)+'  inches full-size');
...
begin
  ...
  mouse_action_selected('    adjust  main - side  platform  start ...','main - side  platform  start',captext(platform_ms_start_mm)+' mm');
  plat_ms_start_mod:=1;
end;
//______________________________________________________________________________
...
begin
  ...
  if platform_ms_length_mm=def_req
     then platform_ms_length_mm:=turnoutx-platform_ms_start_mm;
  ...
  mouse_action_selected('    adjust  main - side  platform  length ...','main - side  platform  length',captext(platform_ms_length_mm)+' mm');
...
begin
  ...
  mouse_action_selected('    adjust  main - side  platform  starting  width ...','main - side  platform  starting  width',captext(platform_ms_start_width_ins)+'  inches full-size');
...
begin
  ...
  mouse_action_selected('    adjust  main - side  platform  ending  width ...','main - side  platform  ending  width',captext(platform_ms_end_width_ins)+'  inches full-size');
...
const
  ...
  ts_start_skew_str:string='Enter the amount by which the start of the turnout-side platform is skewed, in model mm.'
  ...
  ts_end_skew_str:string='Enter the amount by which the end of the turnout-side platform is skewed, in model mm.'
...
begin
     putdim(ts_start_str, 1,'turnout-side  platform  starts  at',platform_ts_start_mm, True,False,False,False);  // no neg, preset ok, zero ok, don't terminate on zero.
     putdim(ts_length_str,1,'turnout-side  platform  length',    platform_ts_length_mm,True,False,True, False);  // no neg, preset ok, no zero, don't terminate on zero.

     putdim(ts_start_skew_str,1,'turnout-side  platform  start  skewed  by',platform_ts_start_skew_mm,False,True,False,False);  // neg ok, no preset, zero ok, don't terminate on zero.
  n:=putdim(ts_end_skew_str,  1,'turnout-side  platform  end  skewed  by',  platform_ts_end_skew_mm,  False,True,False,False);  // neg ok, no preset, zero ok, don't terminate on zero.

  if n<>3 then EXIT;
  if getdims('turnout - side  platform  position',platform_help_str,platform_form,n,od)=True
     then begin
            platform_ts_start_mm:=od[0];
            if platform_ts_start_mm=def_req then platform_ts_start_mm:=0;
...
begin
     putdim(ts_width1_str,2,'turnout-side  platform  starting  width  (F-S inches)',platform_ts_start_width_ins, True,False,False,False);  // no neg, preset ok, zero ok, don't terminate on zero.
  n:=putdim(ts_width2_str,2,'turnout-side  platform  ending  width  (F-S inches)',  platform_ts_end_width_ins,   True,False,False, False); // no neg, preset ok, zero ok, don't terminate on zero.
  if n<>1 then EXIT;
  if getdims('turnout - side  platform  widths',platform_help_str,platform_form,n,od)=True
     then begin
            platform_ts_start_width_ins:=od[0];
            if platform_ts_start_width_ins=def_req then platform_ts_start_width_ins:=144;  // preset 12ft

            platform_ts_end_width_ins:=od[1];
            if platform_ts_end_width_ins=def_req then platform_ts_end_width_ins:=144;  // preset 12ft
          end;
...
const
  ts_spacing_str:string='Enter the spacing of turnout-side (TS) platform edge from the track centre-line, in full-size inches.||For UK standard-gauge practice this dimension should be 57 inches.';
...
begin
  n:=putdim(ts_spacing_str,2,'turnout-side  (TS)  platform  spacing  (F-S inches)',platform_ts_front_edge_ins, False,False,False,False);  // neg ok, preset ok, zero ok, don't terminate on zero.
  if n<>0 then EXIT;
  if getdims('turnout - side  platform  spacing',platform_help_str,platform_form,n,od)=True
     then begin
            platform_ts_front_edge_ins:=od[0];
            if platform_ts_front_edge_ins=def_req then platform_ts_front_edge_ins:=57;   // 4ft-9in default
          end;
...
const
  ...
  ms_start_skew_str:string='Enter the amount by which the start of the main-side platform is skewed, in model mm.'
                          +'||A negative dimension shortens the rear edge of the platform at the start.'
                          +'||A positive dimension extends the rear edge of the platform at the start, but not beyond the template boundary.'
                          +'||For a square-ended platform enter 0 (zero) to cancel skewing.';

  ms_end_skew_str:string='Enter the amount by which the end of the main-side platform is skewed, in model mm.'
                          +'||A negative dimension shortens the rear edge of the platform at the end.'
                          +'||A positive dimension extends the rear edge of the platform at the end, but not beyond the template boundary.'
                          +'||For a square-ended platform enter 0 (zero) to cancel skewing.';
...
begin
     putdim(ms_start_str, 1,'main-side  platform  starts  at',platform_ms_start_mm, True,False,False,False);  // no neg, preset ok, zero ok, don't terminate on zero.
     putdim(ms_length_str,1,'main-side  platform  length',    platform_ms_length_mm,True,False,True, False);  // no neg, preset ok, no zero, don't terminate on zero.

     putdim(ms_start_skew_str,1,'main-side  platform  start  skewed  by',platform_ms_start_skew_mm,False,True,False,False);  // neg ok, no preset, zero ok, don't terminate on zero.
  n:=putdim(ms_end_skew_str,  1,'main-side  platform  end  skewed  by',  platform_ms_end_skew_mm,  False,True,False,False);  // neg ok, no preset, zero ok, don't terminate on zero.

  if n<>3 then EXIT;

  if getdims('main - side  platform  position',platform_help_str,platform_form,n,od)=True
     then begin
            platform_ms_start_mm:=od[0];
            if platform_ms_start_mm=def_req then platform_ms_start_mm:=0;

            platform_ms_length_mm:=od[1];  // return def_req, converted in strails(); to full template length.
...
begin
     putdim(ms_width1_str,2,'main-side  platform  starting  width  (F-S inches)',platform_ms_start_width_ins, True,False,False,False);  // no neg, preset ok, zero ok, don't terminate on zero.
  n:=putdim(ms_width2_str,2,'main-side  platform  ending  width  (F-S inches)',  platform_ms_end_width_ins,   True,False,False, False); // no neg, preset ok, zero ok, don't terminate on zero.
  if n<>1 then EXIT;
  if getdims('main - side  platform  widths',platform_help_str,platform_form,n,od)=True
     then begin
            platform_ms_start_width_ins:=od[0];
            if platform_ms_start_width_ins=def_req then platform_ms_start_width_ins:=144;  // preset 12ft

            platform_ms_end_width_ins:=od[1];
            if platform_ms_end_width_ins=def_req then platform_ms_end_width_ins:=144;  // preset 12ft
          end;
...
const
  ms_spacing_str:string='Enter the spacing of main-side (MS) platform edge from the track centre-line, in full-size inches.||For UK standard-gauge practice this dimension should be 57 inches.';
...
begin
  n:=putdim(ms_spacing_str,2,'main-side  (MS)  platform  spacing  (F-S inches)',platform_ms_front_edge_ins, False,False,False,False);  // neg ok, preset ok, zero ok, don't terminate on zero.
  if n<>0 then EXIT;
  if getdims('main - side  platform  spacing',platform_help_str,platform_form,n,od)=True
     then begin
            platform_ms_front_edge_ins:=od[0];
            if platform_ms_front_edge_ins=def_req then platform_ms_front_edge_ins:=57;   // 4ft-9in default
          end;
...
begin
  ...
  mouse_action_selected('    adjust  main - side  platform  skew  at  start ...','main - side  platform  skew  at  start',captext(platform_ms_start_skew_mm)+' mm');
  plat_ms_skew1_mod:=1;
end;
//______________________________________________________________________________
...
begin
  ...
  mouse_action_selected('    adjust  main - side  platform  skew  at  end ...','main - side  platform  skew  at  end',captext(platform_ms_end_skew_mm)+' mm');
  plat_ms_skew2_mod:=1;
end;
//______________________________________________________________________________
...
begin
  ...
  mouse_action_selected('    adjust  turnout - side  platform  skew  at  start ...','turnout - side  platform  skew  at  start',captext(platform_ts_start_skew_mm)+' mm');
  plat_ts_skew1_mod:=1;
end;
//______________________________________________________________________________
...
begin
  ...
  mouse_action_selected('    adjust  turnout - side  platform  skew  at  end ...','turnout - side  platform  skew  at  end',captext(platform_ts_end_skew_mm)+' mm');
  plat_ts_skew2_mod:=1;
end;
//______________________________________________________________________________
