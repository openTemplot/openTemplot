...
//--------------------------------
  ...
  procedure pan_button_click(scroll:extended; dir:integer; max_jump:boolean);  // called from pad for edge-panning.
...
procedure pan_button_click(scroll:extended; dir:integer; max_jump:boolean);  // also called from pad for edge-panning.
...
begin
  if mouse_modify>0 then EXIT;   // mouse actio in progress.

  if draw_mode<>2 then pad_form.lock_scaling_menu_entry.Click;   // 13-12-99.

  with panning_form do begin

    if max_jump=False then jump_factor:=(coarse_trackbar.Position*10+1)/3000    // 211b
                      else jump_factor:=0.1;
  end;//with

  case dir of
       1: zoom_offsetx:=zoom_offsetx - jump_factor*screenx*scroll;
       2: zoom_offsetx:=zoom_offsetx + jump_factor*screenx*scroll;
       3: zoom_offsety:=zoom_offsety + jump_factor*screeny*scroll;
       4: zoom_offsety:=zoom_offsety - jump_factor*screeny*scroll;
  end;//case
...
const
  ...
  +'||Most of the time in Templot0 you will probably prefer to pan across the trackpad by dragging it with the mouse or by using the arrow keys on the keyboard.'
  +'||And most of the time in Templot0 you will probably prefer to zoom the trackpad by means of the mouse wheel.'

  +'||The zoom/pan dialog is provided for occasions when you want more control, or your system does not support the above functions.|<HR>'

  +'|To use less screen space click the `0<BIG>«</BIG>`1 button repeatedly to shrink the zoom/pan dialog window. To expand the dialog again click the `0<BIG>»</BIG>`1 button repeatedly.'
  +' If the zoom/pan dialog has been hidden, press `0CTRL+F2`2 or the `02`2 key, or click the `0pad > zoom/pan options > show zoom/panning controls`1 menu item to show the dialog again.'

  +'||The `0+`1 and `0-`1 zoom in/out buttons correspond to the `0+`2 and `0-2 (ADD/SUBTRACT) number pad keys.'
  +' The amount by which the zoom setting changes for each click can be set by selecting the `0TRACKPAD > ZOOM (EXPLODE/SHRINK) > EXPLODE/SHRINK STEP SIZE...`1 menu item.'

  +'||Alternatively, click the `0+.:`1 zoom rectangle button and draw a rectangle on the trackpad around the area which you want to zoom into. (Click down on one corner and drag to the other.)'
  +'||For precise zooming, click the `0zoom by mouse action`1 button to zoom with the mouse.'

  +'||The panning function moves the field of view across the trackpad continuously.'
  +'||Click or hold down one of the four blue arrow buttons to pan across the pad. Or alternatively, press or hold down the ARROW keys on the keyboard.'

  +'||If the `0PAPER`1 option button has been selected the trackpad image'
  +' will move in the same direction as the button. If the `0SCROLL`1 option button has been selected the trackpad image will move in the'
  +' opposite direction, in the same way as a normal Windows scroll bar.'

  +'||The speed of movement is controlled by the `0panning speed`1 slider. If the `0CTRL`2 key on the keyboard is held down while using the panning buttons, panning will be at the maximum speed, regardless of the slider setting.'
  ...
  +' If the keyboard arrow keys are used, the speed will also be determined by the current keyboard repetition rate setting.'

  +'||If `0SCROLL LOCK`2 key is OFF and the `0EDGE-PANNING > ON`1 option button is currently selected (or the `0TRACKPAD > ZOOM AND PAN OPTIONS > EDGE PANNING ON PER SCROLL LOCK`1 menu item is currently selected),'
  +' you can also pan across the trackpad simply by moving the mouse pointer against the edge of the pad.'
  +'||To allow for access to the menus, this works for the left-hand edge of the trackpad only if the mouse pointer is in the lower 2/3rds of the trackpad, and for the top edge of the trackpad only at the right-hand end.'
  +' The speed slider and `0CTRL`2 key operate as for the panning buttons, but the `0PAPER/SCROLL`1 direction buttons are ignored for edge-panning.'
  +'||Edge panning is occasionally a nuisance, and can be temporarily disabled by pressing the `0SCROLL LOCK`2 key to ON.'

  +'||Some combinations of speed setting and grid-line spacing will cause the trackpad to appear to be moving in the wrong direction, due to stroboscopic effects.'
  +' To avoid this change the speed setting or grid spacing, or switch the trackpad grid off while panning (select the `0TRACKPAD > TRACKPAD GRID OPTIONS`1 menu item).'
  +'||Handy Hints:'
  +'||When using the faster speeds it is usually better to make repeated clicks on the panning buttons instead of holding them down. Otherwise you may quickly lose your bearings.'
  +' Be aware that the trackpad is virtually limitless in area, you will not come to a stop at the "edge of the paper".'
  +'||If you do lose your way on the pad, click the `0TRACKPAD > ZOOM/VIEW > FIT ALL BACKGROUND TEMPLATES`1 menu item (or press `0SHIFT-F12`2 or just the `0PAGE UP`2 key).'
  +'||Alternatively, click the `0TRACKPAD > ZOOM/VIEW > TRACKPAD VIEW RESET MAX`1 menu item. This zooms out to the full available extent, with the drawing origin'
  +' located at the centre of the pad. You can then zoom back in as required (using the `0+`2 button or the `0ADD`2 key on the number pad).';
...
begin
  if Top<56 then Top:=56;                                                           // leave room for edge-panning or shrunk jotter.
...
procedure Tpanning_form.up_panbuttonMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
...
begin
  if scroll_option_button.Checked=True then scroll:=1.0
                                       else scroll:=-1.0;
...
procedure Tpanning_form.up_panbuttonMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
...
begin
  ...
  if panning_form.Active=True then blue_corner_panel.SetFocus;        // don't want it here.
           
end;
//___________________________________________________________________________________________

procedure Tpanning_form.down_panbuttonMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
...
procedure Tpanning_form.left_panbuttonMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
...
procedure Tpanning_form.right_panbuttonMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
...
procedure Tpanning_form.set_label4MouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
...
procedure Tpanning_form.set_label3MouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
...
procedure Tpanning_form.set_label2MouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
...
procedure Tpanning_form.set_label1MouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
...
begin
  if pad_view_list.Count<1 then EXIT;  // no views in list.

  next_view_button.Enabled:=True;  // enable reverse for 5 seconds.

  INC(pad_view_index);
  if pad_view_index>(pad_view_list.Count-1) then pad_view_index:=0;  // back to top.
...
begin
  ...
  DEC(pad_view_index);
  if pad_view_index<0 then pad_view_index:=pad_view_list.Count-1;  // back to bottom.
