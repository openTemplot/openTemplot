...
var
  ...
  current_diffed_len:extended=0;     // all in model mm... (set in procedure update_check_diff_panels)
...
uses pad_unit, math_unit, control_room, help_sheet, colour_unit, alert_unit, entry_sheet;
...
const
  ...
  +'||If the `0retain on new mint`1 box is ticked, any adjustments will be preserved when creating a new mint template. (`0template > new template (quick set)...`1 menu item, or `0NEW`z tool-button.)'
  +'||If the `0retain on tools: make`1 box is ticked, any adjustments will be preserved when creating a new template using the `0tools > make ---`1 menu items.'
  +' This is the same setting as the `0tools > make tools: options > retain adjusted check rails`1 menu option.'

  +'||Sorry, no further help notes here yet. Please click the link above for more information and diagrams.'

   +'||green_panel_begintree.gif continuous check rails:'
   +'||It is not possible for a plain track template to have check rails. However, a length of plain track with a continuous check rail on one side (for use on sharp curves), or both sides (for use in roadways and level crossings),'
   +' can be created by modifying a turnout template.'
   +'||First add some approach track (`0F3`2 mouse action) of suitable length.'
   ...
   +'||Then put the fixing peg at the toe joint (`0CTRL-1`2) and click `0DO > SNAP TO PEG`1 menu item.'
   +'||To create a full length of continuous check rail with flared ends at both ends, peg two such templates together at their `0CTRL-1`2 positions.'
   +'||Or tick the `0retain on tools: make`1 box and then click the `0TOOLS > MAKE MIRROR ON PEG`1 menu item.'
   +'||To swap a single check rail to the opposite side, click the `0TEMPLATE > INVERT HANDING`1 menu item (or `0CTRL-X`2).'
   +'||Remember that despite appearances these are <U>turnout</U> templates, so to adjust their length use the `0F3`2 `0approach length`3 mouse action (not `0F4`2).'
   +'green_panel_end';
...
const
  mod_str:string='||Enter a dimension by which the normal dimensions of this check!!! rail end should be modified.'
  +'||To return this check!!! rail end to its normal dimensions, enter 0 (zero).'
  +'||green_panel_begintree.gif  handy hints:'
  +'||It is usually much easier to adjust a check or wing rail by using the buttons and mouse actions, instead of direct entry of data here.'
  +'||These settings are relative to the normal dimensions of the check!!! rail end, not the present dimensions if it has been adjusted previously.'
  +'||Instead of these relative dimensions, you can enter the actual length, flare length, or end gap dimension by clicking the relevant read-out panel.'
  +'green_panel_end';
...

{
  ...
  if num_str='' then EXIT;

  if (current_diff_code=503) or (current_diff_code=506)
     then end_str:=StringReplace(mod_str,'check!!!','wing',[rfReplaceAll,rfIgnoreCase])
     else end_str:=StringReplace(mod_str,'check!!!','check',[rfReplaceAll,rfIgnoreCase]);

  help_str:='    `0Data for Adjusting Rail End '+num_str+'`9'+end_str;

  this_diff:=get_checkrail_diff(current_diff_code);

    putdim(help_str,2,       'adjust  '+num_str+'  full - size  length  by',this_diff.len_diff,False,True,False,False); // neg ok, no preset, 0 ok, don't terminate on zero.
    putdim(help_str,2,'adjust  '+num_str+'  full - size  flare  length  by',this_diff.flr_diff,False,True,False,False); // neg ok, no preset, 0 ok, don't terminate on zero.
 i:=putdim(help_str,1,           'adjust  '+num_str+'  model  end  gap  by',this_diff.gap_diff,False,True,False,False); // neg ok, no preset, 0 ok, don't terminate on zero.

  if i<>2 then EXIT;

  if getdims('adjust  rail  end  '+num_str,'',pad_form,i,od)=True
     then begin
...
begin
  cancel_adjusts(False);
  if (current_diff_code<501) or (current_diff_code>508) then EXIT;

  this_diff:=get_checkrail_diff(current_diff_code);

  this_diff.type_diff:=new_flare_type;      //byte; // 0=no type diff   1=change to bent flare    2=change to machined flare

  get_current_diffed_dims;  // get current results, return string not wanted
  if current_diffed_fl_len<minfp then this_diff.flr_diff:=0;           // make sure there is some flare length
  if (current_diffed_end_gap-fw)<minfp then this_diff.gap_diff:=0;     // and some bend-out
...
begin
  ...
  this_diff.flr_diff:=this_diff.flr_diff-current_diffed_fl_len/inscale;  // reduce diff to make flare length zero

  this_diff.gap_diff:=this_diff.gap_diff-(current_diffed_end_gap-fw);  // reduce diff to make bend-out zero
...
const
  ...
  +'||All check and wing rail lengths a measured from the centre of the ''A'' timber (the timber which carries the nose of the crosing vee). Click the link above for more information and diagrams.'
  +'||green_panel_begintree.gif  handy hints:'
  +'||You can change between using model sizes (in mm) and full-size prototype dimensions (in inches) by clicking the `0MODEL MM`1 or `0FULL-SIZE INCHES`1 option buttons.'
  +'||To enter adjustment data relative to the normal length of this check!!! rail end, cancel this and click the `0DATA...`1 button instead.'
  +'||It is usually much easier to lengthen or shorten a check or wing rail by using the buttons and mouse actions, instead of direct entry of data here.'
  +'green_panel_end';
...

{
  ...
  i:=putdim(help_str,fs_code,num_str+'  end  length'+fs_str,current_diffed_len/fs_convert,False,True,False,False); // neg ok, no preset, 0 ok, don't terminate on zero.
  if i<>0 then EXIT;

  if getdims(num_str+'  rail  end  length','',pad_form,i,od)=True
     then begin
            mod_diff_by_mm:=od[0]*fs_convert-current_diffed_len;

            this_diff.len_diff:=this_diff.len_diff+mod_diff_by_mm/inscale;    // len_diff is in fs ins
...
const
  ...
  +'||The flare length is the length of the angled section at the end of the check!!! rail. Click the link above for more information and diagrams.'
  +'||green_panel_begintree.gif  handy hints:'
  +'||You can change between using model sizes (in mm) and full-size prototype dimensions (in inches) by clicking the `0MODEL MM`1 or `0FULL-SIZE INCHES`1 option buttons.'
  +'||To enter adjustment data relative to the normal flare length of this check!!! rail end, cancel this and click the `0DATA...`1 button instead.'
  +'||It is usually much easier to lengthen or shorten a flare length by using the buttons and mouse actions, instead of direct entry of data here.'
  +'green_panel_end';
...

{
  ...
  i:=putdim(help_str,fs_code,num_str+'  flare  length'+fs_str,current_diffed_fl_len/fs_convert,False,True,False,False); // neg ok, no preset, 0 ok, don't terminate on zero.
  if i<>0 then EXIT;

  if getdims(num_str+'  rail  flare  length','',pad_form,i,od)=True
     then begin
            mod_diff_by_mm:=od[0]*fs_convert-current_diffed_fl_len;

            this_diff.flr_diff:=this_diff.flr_diff+mod_diff_by_mm/inscale;  // flr_diff is in fs ins
...
const
  ...
  +'||The end gap is the widened opening at each end of a check rail or at the far end of a wing rail. Click the link above for more information and diagrams.'
  +'||For most UK prototypes this dimension is 3.5" full-size.'
  +'||For model use it needs to be increased from the scale size where wider than scale flangeway gaps are used.'
  +'||green_panel_begintree.gif  handy hints:'
  +'||You can change between using model sizes (in mm) and full-size prototype dimensions (in inches) by clicking the `0MODEL MM`1 or `0FULL-SIZE INCHES`1 option buttons.'
  +'||To enter adjustment data relative to the normal end gap for this check!!! rail end, cancel this and click the `0DATA...`1 button instead.'
  +'green_panel_end';
...

{
  ...
  i:=putdim(help_str,fs_code,num_str+'  end  gap'+fs_str,current_diffed_end_gap/fs_convert,False,True,False,False); // neg ok, no preset, 0 ok, don't terminate on zero.
  if i<>0 then EXIT;

  if getdims(num_str+'  rail  end  gap','',pad_form,i,od)=True
     then begin
...
begin
  ...
  if end_str='' then EXIT;

  mouse_diff:=get_checkrail_diff(current_diff_code);

  mouse_action_selected('    adjust  '+end_str+'  length ...','adjust  '+end_str+'  length','adjust  by : '+captext(mouse_diff.len_diff*inscale)+' mm');
...
begin
  ...
  mouse_action_selected('    adjust  '+end_str+'  flare  length ...','adjust  '+end_str+'  flare  length','adjust  by : '+captext(mouse_diff.flr_diff*inscale)+' mm');
...
begin
  ...
  mouse_action_selected('    adjust  '+end_str+'  end  gap ...','adjust  '+end_str+'  end  gap','adjust  by : '+captext(mouse_diff.gap_diff)+' mm');
...
begin
  ...
  if half_diamond=true then pad_form.k_crossing_check_rails_menu_entry.Click;
...
begin
  ...
  if this_diff.len_diff<min_diff then this_diff.len_diff:=min_diff;
