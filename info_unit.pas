...
begin
  ...
  if new_width1>new_width2 then new_width:=new_width1
                           else new_width:=new_width2;
...
begin
  save_dialog.Filename:=exe_str+Copy(Trim(remove_invalid_str(current_name_str)),1,20){+remove_invalid_str(suffix_str)}+'.txt';
  ...
  if save_dialog.Execute=True
     then begin
            ...
            if box_project_title_str<>'' then WriteLn(info_file,' '+gauge_label.Caption+'    for  '+box_project_title_str+'    ref : '+current_name_str)
                                     else WriteLn(info_file,' '+gauge_label.Caption+'    ref : '+current_name_str);
            ...
            if info_memo.Lines.Count<1                        // info empty (???).
               then WriteLn(info_file,'    blank')
               else for n:=0 to info_memo.Lines.Count-1 do WriteLn(info_file,info_memo.Lines[n]);
            ...
            repeat                      //  find memo lines separated by | chars.
              ...
              if i>0
                 then begin
                        WriteLn(info_file,Copy(s,1,i-1));    // extract a string line from the memo.
...
const
  ...
  +'||This panel displays information about the control template showing on the trackpad. The data in this panel is continuously updated as the control template is adjusted by mouse action.'
  +'||The various items on this panel are:'
  +'||The `0HIDE`1 button removes the panel from view to give a full view of the trackpad area. To see'
  +' the panel again, select the `0trackpad > SHOW INFORMATION PANEL`1 menu item. The panel can be more'
  +' conveniently toggled into and out of view by pressing the `0F2`2 key repeatedly.'

  +'||The `0EXPAND`1 button expands the panel to its full size, if it is not already that size.'

  +'||The `0SHRINK`1 button reduces the panel to the bare minimum which contains useful information'
  +' while remaining in view.'

  +'||The top-left bar (showing normally in pale blue) displays the current gauge/scale and size details for the control template. Clicking this bar will hide the control template, in a similar way to pressing the `0HOME`2 key.'
  +' While the control template is hidden, this bar shows in pale grey. Clicking it again will show the control template again on the trackpad.'

  +'||The `0smallest radius now:`3 box shows the smallest track radius at any point on the control template. If this is less than the limit value'
  +' set in the `0warn if radius under:`3 box, the warning lamp flashes red. Otherwise it shows green. To change the limit value, click the `0CHANGE..`1 button.'
  +' This warning setting is specific to each template, which allows you to set different radius limits for different parts of your track plan.'

  +'||The radius warning function is not available when the template contains a turnout and a `0slew`3 (slewing is intended mainly for plain track.'
  +' For more information about slewing, select the `0GEOMETRY > SLEW (NUDGE) > SLEWING DATA...`1 menu item.)'

  +'||The lower half of the information panel comprises the `0info`3 area which contains dimensional details about the control template.'
  +'||By adjusting the scroll bars on this area, and also the scroll bars for the whole information panel while re-sizing it,'
  +' it is possible to keep the dimensions of interest in view while the main panel is at a much reduced size to give a clearer view of the trackpad.'
  +' The `0FIT TO WINDOW`1 button resizes the info area to fit the available space in the main panel. Use this button after re-sizing the main panel in the usual way by dragging the edges or the bottom-right corner.'

  +'||The `0JOT ALL`1 button copies all the template info to the jotter as a handy future reference.'
  +' The `0JOT VISIBLE`1 button copies only the currently visible lines of info to the jotter.'

  +'||The `0VIEW`1 button displays the current info data in a more easily readable form, which then remains unchanged and visible.'

  +'||The `0EDIT`1 button is similar but duplicates the info data in an editable form in a separate window which updates continuously.'

  +'||The `0WRITE...`1 button will save the info data to a text file, and the `0PRINT`1 button will print it on your printer.'
  +' The current printer text font is used for this, to change it select the `0PROGRAM > PRINTER FONT + MARGINS...`1 menu item on the `0program panel`3 window.'
  +' This info data is also printed out along with each printed control template if required.'

  +'||The `0name`3 of the control template is shown above the `0info`3 area. To change the name, click the `0RENAME..`1 button, or on the name itself.'
  +' Or click on the status bar along the bottom of the panel to use the name showing there, which is the name of the last template copied back from your storage box.'

  +'||The `0MEMO`1 button opens a window in which you can edit or paste any notes you wish to make about the control template, which will be included with it when you store it or copy it to the background.'
  +' (The memo notes can be further edited on the storage box later.)'

  +'||If the `0RINGS > RING`1 tickbox is ticked the ring warning lamp will flash if any part of the control template infringes (crosses into) the current size and position of the spacing-ring tool.'
  +' This is useful when adjusting the template to be clear of structures, obstructions, baseboard joints and edges, etc. If the `0COPIES`1 tickbox is ticked any ring copies which have been created will also be checked for infringements.'

  +'||The amount by which the control template is clear of (shown in bue) or infringing (shown in red) the spacing-ring is shown below the warning lamp.'
  +' Normally the outer-edges of the rails are checked against the inner diameter of the ring, and in this case the warning lamp flashes red when the ring is infringed and shows a steady green when the control template''s rails are clear of the ring.'

  +'||In the case of a `0centre-line only`3 template, the track centre-line is checked against the inner diameter of the ring,'
  +' and in this case the warning lamp flashes white when the ring is infringed and shows a steady pale blue when the track centre-line is clear.'
  +' If desired this mode can be selected for all templates by clicking the `0trackpad > INFORMATION PANEL OPTIONS > RING INFRINGEMENTS > WARN IF CENTRE-LINES INFRINGE`1 menu item.'

  +'||N.B. The figures shown apply only to the spacing-ring itself, not to any ring copies. It is possible for the warning lamp to be flashing (because of an infringed ring copy) while the figures show clear (because the spacing-ring itself is clear).'
  +' For more information about using the spacing-ring tool, click the `0UTILS > DUMMY VEHICLE • SPACING-RING`1 menu item, and then click the `0? HELP`1 button.'

  +'||The `0trackpad data:`3 box shows the current zoom scale of the trackpad, i.e. the model size represented by the full visible width of the trackpad. If this is free to change'
  +' as changes are made to the length of the control template the label will show ZOOM FREE in green. If the label shows LOCKED AT in white, then the current trackpad zoom setting'
  +' will remain fixed. To change to free zooming, click the `0UNLOCK`1 button.'
  +' To lock the trackpad zoom setting at any size, click the `0LOCK AT..`1 button. (These zoom options are also available on the `0trackpad`1 menu and on the right-click menu.)'

  +'||N.B. The free zooming option is useful when adjusting stock templates which are on their drawing datum (green dot). When working on a complete track plan you will normally'
  +' want to have the zoom setting locked, and use the + and - buttons and other zoom controls and mouse actions to change it.'

  +'||The `0trackpad data:`3 box can be scrolled to see more zoom data and the current position of the pegging notch.';
...
begin
  ...
  memo_str:='    '+ref_name_label.Caption
      +'||'+gauge_label.Caption
      +'||--------------------------------------------------------------'
...
begin
  ...
  if line_height<1 then EXIT;

  n1:=Round(info_scrollbox.VertScrollBar.Position/line_height);    // starting line index.

  if (n1>-1) and (n1<info_memo.Lines.Count)
     then begin
              n2:=Round((info_scrollbox.VertScrollBar.Position+info_scrollbox.ClientHeight)/line_height)-1;       // ending line if whole scrollbox visible.
              n3:=Round((info_scrollbox.VertScrollBar.Position+ClientHeight-info_scrollbox.Top)/line_height)-1;   // ending line if part scrollbox obscured.

              if n2>n3 then n2:=n3; // use shortest number of lines.

              if n2<0 then EXIT;
              if n2>(info_memo.Lines.Count-1) then n2:=info_memo.Lines.Count-1;
              if n2<n1 then EXIT;


              jotter_form.jotter_memo.Lines.Add('__');

              for n:=n1 to n2 do jotter_form.jotter_memo.Lines.Add(info_memo.Lines[n]);
          end;
end;
//__________________________________________________________________________________________
...
begin
  if (ring_copies_warn_checkbox.Checked=True) and (ring_index<1)   // no copies.
     then begin
            repeat
              ...
              if i=4 then help(0,ring_help_str,'');
            until i<>4;
  ...
  if (ring_warn=False) and (ring_copies_warn=False)
     then begin
            ring_lamp_panel.Color:=info_form.Color;
            ring_lamp_panel.Tag:=2;                   // flashing disabled.
          end;
...
begin
  if pad_form.show_control_template_menu_entry.Checked=True             // 209c mods
     then pad_form.hide_control_template_menu_entry.Click
     else pad_form.show_control_template_menu_entry.Click;
end;
//___________________________________________________________________________________________
