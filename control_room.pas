...
const
  ...
  +'||Because the DXF files from Templot0 ignore the printer calibration and output scaling functions, these data distortions are provided instead as a means of making'
  ...
  +' distortions when printing directly from Templot0 the grid will therefore be inaccurate and should be turned off.'
  ...
  +' are intended for use when printing in sideways (landscape) format. Templot0 does NOT check this however and will distort both ways at once if you'
  ...
  +' printing directly from Templot0, where any part of the drawing to the left of or below the page outlines is cropped.)';
...
coning_help_str:string='      Coning  Distortions'
  ...
  def_req:extended=0-1.0E300;              // DON'T CHANGE (used directly as the value in some places 0.93.a)  // this value in a float requests a default to be used instead.
...
uses
  ShellAPI, Math, Clipbrd, {FileCtrl,} {@demo Clipbrd,} Printers, alert_unit, help_sheet, math_unit,
  pad_unit, info_unit,
  ...
  if (wine_warn_msg_pref=False) or (over_ride=True)
     then begin
            ...
            alert(2,'   Running  on   Linux / Wine / CrossOver',
                  ...
                  +'||To return to using Windows native fonts it will be necessary to restart Templot0.',
                    '','','','','','OK',0);
...
begin
  if alert(1,'php/990    running  under  Linux / Wine / CrossOver',
         ...
         +'||The '+str+' dialog is required to be modal, which means that it must be completed before Templot0 can continue.'
         ...
         +'||If you click elsewhere on the screen the dialog may become hidden behind other windows, after which Templot0 will not be able to continue until you find the dialog again and complete it.'
         ...
         +'||This is a feature of Wine beyond Templot0''s control.'
...
const
  ...
  +' "Stored" templates are held only in Templot0''s memory, so that each template is instantly available without the need to access files.'
  +'||Templot0 can automatically restore the contents of your storage box and your background track plan (if any) between sessions, but it can only "remember" one set of box contents at any one time.'

  +'||For future reference and reloading whenever you need them, you may also want to save the current box contents in a separate named data file before quitting Templot0.'
  ...
  +' This works independently of the option settings for automatic restore on startup. You can click RESTORE PREVIOUS as often as you wish, the data from your previous Templot0 session remains available throughout the current session.'

  +'||Likewise, the FILES > RESTORE PRIOR-PREVIOUS menu item restores the box contents from the Templot0 session prior to the previous one, i.e. from two sessions back.'
  ...
  +'||N.B. If you run two instances of Templot0 concurrently (not recommended for Windows 95/98/ME) from the same TEMPLOT folder,'
...
begin
  ...
  case quit_code of
                   ...
                   if (keeps_list.Count=0) and (RESULT=True)   // nothing else to save.
                      then EXIT                                // so don't alert him again, just quit.
                             ...
                             case i of
                                 4: RESULT:=True;
                                 5: RESULT:=False;
                                 6: if save_done=False then RESULT:=save_box(0,0,0,'')  //  go do the save.
                                                       else RESULT:=True;
                             end;//case
                           end;
                   EXIT;
            ...
            else RESULT:=False;  // !!! no other cases of quit_code yet 14-6-98.
  end;//case
end;
//______________________________________________________________________________
...
try
  ...
  if pad_form.WindowState<>wsMinimized then pad_form_window_state:=pad_form.WindowState;   // 0.91.b   for use in AppRestore

  if (on_idle_can_run=False) or (under_way=False) or (prog_running=False) then EXIT;

  with pad_form do begin

    if (plain_track=True) or (half_diamond=False) or (tradius_is_straight=True)
       then indicator_prefix_str:=''
       else indicator_prefix_str:='I';    // irregular half-diamond

    if plain_track=True                                    // 212a
       then xing_indicator_panel.Font.Color:=clSilver
       else xing_indicator_panel.Font.Color:=clGreen;

    if retpar_i=1
       then xing_indicator_panel.Caption:=indicator_prefix_str+'P'    // 212a ...
       else begin
              case xing_type_i of
               -1: xing_indicator_panel.Caption:=indicator_prefix_str+'G';   // generic xing
                0: xing_indicator_panel.Caption:=indicator_prefix_str+'R';   // regular
                1: xing_indicator_panel.Caption:=indicator_prefix_str+'C';   // curviform
              end;//case
            end;
       ...
       then begin
              with info_form do begin
                if (plain_track=False) or (slew_mode=2)     // min rad info not available for slewed turnouts or any mode 2.
                   ...
                   else begin                        // OK to show min rad if plain track and mode 1.
    ...
    if distortions=0
       then distortion_warning_panel.Hide
       else begin
              ...
              if (info_form.Left<(distortion_warning_panel.Left+distortion_warning_panel.Width))
                 ...
                 then info_form.Top:=distortion_warning_panel.Top+distortion_warning_panel.Height*2;  // ensure warning not obscured by info.
            end;

    if (re_org_x=0) and (re_org_y=0)
       then reorg_warning_panel.Hide
       else begin
              ...
              if (info_form.Left<(reorg_warning_panel.Left+reorg_warning_panel.Width))
                 ...
                 then info_form.Top:=reorg_warning_panel.Top+reorg_warning_panel.Height*2;  // ensure warning not obscured by info.
            end;
    ...
    snap_to_EGTP_menu_entry.Enabled:= NOT (plain_track or spiral or slewing or (egpx>(xorg-minfp_big)) );
    snap_to_IGTP_menu_entry.Enabled:= NOT (plain_track or spiral or slewing or (igpx>(xorg-minfp_big)) );
       ...
       then begin
              ...
              case slew_mode of
                              1: begin
                                   ...
                                   slewing_panel.Caption:='caution :  template  contains  a  SLEW  ( mode  1 )';
                                   info_form.slew_caution_mode_label.Caption:='caution :    this  template  contains  a  SLEW  ( mode  1 )';
                                 end;
                              2: begin
                                   slew_mode2_menu_entry.Checked:=True;            // radio item.
                                   adjust_slew2_factor_menu_entry.Enabled:=True;
                                   slewing_panel.Caption:='caution :  template  contains  a  SLEW  ( mode  2 )';
                                   info_form.slew_caution_mode_label.Caption:='caution :    this  template  contains  a  SLEW  ( mode  2 )';
                                 end;
              end;//case
    ...
    if spiral=True then begin
                          ...
                          info_form.curving_label.Caption:=curving_label_prefix_str+'tr';    // 213a
                        end
                   else if ABS(nomrad)<max_rad_test
                           then begin
    ...
    if dummy_template=True then dummy_template_menu_radio.Checked:=True   // radio items  211c
                           else if cl_only=True then centre_lines_only_menu_radio.Checked:=True       // radio items  0.93.a mods
                                                else if track_centre_lines_flag=True then normal_track_centre_lines_menu_radio.Checked:=True
                                                                                     else no_track_centre_lines_menu_radio.Checked:=True;   // radio items

    switch_drive_menu_entry.Checked:=(switch_drive_flag=True) and (plain_track=False) and (half_diamond=False);  // 0.82.a
    ...
    swell_menu_entry.Enabled:= NOT (spiral OR slewing);    // no swell mouse action for transition curves or slews.

    if plain_track=True
       then begin
              template_menu_top_entry.Caption:='PLAIN  TRACK :';
              adjust_turnout_approach_menu_entry.Caption:='adjust  pl&ain-track  length';     // F3   205a
              adjust_length_menu_entry.Caption:='adjust  pl&ain-track  length';               // F4   205a
            end
       else begin
              if half_diamond=False
                 then template_menu_top_entry.Caption:='TURNOUT :'
                 else template_menu_top_entry.Caption:='HALF-DIAMOND :';

              adjust_turnout_approach_menu_entry.Caption:='adjust  turnout  &approach  length';     // F3   205a
              adjust_length_menu_entry.Caption:='adjust  overall  template  l&ength';               // F4   205a
    ...
    convert_turnout_to_half_diamond_menu_entry.Enabled:=(plain_track=False) and (half_diamond=False);
    ...
    if half_diamond=False then mint_menu_entry.Caption:='&mint  from  current'
                          else mint_menu_entry.Caption:='&mint  turnout  from  current';
    ...
    if timbinc<minfp    // gradual lengths
       then begin
              ...
              if (ms_ends=False) or (half_diamond=True)
                 then timbers_centralized_menu_entry.Checked:=True  // radio item.
                 else timbers_in_line_menu_entry.Checked:=True;     // radio item.
            end;

    if no_timbering=True
       then no_timbering_menu_entry.Checked:=True  // radio item
       else begin
              if half_diamond=True
                 then half_diamond_timbering_style_menu_entry.Checked:=True
                 else begin
                        if timbers_equalized=True
                           then begin
                                  if equalizing_fixed=False then equalized_incremental_menu_entry.Checked:=True  // radio item.
                                                            else equalized_constant_menu_entry.Checked:=True;    // radio item.
                                end
                           else begin
                                  if square_on_angled=False then square_on_menu_entry.Checked:=True              // radio item.
                                                            else angled_on_menu_entry.Checked:=True;             // radio item. 29-7-01.
                                end;
                      end;
            end;
    ...
    timbers_centralized_menu_entry.Enabled:=(timbinc>minfp); // (nine_foot=True) or (eight_foot_six=True);
    timbers_in_line_menu_entry.Enabled:=(timbinc>minfp) { ((nine_foot=True) or (eight_foot_six=True))} and NOT half_diamond;

    if platform_form.Showing=True
       then begin
              platform_buttons;   // 0.93.a update the platforms form
              if (draw_ts_platform=True) or (draw_ms_platform=True)
                 then begin
    ...
    if trackbed_form.Showing=True
       then begin
              trackbed_buttons;   // 215a update the trackbed form
              if (draw_ts_trackbed_edge=True) or (draw_ms_trackbed_edge=True)
                 then begin
                        adjacent_trackbed_platforms_menu_entry.Checked:=True;  // radio item
                        adjacent_edges:=True;
                        do_railedges;
  ...
  if export_form.Showing=True       // 208a
     then begin
            export_form.export_dxf_button.Enabled:= NOT export_form.export_control_template_radiobutton.Checked;
          end;


  if grid_form.Showing=True      // 0.98.a
     then begin
            grid_form.dummy_vehicle_make_copy_button.Enabled:=grid_form.show_dummy_vehicles_radio_button.Checked;

            if half_diamond=True
               then grid_form.turnout_road_dummy_vehicle_radio_button.Caption:='on diagonal road'
               else grid_form.turnout_road_dummy_vehicle_radio_button.Caption:='on turnout road';

            if plain_track=True
               then begin
                      grid_form.main_road_dummy_vehicle_radio_button.Checked:=True;     // radio item
                      grid_form.turnout_road_dummy_vehicle_radio_button.Enabled:=False;
                    end
               else grid_form.turnout_road_dummy_vehicle_radio_button.Enabled:=True;
          end;

  do_info_colours; // indicate if control template visible and accessible...


  if print_busy=True then EXIT; //  do nothing if printer busy using data.

  if data_changed=True       //  only re-calc and draw on data change (from redraw;),
                             //  not if error in calc occurred.
     then begin
            if abandon_calcs=True
               then begin
                      case alert(2,'    calculations abandoned',
                                  ...
                                  ,'','','','re-calculate  and  show  diagnostic  information','cancel  -  restart  with  new  B-6  turnout','try  nearest  turnout  to  fit',0)
                              of
                              4: begin
                              ...
                              6: pad_form.nearest_menu_entry.Click;
                            else pad_form.nearest_menu_entry.Click;
                      end;//case
                    end
                      ...
                      do_rollback:=True;                  // default for next call.                                      // only one redraw per undo.
                    end;
  ...
  if backup_wanted=True then create_backup_file(False);     // sets backup_wanted False;


finally

  if Application.Active=False
     then Done:=True           // not active.
     else Done:=allow_idle;    // True means abandon time-slice and back to Windows.
...
begin
  try
    ...
    ShowMessage('Sorry, your computer does not have enough free memory to run '+Application.Title+'.'
               +#13+#13
               +'Try restarting '+Application.Title+' with no other programs running.');  // 0.93.a
  ...
  for index:=0 to html_back_c do begin
    with html_back[index] do begin
      ...
      src_str:='<FONT COLOR="RED">Sorry, this page slot is empty.</FONT>'
              +'||Please click the &nbsp;`0&lt;`1 &nbsp;or &nbsp;`0&gt;`1 &nbsp;buttons (back or forward) repeatedly until you find your previous pages.'
              +'||There are 8 cyclic page slots (&nbsp;labelled a&nbsp;-&nbsp;h&nbsp;) in the roll-back register for this Help viewer.'
              +'||For more help information visit the full indexed web help pages at: <A HREF="go_to_templot_companion.85a">Templot Companion</A>.'
              +'||To return to the program, click the `0continue`1 or `0hide`1 button below.';
      src_position:=0;
    end;//with
  end;//next
...
{ OT-FIRST
  if Win32Platform=VER_PLATFORM_WIN32_NT   // running under Windows NT/2000/XP/Vista/8/10 , set 32-bit default...  0.93.a was 24-bit
     then begin}
            ...
            max_draw_int:=1000000000;   //  these are actually less than 31-bit integers for drawing limits...
...
{ OT-FIRST          end
  ...
  if user_prefs_in_use=True                // GO button clicked with prefs
     then load_prefs('',True,False,True);  // get file, don't show changes, startup msg prefs only

  // 211b out       if Screen.PixelsPerInch=96 then pad_form.pad_timber_font_label.Font.Size:=7;   // (bitmap font.)


  if Screen.DesktopWidth<700      // was .Width 0.91.b  640 x 480 is better with bitmap screen fonts...
     then begin
  ...
  if hi_color=False                   //  8-bit (or lower)
     then begin                       //  set the form colours for lo-colour...
            ...
            start_colours:=0;                                    // hi-color, so normal scheme anyway.

          end;

  if initdone_flag=False then templot_init;       //  only initialise once.
     ...
     then begin
            ...
            if alert(7,'   Templot0   -   Multiple  Monitors',
                      ...
                      +' If you then extend the trackpad beyond that, part of the trackpad area will be unusable.| ',
                       '','','','no  -  match  primary  monitor  only','','yes  -  use  all  available  monitors',0)=6
               then he_wants_multiple_monitors:=True;
  ...
  if (GetKeyState(VK_CAPITAL) AND 1)<>0    // caps lock on at startup...
     then begin
            alert(3,'   Templot0   -   Caps Lock  ON',
                    ...
                    +'||You can prevent this by switching the Caps Lock OFF. To do this press the  `0CAPS LOCK`2  key once so that the indicator light is no longer showing.'
                    +'||Please click the green OK bar to continue.',
                     '','','','','','O K',0);
          end;
  ...
  if (he_wants_multiple_monitors=True) or (running_under_wine=True)  // monitors 0.91.b,   Wine 212a to allow dragging to find hidden modal dialogs
     then begin
     ...
     else pad_form.WindowState:=wsMaximized;           // single or primary monitor.
  ...
  repeat
    if Application.Terminated=False then Application.ProcessMessages     //  make sure the pad is showing,
                                    else BREAK;
  ...
  if Screen.Width<=(make_slip_form.Width+pad_form.top_toolbar_panel.Width) then make_slip_form.Hide;  // see do_toolbars
  ...
  info_form.Top:=80;  // 216a   deeper toolbar    was 60 0.91.d     //was as above. problems with offset multiple monitors.
  ...
  if FileExists(pbo_str)=False then first_time_here_label.Show;  // 0.97.b  // beginner first few times (no prior-previous)

  init_rollbacks;         // also inits notch rollback and parking bays.

  if (FileExists(ebk1_str)=True) and (FileExists(ebk2_str)=True)  // faulty backup (e.g. power failure while writing disk?)
     then begin
            if FileAge(ebk1_str)>FileAge(ebk2_str) then DeleteFile(ebk1_str)     // so delete the latest.
                                                   else DeleteFile(ebk2_str);
          end;

          // make new prior-previous file if we have an existing previous file and will have a new previous file...

  if ( (FileExists(ebk1_str)=True) or (FileExists(ebk2_str)=True) ) and (FileExists(pb_str)=True)
     then begin
            DeleteFile(pbo_str);         // delete any old prior-previous file.
            RenameFile(pb_str,pbo_str);  // the previous data becomes prior-previous, and we copy the backup file on startup as new previous.
          end;

          // make new previous file if we have data...

  if FileExists(ebk1_str)=True
     then CopyFile(PChar(ebk1_str),PChar(pb_str),False)       // copy previous backup file as new previous box file.
     else if FileExists(ebk2_str)=True
          then CopyFile(PChar(ebk2_str),PChar(pb_str),False)
          else if FileExists(pb_str)=True                                 // or no backups there (quit with empty box?), so create it from existing previous box file.
                  then CopyFile(PChar(pb_str),PChar(ebk1_str),False);
  ...
  for i:=0 to 2 do fill_kd(parking_bay[i]);   // ditto initial fill all parking bays per current pad.

  on_idle_can_run:=True;

  if user_prefs_in_use=True       // GO button clicked with prefs, file already got (for startup messages).
     then begin
            load_prefs('',False,True,False); // get_file,show_them,startup_msg_prefs

            if prefs_warn_msg_pref=False
               then begin
                      alert_box.preferences_checkbox.Checked:=False;       //%%%%
                      alert_box.preferences_checkbox.Show;
                      ...
                      if alert(3,'   Templot0  started  using  saved  program  preferences',
                                ...
                                +'||If you are a new or infrequent user of Templot0 please be aware that running Templot0 using saved program preferences may mean that you won''t see important help messages and hints.'
                                +'||If you experience difficulty you may prefer to quit Templot0 and restart without using saved preferences (by clicking the initial GO button).| ',
                                 '','','','more  information','','O K',4)=4 then saved_preferences_setup_menu_entry.Click;
  ...
  with control_room_form do begin
    ...
    if pad_button.Showing=True then pad_button.SetFocus;
    ...
    if (temp_comp_j is TForm)
      ...
      then begin
             ...
             for i:=temp_comp_j.ComponentCount-1 downto 0 do begin
               ...
               if (temp_comp_i is TButton)
                  then begin
                         if TButton(temp_comp_i).Font.Name='Arial'
                            then TButton(temp_comp_i).Font.Name:='Liberation Sans';
                       end;


               if (temp_comp_i is TPanel)
                  then begin
                         if TPanel(temp_comp_i).Font.Name='Arial'
                            then TPanel(temp_comp_i).Font.Name:='Liberation Sans';
                       end;


               if (temp_comp_i is TLabel)
                  then begin
                         if TLabel(temp_comp_i).Font.Name='Arial'
                            then TLabel(temp_comp_i).Font.Name:='Liberation Sans';

                         TLabel(temp_comp_i).AutoSize:=False;
                       end;


               if (temp_comp_i is TGroupBox)
                  then begin
                         if TGroupBox(temp_comp_i).Font.Name='Arial'
                            then TGroupBox(temp_comp_i).Font.Name:='Liberation Sans';
                       end;

               if (temp_comp_i is TRadioGroup)
                  then begin
                         if TRadioGroup(temp_comp_i).Font.Name='Arial'
                            then TRadioGroup(temp_comp_i).Font.Name:='Liberation Sans';
                       end;


               if (temp_comp_i is TCheckBox)
                  then begin
                         if TCheckBox(temp_comp_i).Font.Name='Arial'
                            then TCheckBox(temp_comp_i).Font.Name:='Liberation Sans';
                       end;


               if (temp_comp_i is TRadioButton)
                  then begin
                         if TRadioButton(temp_comp_i).Font.Name='Arial'
                            then TRadioButton(temp_comp_i).Font.Name:='Liberation Sans';
                       end;


               if (temp_comp_i is TTabSheet)
                  then begin
                         if TTabSheet(temp_comp_i).Font.Name='Arial'
                            then TTabSheet(temp_comp_i).Font.Name:='Liberation Sans';
                       end;


               if (temp_comp_i is TPageControl)
                  then begin
                         if TPageControl(temp_comp_i).Font.Name='Arial'
                            then TPageControl(temp_comp_i).Font.Name:='Liberation Sans';
                       end;


               if (temp_comp_i is TMemo)
                  then begin
                         if TMemo(temp_comp_i).Font.Name='Arial'
                            then TMemo(temp_comp_i).Font.Name:='Liberation Sans';
                       end;


               if (temp_comp_i is TEdit)
                  then begin
                         if TEdit(temp_comp_i).Font.Name='Arial'
                            then TEdit(temp_comp_i).Font.Name:='Liberation Sans';
                       end;


               if (temp_comp_i is TListBox)
                  then begin
                         if TListBox(temp_comp_i).Font.Name='Arial'
                            then TListBox(temp_comp_i).Font.Name:='Liberation Sans';
                       end;


               if (temp_comp_i is TComboBox)
                  then begin
                         if TComboBox(temp_comp_i).Font.Name='Arial'
                            then TComboBox(temp_comp_i).Font.Name:='Liberation Sans';
                       end;
...
begin
  ...
  about_str:='<P STYLE="text-align:center; margin-top:20px;"><IMG SRC="'+exe_str+'internal\hlp\'+logo_img_str+'"></P>'
            ...
            +'<P STYLE="font-size:12px; color:#555555;">'+licence_str+'</P>';
  ...
  if help(-250,about_str,'make  a  donation')=1 then go_to_donation;
...
begin
  if quit_alert_done=False then quit_alert_done:=quit_alert;     //  ask if o.k to close

              // finally prefs (in case changed in quit_alert)...

  if (quit_code=3) and (quit_alert_done=True)        // normal quit..
     then CanClose:=prefs_quit                       // so go save preferences
     else CanClose:=quit_alert_done;                 // abnormal quit, or he cancelled in quit_alert.
     ...
     then begin
            if under_way=True
               then begin
                      ...
                      if CanClose=True
                         then begin
                                with jotter_form.jotter_memo.Lines do begin
                                  if Count>0 then SaveToFile(exe_str+'jotbak.txt');     // jotter text for restore.
                                end;//with

                                with boxmru_list do begin
                                  if Count>0 then SaveToFile(exe_str+'boxmru.txt');     // 0.82.a 22-08-06 mru list for box files restore.
                                end;//with

                                with bgsmru_list do begin
                                  if Count>0 then SaveToFile(exe_str+'bgsmru.txt');     // 0.82.a 22-08-06 mru list for bgs files restore.
                                end;//with

                                save_custom_gauges;  // save custom gauge/scale for next session   215a   in prefs_unit

                                info_form.Close;      //  !!! bug in Delphi - address violation if child forms overlapping the bottom of the pad on quit.
                                grid_form.Close;
                     ...
                     end;                                    //  !!! above lines not needed if pad_form.autoscroll is false.
            if CanClose=True then under_way:=False;          //  no more redraws
          end;
...
const
  ...
  +' containing some words from me, invariably of no great consequence!'
  +'||Martin.';
...
const
  help_str:string='            Program  Panel.'
   ...
   +'||If you have just started TEMPLOT for the first time, click the `0trackpad`1 button to get going, and then click the `0help`1 menu items on the trackpad.'
   +' Later on, you will probably start by clicking the `0RECENT FILES`1 or `0FILE...`1 buttons which load or reload your template storage box with the templates from a previously saved data file.'

   +'||The `0SESSION`1 menu items are functions which you normally need to access only once per working session. You can set the track gauge and modelling scale'
   +' which you wish to use, and choose a title for the project.'

   +'||You can return to the program panel at any time by clicking the `0MAIN > PROGRAM PANEL`1 menu item on the trackpad.'
   +' The program panel can be kept in view by resizing the trackpad window. Closing the program panel will quit Templot0.'

   +'||The `0PROGRAM`1 menu includes various options for experienced users to customise the way Templot0 works.'

   +'||green_panel_begin tree.gif Until you become an experienced user of Templot0 you will probably have little use for the program panel, as most of the menu items are repeated elsewhere in Templot0.green_panel_end';
...
begin
  if (initdone_flag=False) or (room_scaling=True) then EXIT;   // not if resizing from scaling buttons or for dpi-aware at startup
  ...
  with math_form do begin
    ...
    if ModalResult=mrOK
       then begin
              ...
              if box_project_title_str='' then box_project_title_str:='untitled project';
              save_done:=False;
              backup_wanted:=True;
            end;
...
begin
  ...
  repeat
    i:=alert(4,'      transition - custom  terms ...',
             ...
             +' The current custom setting is '+IntToStr(trans_terms)+' terms. You should have a good reason for not using the normal'
             +' AUTO TERMS setting.',
              'important  information  -  please  read',
              'enter  some  other  setting',
              '3  terms  -  30  degree  swing',
              '4  terms  -  60  degree  swing',
              'cancel  -  use  AUTO  TERMS  instead',
              '6  terms  -  90  degree  swing',1);

    case i of
         1: if alert_help(0,help_str+'||Do you want to do this now ?','yes  AUTO  TERMS')=1
               then begin
                      revert_auto:=True;
                      auto_terms_menu_entry.Click;
                    end;
         2: begin
              n:=putdim(help_str,0,'number of transition terms',trans_terms,True,False,True,False);  // no neg, preset ok, no zero, don't terminate on zero.
              if n<>0 then EXIT;

              if getdims('transition  terms','',control_room_form,n,od)=True
                 then begin
                        if od[0]=def_req then trans_terms:=8                  // default 8 transition terms.
                                         else trans_terms:=Round(ABS(od[0]));
                      end;
            end;
         3: trans_terms:=3;
         4: trans_terms:=4;
         5: auto_terms_menu_entry.Click;
         6: trans_terms:=6;
       else EXIT;
    end;//case
...
const
  slow_str:string='      Slow  Running'
    ...
    +'||A suitable count for your computer will need to be established by trial and error. Higher counts cause Templot0 to run more slowly.'
    +' A count in the range 20-100 would be a starting point.'
    +'||To properly see the effect of slow running you will need to be using the LOW MEMORY refresh mode, which requires a Templot0 restart.'
...
begin
  n:=putdim(slow_str,0,'slow  running  count',slow_run,True,False,False,False);  // no neg, preset ok, zero ok, don't terminate on zero.
  if n<>0 then EXIT;
  if getdims('slow  running','',control_room_form,n,od)=True
     then begin
            if od[0]=def_req then slow_run:=0
                             else slow_run:=ABS(od[0]);
          end;

  if slow_run=0 then run_slow_menu_entry.Checked:=False
                else run_slow_menu_entry.Checked:=True;
end;
//________________________________________________________________________________________
...
const
  max_spiral_str:string='    Expert  Help  -  Max  Spiral'
    +'||There is a limit to the "gentleness" of transition curves (spirals) which can be calculated. A transition which is too gentle (i.e. too small a change of radius and/or in too long a transition zone)'
    +' is likely to be distorted, possibly severely.'
    +'||Templot0 therefore limits transitions to a maximum "gentleness", i.e. a maximum value for the "spiral constant" showing in the INFORMATION panel.'
    +'||You can change the value of this limit according to the capabilities of your processor and your acceptable distortion. Experience suggests that AMD processors (Athlon, Duron) can accept a higher limit than Pentium processors.'
    +'||The pre-set limit is 500. Higher figures will allow more gentle transitions, but also more distortion.'
    +'||If distortion is a problem, try reducing the smaller radius slightly. Often only a small change is needed. Shortening the transition zone length can also be tried, but in this case a more significant reduction may be needed.'
    +'||CAUTION: Setting too high a limit could cause Templot0 to appear to "hang" when adjusting transition curves by mouse action. A figure in excess of 2000 is unlikely to produce a usable template.'
    +' An alternative to a very gentle transition is to use a series of constant radius templates, each varying very slightly from the next in radius. In some cases a small amount of slew will also give the desired result.';
...
begin
  n:=putdim(max_spiral_str,0,'max  spiral  constant',max_spiral_constant/1.0E6,True,False,False,False);  // no neg, preset ok, zero ok, don't terminate on zero.
  if n<>0 then EXIT;
  if getdims('max  spiral','',control_room_form,n,od)=True
     then begin
            if od[0]=def_req then max_spiral_constant:=500.0E6
                             else max_spiral_constant:=ABS(od[0]*1.0E6);
          end;
end;
//_________________________________________________________________________________________
...
const
  help_str:string='      Expert  Help  -  Step  Size'
           +'||Templot0 calculates the alignment of the rails by stepping along them. The size of each step can be changed, although there is normally'
           +' no reason to do this. The pre-set step size is calculated as the square root of the scale equivalent of 9 feet in mm. For example, in'
           +' 4 mm scale, 9 feet is 36 mm, and the step size is therefore 6 mm. In 7 mm scale the step size is the square root of 63, i.e. approximately 8 mm.'
           +'||If you are having problems with a slow processor, try setting a significantly larger step size to reduce the amount of calculations required for each'
           +' screen re-draw. If you set a very large step size, remember to return to the pre-set step size before printing the template, otherwise your curved rails'
           +' will be noticeably dog-legged. A sensible maximum step size is about 8ft scale (32 mm in 4mm scale), beyond which the drawing is likely to become significantly disfigured.'
           +'||There is no advantage in using a step size smaller than the pre-set unless you are printing very sharp curves on an extremely high resolution printer, or using the print'
           +' output scaling function to enlarge the drawing by several hundred percent.'
           +'||A radius of 500 mm would need to be printed at more than 2500 dpi ( 8 times better than many inkjets ) to resolve the difference between a'
           +' perfect curve and one drawn in 6 mm steps. And a radius of 900 mm ( a practical minimum in 4 mm scale ) would require better than 5000 dpi.'
           +' Do not be misled by the advertised resolution of some printers, which refers to their image manipulation software - only very expensive models can be'
           +' expected to physically position the print head to better than 600 dpi.'
           +'||If you do set a smaller step size, the extra calculations required may slow down your screen re-draws, or lead to memory problems.'
           +'||The step size used is saved as part of the template specification in the storage box and re-used on reloading. The pre-set step size is'
           +' restored when you do a B-6 TURNOUT RESET or change the scale.'
           +'||The range of permitted step size is from 3" scale to 25ft scale ( 1-100 mm in 4mm scale ).';
...
begin
  repeat
    m:=incx;
    n:=putdim(help_str,1,'calculation step size along rail-lines',m,True,False,True,False);  // no neg, preset ok, no zero, don't terminate on zero.
    if n<>0 then EXIT;
    if getdims('step  size','',control_room_form,n,od)=True
       then begin
              m:=od[0];
              if m=def_req then Break;

              if (m<(scale/4)) or (m>(25*scale)) or (m<0.01)   // 3" to 25 ft max sensible range (1-100 mm for 4 mm scale).
                 then begin
                          i:=alert(0,'','||The step size is outside the permitted range :||  '+round_str(scale/4,2)+'  to  '+round_str(25*scale,2)+'  mm'
                                 ,'','','','','cancel','re-try',0);
                          if i=5 then EXIT;
                      end
                 else Break;
  ...
  if ABS(incx-m)>minfp then incx:=m;     // set any new incx.
...
begin
  if distortions=0 then EXIT;       // might be a B-6 reset.
  if cleared_bgnd=False then EXIT;  // first clear any background keeps.
...
const
  help_str:string='      Expert  Help  -  Re-origination'
           ...
           +'||c. Re-origination applies globally for as long as it remains in force, so all templates drawn on the trackpad or in the storage box will be'
           +' re-originated, but will return to normal when re-origination is cancelled. The re-origination data is not included in the template files'
           +' when they are saved from the box, and must be re-entered as required for each working session.'
           +'||( Handy hint - make a note of your re-origination settings in your memo text. They can then be quickly copied and pasted back into the data entry form after reloading.)'
           +'||This re-origination function is intended primarily for use with data distortions when templates'
           +' are being output for use elsewhere (e.g. exported in DXF format) - there should normally be no reason to use it when printing directly'
           +' from Templot0. For more information select the PROGRAM > EXPERT > DATA DISTORTIONS > ? HELP menu item.'
           +'||Enter X and Y settings ( + / -  mm ) for the amount of shift required. X dimensions are positive screen left-to-right,'
           +' Y dimensions are positive screen bottom-to-top.'
           +'||Note that re-origination affects only the drawing data. Grid lines and printed trim margins are unaffected (and are not included in DXF files).';
...
begin
  if cleared_bgnd=False then EXIT;  // first clear any background keeps.
     ...
     putdim(help_str,1,'re - origination  shift  X   ( from left )  ',re_org_x/100,False,False,False,False); // neg ok, preset ok, zero ok, don't terminate on zero.
  n:=putdim(help_str,1,'re - origination  shift  Y  ( from bottom )',re_org_y/100,False,False,False,False);  // neg ok, preset ok, zero ok, don't terminate on zero.
  if n<>1 then EXIT;
  if getdims('re - origination','',control_room_form,n,od)=True
     then begin
            if od[0]=def_req then re_org_x:=0
                             else re_org_x:=od[0]*100;     // in 1/100th mm.

            if od[1]=def_req then re_org_y:=0
                             else re_org_y:=od[1]*100;
          end;

  if (re_org_x=0) and (re_org_y=0) then re_org_menu_entry.Checked:=False
                                   else re_org_menu_entry.Checked:=True;
  redraw(True);
end;
//_________________________________________________________________________________________
...
const
  zero_help_str:string='   Trailing  Zeros'
                      +'||For the drawing calculations Templot0 always uses the maximum precision available from your processor, but normally rounds results to two decimal places'
                      +' when displaying or printing them, with trailing zeros omitted but at least one decimal place always shown, e.g.'
...
begin
  i:=alert(4,'   negative  numbers',
            ...
            +'||The brackets can be omitted if preferred.',
             '','','','omit  brackets  from  negative  numbers','cancel  -  no  change','show  negative  numbers  in  brackets',0);
  case i of
      4: omit_neg_brackets:=True;
      6: omit_neg_brackets:=False;
  end;//case
  ...
  repeat
    i:=alert(4,'   trailing  zeros',
               ...
               '','?  help',
               'always show two decimal places (e.g. 8.20, 1.00)',
               'omit all trailing zeros (e.g. 8.2, 1 )',
               'cancel  -  no  change',
               'omit second place zero only (e.g. 8.2, 1.0)',2);
    case i of
        2: alert_help(0,zero_help_str,'');
        3: zero_supp:=0;    // omit no zero decimal places.
        4: zero_supp:=2;    // omit both zero decimal places.
        5: EXIT;
        6: zero_supp:=1;    // omit second zero decimal place only.
      else run_error(30);
    end;//case
  until i<>2;
end;
...
const                               // these overide global declarations so that I can use these names elsewhere if nec.
     TECHNOLOGY      =   2;
     HORZRES         =   8;
     VERTRES         =  10;
     BITSPIXEL       =  12;
...
begin
  ...
  n:=GetDeviceCaps(control_room_form.Canvas.Handle, BITSPIXEL);   //  number of colour bits per pixel per plane.
...
begin
  ...
  with heap_status do begin
    ...
    s:=s+'|| $ '+IntToHex(TotalAddrSpace,8)+'    =    '+FormatFloat('0000000,',TotalAddrSpace)+'    Total Requested Address Space';
    s:=s+'||of which:';

    s:=s+'|| $ '+IntToHex(TotalUncommitted,8)+'    =    '+FormatFloat('0000000,',TotalUncommitted)+'    is Uncommitted';
    s:=s+'| $ '+IntToHex(TotalCommitted,8)+'    =    '+FormatFloat('0000000,',TotalCommitted)+'    is Committed';

    s:=s+'||------------------------------';

    s:=s+'|| $ '+IntToHex(FreeSmall,8)+'    =    '+FormatFloat('0000000,',FreeSmall)+'    Free in Small Blocks';
    s:=s+'| $ '+IntToHex(FreeBig,8)+'    =    '+FormatFloat('0000000,',FreeBig)+'    Free in Large Blocks';
    s:=s+'| $ '+IntToHex(Unused,8)+'    =    '+FormatFloat('0000000,',Unused)+'    Free Unused';
    s:=s+'| $ '+IntToHex(TotalFree,8)+'    =    '+FormatFloat('0000000,',TotalFree)+'    Total Free';

    s:=s+'|| $ '+IntToHex(Overhead,8)+'    =    '+FormatFloat('0000000,',Overhead)+'    Manager Overhead';

    s:=s+'|| $ '+IntToHex(TotalAllocated,8)+'    =    '+FormatFloat('0000000,',TotalAllocated)+'    Total Allocated';
...
begin
  ...
  with printer_list do begin
    if Count>0 then for n:=0 to printer_list.Count-1 do Tprint_cal(printer_list.Objects[n]).Free;
  end;//with
  ...
  if pad_view_list.Count>0 then for n:=0 to pad_view_list.Count-1 do Tpad_view(pad_view_list.Objects[n]).Free;  // 0.91.c
...
const
  ...
  +' I have been advised, however, that this is not always the case.'
  +'||The coning and skewing distortions are my attempt to offer a correction for worn or poor quality printers. How well they work'
  +' in practice rather depends on the individual printer of course. Have plenty of scrap paper to hand before experimenting !';
...
const
 ...
 +'||If your intention is simply to enlarge or reduce a printed template, cancel this and select instead the PRINT > PRINTED OUTPUT SCALING menu item.'
 +'||If your printer is not producing accurate results, use the PRINT > PRINTER CALIBRATION function to make the necessary'
 +' corrections. If the problem with your printer persists, due perhaps to wear and tear, try using the SKEWING'
 +' or CONING distortions.'
 +'||X-dimensions (across the screen from the left) are multiplied by the X-scaling factor.'
 +'||The result of setting a X-scaling factor of 200%, for example, is that the drawing expands to double its length.'
 +' The X-scaling factor has no effect on the width of the drawing. To make small corrections a typical X-scaling factor setting would be'
 +' close to 100% and look like 100.71% or 98.35%, for example. If you make large distortions, be aware that the results of the mouse actions and'
 +' other settings may be unexpected or become unpredictable.'
 +'||The effect of setting a negative X-scaling factor is that the drawing is'
 +' mirrored 180 degrees about the left margin, and will not be visible without re-origination or a shift.'
 +' Also, the mouse actions and other settings will be reversed in direction, causing much confusion.'
 +'||The X-scaling factor is reset to 100% when you do a B-6 TURNOUT RESET - otherwise it will remain in force until you'
 +' come here again and change it. So please remember to reset it to 100% after experimenting. If you forget to do so,'
 +' all your subsequently printed templates will be useless!'
 +' ||When using X-scaling distortion the printed grid (which is unaffected by distortions) will no'
 +' longer be accurate - so please heed the warnings about not using these data distortions when printing directly from Templot0.'
 +'||When exporting in DXF format the normal printer calibration and output scaling settings are ignored and the DXF file'
 +' does not include the grid or page trim lines.'
 +'||X-scaling distortion applies globally for as long as it remains in force, so all templates drawn on the trackpad or in the storage box will be'
 +' distorted but will return to normal when the distortion is cancelled. The X-scaling factor is not included in the template files'
 +' when they are saved from the box, and must be re-entered as required for each working session.'
 +'||( Handy hint - make a note of your scaling settings in your memo text. They can then be quickly copied and pasted back into the data entry form after reloading.)';
...
begin
  ...
  try
    repeat
      n:=putdim(help_str,4,'X - scaling  factor',x_distortion_factor*100,False,False,True,False);  // neg ok, preset ok, no zero, don't terminate on zero.
      if n<>0 then EXIT;
      if getdims('X - scaling  factor',distortion_help_str,control_room_form,0,od)=True
         then begin
                if od[0]=def_req then x_distortion_factor:=1
                                 else begin
                                        if ABS(od[0])<minfp then od[0]:=1;
                                        x_distortion_factor:=od[0]/100;
                                        x_scaling_menu_entry.Checked:=True;
                                      end;
              end
         else x_distortion_factor:=1;
      ...
      if x_distortion_factor=1 then begin
                                    ...
                                    i:=alert(7,'        X - scaling  distortion ?',
                                              ...
                                              +'||Are you sure you want to distort your data ?',
                                               '','','yes  -  use  '+round_str(temp*100,2)+' %   X - scaling  distortion','?  help','no  -  cancel  distortion',
                                               'no  -  do  printer  calibration  instead',4);
                                    if i=3 then EXIT
                                           else begin
                                                  x_distortion_factor:=1;                     // he's not sure so cancel.
                                                  x_scaling_menu_entry.Checked:=False;
                                                  case i of
                                                      4: begin
                                                            if alert_help(0,'Clicking HELP implies that you are unsure about the X-scaling factor'
                                                                           ...
                                                                           +'|||'+help_str,'cancel  distortion')=1 then EXIT;
                                                            end;
                                                      5: EXIT;
                                                      6: begin
    ...
    if x_distortion_factor=1 then distortions:=distortions and $FFFE    // clear x bit in the flags byte.
                             else distortions:=distortions or  $0001;   // set it.
  end;//try
  redraw(True);
end;
//_________________________________________________________________________________________
...
const
 ...
 +' corrections. If the problem with your printer persists, due perhaps to wear and tear, try using the SKEWING'
 +' or CONING distortions.'
 +'||Y-dimensions (up the screen from the bottom) are multiplied by the Y-scaling factor.'
 +'||The result of setting a Y-scaling factor of 200%, for example, is that the drawing expands to double its width.'
 +' The Y-scaling factor has no effect on the length of the drawing. To make small corrections a typical Y-scaling factor setting would be'
 +' close to 100% and look like 100.71% or 98.35%, for example. If you make large distortions, be aware that the results of the mouse actions and'
 +' other settings may be unexpected or become unpredictable.'
 +'||The effect of setting a negative Y-scaling factor is that the drawing is'
 +' mirrored 180 degrees about the bottom margin, and will not be visible without re-origination or a shift.'
 +' Also, the mouse actions and other settings will be reversed in direction, causing much confusion.'
 +'||The Y-scaling factor is reset to 100% when you do a B-6 TURNOUT RESET - otherwise it will remain in force until you'
 +' come here again and change it. So please remember to reset it to 100% after experimenting. If you forget to do so,'
 +' all your subsequently printed templates will be useless!'
 +' ||When using Y-scaling distortion the printed grid (which is unaffected by distortions) will no'
 +' longer be accurate - so please heed the warnings about not using these data distortions when printing directly from Templot0.'
 +'||When exporting in DXF format the normal printer calibration and output scaling settings are ignored and the DXF file'
 +' does not include the grid or page trim lines.'
 +'||Y-scaling distortion applies globally for as long as it remains in force, so all templates drawn on the trackpad or in the storage box will be'
 +' distorted but will return to normal when the distortion is cancelled. The Y-scaling factor is not included in the template files'
 +' when they are saved from the box, and must be re-entered as required for each working session.'
 +'||( Handy hint - make a note of your scaling settings in your memo text. They can then be quickly copied and pasted back into the data entry form after reloading.)';
...
begin
  ...
  try
    repeat
      n:=putdim(help_str,4,'Y - scaling  factor',y_distortion_factor*100,False,False,True,False);  // neg ok, preset ok, no zero, don't terminate on zero.
      if n<>0 then EXIT;
      if getdims('Y - scaling  factor',distortion_help_str,control_room_form,0,od)=True
         then begin
                if od[0]=def_req then y_distortion_factor:=1
                                 else begin
                                        if ABS(od[0])<minfp then od[0]:=1;
      ...
      if y_distortion_factor=1 then begin
                                    ...
                                    i:=alert(7,'        Y - scaling  distortion ?',
                                               'Setting the Y-scaling factor to any value other than 100% will cause'
                                              +' your template to be unusable when printed. This function is intended as'
                                              +' a means of correction only when the output will be elsewhere.'
                                              +'||If your printer is not producing accurate results, do a printer calibration instead.'
                                              +'||Are you sure you want to distort your data ?',
                                               '','','yes  -  use  '+round_str(temp*100,2)+' %   Y - scaling  distortion','?  help','no  -  cancel  distortion',
                                               'no  -  do  printer  calibration  instead',4);
                                    if i=3 then EXIT
                                           else begin
                                                  y_distortion_factor:=1;                     // he's not sure so cancel.
                                                  y_scaling_menu_entry.Checked:=False;
                                                  case i of
                                                      4: begin
                                                            if alert_help(0,'Clicking HELP implies that you are unsure about the Y-scaling factor'
                                                                           +' ( '+round_str(temp*100,2)+'% ) which you have set.'
                                                                           +'||It has therefore been cancelled. Clicking CONTINUE below will return you to the data entry form,'
                                                                           +' where you can re-enter the factor if your intentions remain unchanged.'
                                                                           +'|||'+help_str,'cancel  distortion')=1 then EXIT;
    ...
    if y_distortion_factor=1 then distortions:=distortions and $FFFD    // clear Y bit in the flags byte.
                             else distortions:=distortions or  $0002;   // set it.
  end;//try
  redraw(True);
end;
//_________________________________________________________________________________________
...
const
 ...
 +' the accuracy or aspect ratio of a template which is being output elsewhere, for example when generating a DXF file for transfer to CAD software.'
 +'||There should NEVER be any need to apply an aspect-distortion factor when printing directly from Templot0 - leave the factor setting at 100%'
 +'||If your printer is not producing accurate results, use the printer calibration function to make the necessary'
 +' corrections - see under the PRINT menu. If the problem with your printer persists, due perhaps to wear and tear, try using the SKEWING'
 +' or CONING distortions instead.'
 +'||The aspect-distortion factor is applied to the data as follows:'
 +'||X-dimensions (across the screen from the left) are multiplied by the aspect-distortion factor; and'
 +'|Y-dimensions (up the screen from the bottom) are divided by the aspect-distortion factor;'
 +'||with the result that the area occupied by the drawing remains constant.'
 +'||The result of setting a aspect-distortion factor of 50%, for example, is that the drawing shrinks to half its length'
 +' and is doubled in width. To make small corrections a typical aspect-distortion factor setting would be close to 100% and'
 +' look like 100.71% or 98.35%, for example. If you make large distortions, be aware that the results of the mouse actions and'
 +' other settings may be unexpected or become unpredictable.'
 +'||The effect of setting a negative aspect-distortion factor is that the drawing is'
 +' additionally rotated 180 degrees about the origin, and will not be visible without re-origination or a shift.'
 +' Also, the mouse actions and other settings will be reversed in direction, causing much confusion.'
 +'||The aspect-distortion factor is reset to 100% when you do a B-6 TURNOUT RESET - otherwise it will remain in force until you'
 +' come here again and change it. So please remember to reset it to 100% after experimenting. If you forget to do so,'
 +' all your subsequently printed templates will be useless!'
 +'||Having corrected the aspect ratio by means of the aspect-distortion factor, it is usual to restore the drawing to its correct'
 +' size by means of the scaling-distortion factors. This will mean that the printed grid (which is unaffected by distortions) will no'
 +' longer be accurate - so please heed the warnings above about not using data distortions when printing directly from Templot0.'
 +'||Note also that when exporting in DXF format the normal printer calibration and output scaling settings are ignored, so restoration'
 +' of the correct drawing size can only be done using the scaling-distortions - or by means of'
 +' the destination CAD program''s own scaling or zoom functions. The DXF file does not include the grid or page trim lines.'
 +'||Aspect distortion applies globally for as long as it remains in force, so all templates drawn on the trackpad or in the storage box will be'
 +' distorted but will return to normal when distortion is cancelled. The distortion data is not included in the template files'
 +' when they are saved from the box, and must be re-entered as required for each working session.'
 +'||( Handy hint - make a note of your aspect-distortion setting in your memo text. It can then be quickly copied and pasted back into the data entry form after reloading.)';
...
begin
  ...
  try
    repeat
      n:=putdim(help_str,4,'aspect - distortion  factor',aspect_distortion_factor*100,False,False,True,False);  // neg ok, preset ok, no zero, don't terminate on zero.
      if n<>0 then EXIT;
      if getdims('aspect  distortion  factor',distortion_help_str,control_room_form,0,od)=True
         then begin
                if od[0]=def_req then aspect_distortion_factor:=1
                                 else begin
                                        if ABS(od[0])<minfp then od[0]:=1;
         ...
         else aspect_distortion_factor:=1;
      ...
      if aspect_distortion_factor=1 then begin
                                    ...
                                    i:=alert(7,'        aspect - distortion ?',
                                               'Setting the aspect-distortion factor to any value other than 100% will cause'
                                              +' your template to be unusable when printed. This function is intended as'
                                              +' a means of correction only when the output will be elsewhere.'
                                              +'||If your printer is not producing accurate results, do a printer calibration instead.'
                                              +'||Are you sure you want to distort your data ?',
                                               '','','yes  -  use  '+round_str(temp*100,2)+' %   aspect - distortion','?  help','no  -  cancel  distortion',
                                               'no  -  do  printer  calibration  instead',4);
                                    if i=3 then EXIT
                                           else begin
                                                  aspect_distortion_factor:=1;                     // he's not sure so cancel.
                                                  aspect_distortion_menu_entry.Checked:=False;
                                                  case i of
                                                      4: begin
                                                            if alert_help(0,'Clicking HELP implies that you are unsure about the aspect-distortion factor'
                                                                           +' ( '+round_str(temp*100,2)+'% ) which you have set.'
                                                                           +'||It has therefore been cancelled. Clicking CONTINUE below will return you to the data entry form,'
                                                                           +' where you can re-enter the factor if your intentions remain unchanged.'
                                                                           +'|||'+help_str,'cancel  distortion')=1 then EXIT;
    ...
    if aspect_distortion_factor=1 then distortions:=distortions and $FFFB    // clear our bit in the flag byte.
                                  else distortions:=distortions or  $0004;   // set it.
  end;//try
...
begin
  ...
  if mirror_x_menu_entry.Checked=True
     then begin
            mirror_x:=-100;                        // mirror does conversion to 1/100th mm.
...
begin
  ...
  if mirror_y_menu_entry.Checked=True
     then begin
...
begin
  ...
  try
    n:=putdim(coning_help_str,0,'X - coning  factor',x_coning_distortion_factor*1.0E6,False,False,False,False);  // neg ok, preset ok, zero ok, don't terminate on zero.
    if n<>0 then EXIT;
    if getdims('X - coning  factor',distortion_help_str,control_room_form,0,od)=True
       then begin
              if od[0]=def_req then x_coning_distortion_factor:=0
                               else begin
                                      x_coning_distortion_factor:=od[0]/1.0E6;
                                      x_coning_menu_entry.Checked:=True;
                                    end;
            end
       else x_coning_distortion_factor:=0;

    if x_coning_distortion_factor=0 then x_coning_menu_entry.Checked:=False;

  finally
    if x_coning_distortion_factor=0 then distortions:=distortions and $FFEF    // clear bit in the flags byte.
                                    else distortions:=distortions or  $0010;   // set it.
  end;//try
  redraw(True);
end;
//________________________________________________________________________________________
...
begin
  ...
  try
    n:=putdim(coning_help_str,0,'Y - coning  factor',y_coning_distortion_factor*1.0E6,False,False,False,False);  // neg ok, preset ok, zero ok, don't terminate on zero.
    if n<>0 then EXIT;
    if getdims('Y - coning  factor',distortion_help_str,control_room_form,0,od)=True
       then begin
              if od[0]=def_req then y_coning_distortion_factor:=0
                               else begin
                                      y_coning_distortion_factor:=od[0]/1.0E6;
                                      y_coning_menu_entry.Checked:=True;
                                    end;
            end
       else y_coning_distortion_factor:=0;

    if y_coning_distortion_factor=0 then y_coning_menu_entry.Checked:=False;

  finally
    if y_coning_distortion_factor=0 then distortions:=distortions and $FFDF    // clear bit in the flags byte.
                                    else distortions:=distortions or  $0020;   // set it.
  end;//try
  redraw(True);
end;
//___________________________________________________________________________________________
...
begin
  ...
  if (re_org_x=0) and (re_org_y=0) then EXIT;    // might be a B-6 reset.
  if cleared_bgnd=False then EXIT;               // first clear any background keeps.
...
begin
  ...
  try
    n:=putdim(skewing_help_str,0,'X - skewing  factor',x_skewing_distortion_factor*5.0E3,False,False,False,False);  // neg ok, preset ok, zero ok, don't terminate on zero.
    if n<>0 then EXIT;
    if getdims('X - skewing  factor',distortion_help_str,control_room_form,0,od)=True
       then begin
              if od[0]=def_req then x_skewing_distortion_factor:=0
                               else begin
                                      x_skewing_distortion_factor:=od[0]/5.0E3;  // 5E3 arbitrary.
                                      x_skewing_menu_entry.Checked:=True;
                                    end;
            end
       else x_skewing_distortion_factor:=0;

    if x_skewing_distortion_factor=0 then x_skewing_menu_entry.Checked:=False;

  finally
    if x_skewing_distortion_factor=0 then distortions:=distortions and $FFBF    // clear bit in the flags byte.
                                     else distortions:=distortions or  $0040;   // set it.
  end;//try
  redraw(True);
end;
//__________________________________________________________________________________________
...
begin
  ...
  try
    n:=putdim(skewing_help_str,0,'Y - skewing  factor',y_skewing_distortion_factor*5.0E3,False,False,False,False);  // neg ok, preset ok, zero ok, don't terminate on zero.
    if n<>0 then EXIT;
    if getdims('Y - skewing  factor',distortion_help_str,control_room_form,0,od)=True
       then begin
              if od[0]=def_req then y_skewing_distortion_factor:=0
                               else begin
                                      y_skewing_distortion_factor:=od[0]/5.0E3;  // 5E3 arbitrary.
                                      y_skewing_menu_entry.Checked:=True;
                                    end;
            end
       else y_skewing_distortion_factor:=0;

    if y_skewing_distortion_factor=0 then y_skewing_menu_entry.Checked:=False;

  finally
    if y_skewing_distortion_factor=0 then distortions:=distortions and $FF7F    // clear bit in the flags byte.
                                     else distortions:=distortions or  $0080;   // set it.
  end;//try
  redraw(True);
end;
//__________________________________________________________________________________________
...
begin
  if set_font=True then printer_text_font.Assign(get_font('choose  a  new  font  and  colour  for  text  printing',printer_text_font,True));

     putdim(margin_help_str,1,'width  of  left  page  margin',printer_text_left_margin,True,True,False,False);  // no neg, no preset, zero ok, don't terminate on zero.
  n:=putdim(margin_help_str,1,'width  of  right  page  margin',printer_text_right_margin,True,True,False,False);  // no neg, no preset, zero ok, don't terminate on zero.

  if n<>1 then EXIT;
  if getdims('printer  page  margins  for  text','',calling_form,n,od)=True
     then begin
            printer_text_left_margin:=od[0];       // in mm.
            printer_text_right_margin:=od[1];

            help_form.html_view.PrintMarginLeft:=printer_text_left_margin/10;            // in cm for viewer.
            help_form.html_view.PrintMarginRight:=printer_text_right_margin/10;

            keep_form.keep_html_view.PrintMarginLeft:=printer_text_left_margin/10;       // in cm for viewer.
            keep_form.keep_html_view.PrintMarginRight:=printer_text_right_margin/10;

            alert_box.alert_html_viewer.PrintMarginLeft:=printer_text_left_margin/10;    // in cm for viewer.
            alert_box.alert_html_viewer.PrintMarginRight:=printer_text_right_margin/10;

            mint_form.mint_html_view.PrintMarginLeft:=printer_text_left_margin/10;       // in cm for viewer.
...
begin
  case mouse_click_action of
       -1: either_action_menu_entry.Checked:=True;     // radio item.
        0: button_down_menu_entry.Checked:=True;       // radio item.
        1: click_move_click_menu_entry.Checked:=True;  // radio item.
  end;//case
...
const
  ...
  +' using the `0real > V-crossing options > convert RAM to CLM`z menu item on the trackpad. After setting a crossing unit angle, say 1:7,'
  +' this tool will change the crossing angle to 1:7 CLM. The angle will, however, continue to be displayed in RAM units,'
  +' showing as, say, B - 6.96 as the turnout size in the information panel heading. The information text always shows the current crossing angle in both'
  +' RAM and CLM units.'
  +'||Be aware that after every change you make to the crossing angle, either by direct entry or by mouse action (F5), it will be'
  +' necessary to use the converter tool again if you want to have the new unit angle in CLM units.'
  +'||Later versions of Templot0 will permit a global change of unit measure (here in this menu), and the converter tool will then'
  +' be no longer required. Some railways used other measures, e.g. IM (Isosceles Measure) or SM (Sine Measure), and some non-U.K. railways have other measures (e.g. the grade system),'
  +' all of which will also be available later.';
...
const
  ...
  +'|The pre-set dimension is 10 mm.'  // 0.93.a was 15mm
  ...
  +'||If you experience problems this dimension can be increased to 25 or 50 mm without any great detriment in 4mm/ft scale and above.'

  +'||N.B. For Windows 95/98/ME some features of your templates are likely to be omitted from the trackpad as you zoom in to trackpad widths less than approximately 1ft scale size.'
  +' They will re-appear as you zoom out again. This is a feature of the Windows graphics system, not Templot, and does not apply to Windows NT/2000/XP/Vista/7.'
  +'||The recommended minimum settings are 5 mm for working at 4mm/ft scale, and 9 mm for 7mm/ft scale.';
...
begin
  ...
  n:=putdim(explode_help_str,1,'trackpad  width  at  maximum  explode',m,True,False,True,False);  // no neg, preset ok, no zero, don't terminate on zero.
  if n<>0 then EXIT;
  if getdims('maximum  explode  ( zoom-in )','',control_room_form,n,od)=True
     then begin
            m:=od[0];
            if m=def_req then m:=10;    // default 10mm.   0.93.a was 15mm
            screenx_min:=limits(0.5,1000,ABS(m),dummy_i);
          end;
  if screenx<screenx_min then screenx:=screenx_min;
  redraw(True);
end;
//__________________________________________________________________________________________
...
const
  ...
  +'||The length of the angled arms on the fixing peg can be changed. This is sometimes useful when aligning templates by eye on the pad'
  ...
  +'||The new length will appear immediately on the control template, but it is necessary to rebuild the background templates before the new arm length'
  +' will take effect on them (GENERATOR > REBUILD ALL BACKGROUND menu item).'
  ...
  +' The angled arms on the pegging notch will be set to the same length.';
...
begin
  ...
  n:=putdim(peg_help_str,1,'length  of  angled  arms  on  fixing  peg',m,True,False,False,False);  // no neg, preset ok, zero ok, don't terminate on zero.
  if n<>0 then EXIT;
  if getdims('peg  arm  length','',control_room_form,n,od)=True
     then begin
            m:=od[0];
            if m=def_req then m:=3*scale;    // default 3ft.
            peg_arm_length:=m;
          end;
  redraw(True);
end;
...
begin
  ...
  Screen.Cursors[adjust_corners_cursor_invert]:=LoadCursor(HInstance,'CURSOR_ADJUST_CORNERS_INVERT');
  ...
  if prefs_existed_on_startup=True
     then begin
  ...
  if FileExists(ExtractFilePath(Application.ExeName)+'reminder.txt')
     then begin
     ...
     else reminder_label.Caption:='Reminder message:'+#13+#13+'A reminder message from the previous working session can be shown here.';
...
begin
  if loading_in_progress=True then EXIT;  // 208c

  if final=True then code:=-1       // final backup on exit
                else code:=1;       // normal rolling backup.
...
begin
  ...
  if initdone_flag=True then bgnd_form.paste_button.Enabled:=Clipboard.HasFormat(CF_BITMAP);  // 211b
end;
//______________________________________________________________________________
...
const
  ...
  +' This option will give the fastest response to mouse activity and keyboard keys, but your system cpu processor will be running at near 100% capacity, with consequent power consumption and heat generation.'
  +' This option is not recommended for laptops and battery-powered systems.'

  +'||The ALLOW FULL IDLE (SLOW) option causes Templot0 to relinquish control of the cpu whenever there is no further processing to be done. This option will give a slower response to mouse activity and keyboard keys,'
  +' but your system cpu processor will be running at much lower capacity, with consequent savings in power and battery life.'
  +' This option is the default setting and recommended for most users.'

  +'||The 100% FOR MOUSE ACTIONS ONLY option is a half-way compromise setting. While a mouse action is being used the cpu will be switched to 100% usage to give the best response to the mouse movements.'
  +' At all other times the cpu will be allowed to idle to conserve power.'

  +'||The extent to which these options vary the response time on your system will be determined by your hardware configuration and the number of other software applications and background programs which are running.'
  +' On many systems there may be little difference in perceived response time between these various options.'

  +'||n.b. In all versions of Templot0 prior to 0.73.a the only option available was 100% ALWAYS (FAST).';
...
const
  ...
  +'||Change to the no-limits setting if you are content to leave all parameter checking to the Windows graphics systems. This will give the fastest program response,'
  +' but may cause problems and/or graphics hardware malfunctions when working at extreme zoom-in or printing very large picture shape images.'

  +'|<HR NOSHADE COLOR="#EE7700">'

  +'For <B>Windows 95 / 98 / ME</B> the setting is fixed at 16-bit and cannot be changed. This is a limitation in Windows, not Templot0.'
  +' When zooming-in on the screen and printing at high resolution you may find that you lose some drawing features (e.g. large background shapes and very long timbers).'
  +' Background picture shape images may not print on full-size templates. For more information click the `0PICTURE SHAPES`1 tab on the `0print pages`3 window when it appears.'

  +'|<HR NOSHADE COLOR="#EE7700">'

  +'To change the zoom-in limit, click the `0PROGRAM > MAX EXPLODE (ZOOM-IN)...`1 menu item.';
...
begin
  ...
  with edit_memo_form do begin
    ...
    if FileExists(file_name_str)
       then vis_edit_memo.Lines.LoadFromFile(file_name_str)
       else begin
    ...
    if do_show_modal(edit_memo_form)=mrOk    // 212a
       then begin
              if vis_edit_memo.Lines.Count>0 then vis_edit_memo.Lines.SaveToFile(file_name_str);
            end;
...
begin
  if DeleteFile(exe_str+'reminder.txt')=True then ShowMessage('Your reminder message has been deleted.');
end;
//______________________________________________________________________________
...
begin
  ...
  if prefs_available=True
     then saved_preferences_setup_menu_entry.Caption:='&saved  preferences ...'
     else saved_preferences_setup_menu_entry.Caption:='begin  &saving  preferences ...';
end;
//______________________________________________________________________________
...
const
  col_help_str:string='||If your computer screen is currently using 256 colours (8-bit palette colour) or fewer, you may want to try changing to 12-bit colour depth'
   +' or better (Hi-Color or Tru-Color). You will then find it easier to adjust individual colours.'
   +'||If you prefer not to change the colour settings on your computer, Templot0 provides a few pre-set colour schemes using only basic colours. Click the button below to try them.'
   +'||<SPAN STYLE="FONT-SIZE:12px;">( To find out if you can change the colour depth, click START > SETTINGS > CONTROL PANEL > DISPLAY > SETTINGS and explore the entries'
   +' under COLORS or COLOR DEPTH or COLOR PALETTE. If the memory available to your video card is limited, selecting extra colours may cause a trade-off'
   +' against the screen resolution. There is little to be gained in Templot0 by setting the colour depth greater than 16-bit, and doing so may significantly'
   +' reduce the mouse response speed when using the mouse actions to adjust your templates.'
   +' If you make any changes you will need to quit and restart Templot0 for the changes to take effect.)</SPAN>';

  res_help_str:string='||Templot0 was designed to run best at 1024 x 768 screen resolution or higher. If your screen resolution is currently lower than this, Templot0''s windows and forms (such as this one) may appear quite large and chunky.'

   +'||If you prefer not to change the screen resolution settings on your computer, you can scale the size of each window by repeatedly clicking the up-down arrow buttons'
   +' which appear in a blue panel at or near the top left corner of each window. Try this now on this window.'

   +'||For many of Templot0''s windows the visible area can also be enlarged, reduced, and scrolled in the usual way by dragging the edges. This is not the same effect as scaling the size with the up-down buttons.'

   +'||If you change the scale size several times, rounding effects within Windows may cause parts of the window to become misaligned or obscured. The functional working will not be affected.'
   +'||Scaling the size of a window may produce an unsatisfactory font size, so windows which contain a significant'
   +' amount of text also have a <SPAN STYLE="COLOR:#0000FF">font</SPAN> button (usually top right) with which the text font size and colour can also be changed.'

   +'||<SPAN STYLE="FONT-SIZE:12px;">( To find out if you can change the screen resolution, click START > SETTINGS > CONTROL PANEL > DISPLAY > SETTINGS and explore the entries'
   +' under SCREEN or RESOLUTION or DISPLAY AREA and under FONTS or FONT SIZE. You may have to click ADVANCED or ADVANCED SETTINGS to find these. If the memory available to your graphics card is limited, selecting a higher resolution'
   +' may cause a trade-off against the number of colours available. If you make any changes you will need to quit and restart Templot0 for the changes to take effect.)</SPAN>';

  ppi_help_str:string='||Templot0 was developed using Large Fonts (120 pixels per inch). If your screen setting is currently for Small Fonts (96 pixels per inch),'
   +' you may find that Templot0''s windows and fonts appear larger than you have been using in other software.'

   +'||If you prefer to have them smaller, you can scale the size of each window by repeatedly clicking the up-down arrow buttons'
   +' which appear in a blue panel at or near the top left corner of each window. Try this now on this window.'
   ...
   +'||<SPAN STYLE="FONT-SIZE:12px;">( You may prefer to change your screen font size to Large Fonts, which are much easier on the eyes. To find out if you can change the font size,'
   +' click START > SETTINGS > CONTROL PANEL > DISPLAY > SETTINGS and explore the entries'
   +' under FONTS or FONT SIZE. You may have to click ADVANCED or ADVANCED SETTINGS to find these. If you make any changes you will need to restart your computer for the changes to take effect.)</SPAN>';

  header_style_str1:string='<SPAN STYLE="font-weight:bold; color:#0077DD; font-family:''Verdana''; font-size:20px; font-style:italic;">    ';
...
begin
 if Screen.DesktopWidth<1024
    then res_str:='<SPAN STYLE="COLOR:#990000;"><I>Your computer screen resolution is currently lower than 1024 pixels wide.</I></SPAN>'
    else res_str:='<SPAN STYLE="COLOR:#990000;"><I>Your computer screen resolution is currently 1024 pixels wide or higher, so the following notes can be disregarded.</I></SPAN>';

 help(0,header_style_str1+'Older  Computers  -  Screen  Resolution'+header_style_str2+res_str+res_help_str,'');

 if Screen.PixelsPerInch=96
    then ppi_str:='<SPAN STYLE="COLOR:#990000;"><I>Your computer screen is currently set to 96 pixels per inch.</I></SPAN>'
    else ppi_str:='<SPAN STYLE="COLOR:#990000;"><I>Your computer screen is currently set to '+IntToStr(Screen.PixelsPerInch)+' pixels per inch, so the following notes can be disregarded.</I></SPAN>';

 help(0,header_style_str1+'Older  Computers  -  Screen  Fonts'+header_style_str2+ppi_str+ppi_help_str,'');

 if hi_color=False
    then cols_str:='<SPAN STYLE="COLOR:#990000;"><I>Your computer screen is currently using 256 colours or fewer.</I></SPAN>'
    else cols_str:='<SPAN STYLE="COLOR:#990000;"><I>Your computer screen is currently using more than 256 colours, so the following notes can be disregarded.</I></SPAN>';

 if help(0,header_style_str1+'Older  Computers  -  Screen  Colours'+header_style_str2+cols_str+col_help_str,'try  a  different  colour  scheme')=1 then  pad_form.preset_schemes_menu_entry.Click;
...
begin
  data_child_form.data_memo.Font.Assign(get_font('choose  a  new  text  font  for  the data  panel',data_child_form.data_memo.Font,True));
end;
//______________________________________________________________________________
...
begin
  ...
  if pad_form.show_bgnd_keeps_in_rect_menu_entry.Checked=False
     then begin
...
begin
  ...
  if pad_form.show_bgnd_keeps_in_rect_menu_entry.Checked=False then pad_form.top_toolbar_panel.ParentColor:=True;      // shows trackpad colour

  pad_form.second_toolbar_panel.ParentColor:=True;                                                                     // 217a shows trackpad colour
...
begin
  if classic_templot=True then classic_templot_mode_menu_entry.Checked:=True     // radio items
...
const
  popup_help_str:string='Enter a screen position for the pop-up menu which appears when you click on a background template.'

    +'||These co-ordinates are in screen dots from the top left corner of the screen: X across from the left, Y down from the top.'
    +'||These co-ordinates refer to your full computer display screen for Windows, the position of Templot0 on the screen (if it is not full-screen) is not relevant.'
    +'||Take care to enter sensible figures, otherwise the pop-up menu may not be visible.'
    +'||If the menu position does not correspond to your entered co-ordinates, most likely your dimensions are not valid and are being adjusted by Windows to fit your screen.'
    +'||To revert to using the default menu position on the left, enter -1 (minus 1) for X and Y.';
...
begin
     putdim(popup_help_str,0,'template  context  menu  position  X  ( from  left )',user_popup_X,False,True,False,False);   // neg ok, no preset, zero ok, don't terminate on zero.
  n:=putdim(popup_help_str,0,'template  context  menu  position  Y  ( from  top )',user_popup_Y,False,True,False,False);   // neg ok, no preset, zero ok, don't terminate on zero.

  if n<>1 then EXIT;

  if getdims('template  context  popup  menu  position','',control_room_form,n,od)=True
     then begin
...
const
  trackpad_help_str:string='Enter a screen position for the trackpad window when it is not maximized.'

    +'||These co-ordinates are in screen dots from the top left corner of the screen: X across from the left, Y down from the top.'
    +'||These co-ordinates refer to your full computer display screen for Windows, the current position of Templot0 on the screen is not relevant.'
    +'||Take care to enter sensible figures, otherwise the trackpad window may not be visible.';
...
begin
  if pad_form.WindowState<>wsNormal
     then begin
            ShowMessage('The trackpad window position cannot be changed because the trackpad window is currently maximized to full-screen.'
                      +#13+#13+'Restore Down on the trackpad window first.');
            EXIT;
          end;


     putdim(trackpad_help_str,0,'trackpad  window  position  X  ( from  left )',pad_form.Left,False,True,False,False);   // neg ok, no preset, zero ok, don't terminate on zero.
     putdim(trackpad_help_str,0,'trackpad  window  position  Y  ( from  top )',pad_form.Top,False,True,False,False);     // neg ok, no preset, zero ok, don't terminate on zero.

     putdim(trackpad_help_str,0,'trackpad  window  width',pad_form.Width,True,True,True,False);   // no neg, no preset, no zero, don't terminate on zero.
  n:=putdim(trackpad_help_str,0,'trackpad  window  height',pad_form.Height,True,True,True,False); // no neg, no preset, no zero, don't terminate on zero.

  if n<>3 then EXIT;

  if getdims('trackpad  window  position','',control_room_form,n,od)=True
     then begin
             ...
             for i:=temp_comp_j.ComponentCount-1 downto 0 do begin
               ...
               if (temp_comp_i is TMenuItem)
                  then begin
                         ...
                         or (TMenuItem(temp_comp_i).Caption=' ')
                            then TMenuItem(temp_comp_i).Visible:=show_blanks;
                       end;
...
begin
  ...
  if LowerCase(ExtractFileExt(in_str))='.jpeg'
     then RESULT:=ChangeFileExt(in_str,'.jpg');
...
const
  ...
  +'||You can reduce this if you need to create a smaller file size. A setting below about 80% will cause a noticeable reduction in image quality.'
  +'||green_panel_begin tree.gif Generally it is better to use PNG format rather than JPG when saving image files from Templot0.'
  +'||In most cases PNG will create a smaller file size with 100% image quality. The JPG format is intended only for photographic images from a camera or a scanner.green_panel_end';
...
begin
  n:=putdim(jpg_help_str,4,'JPG  image  quality',jpg_quality,True,True,True,False);  // no neg, no preset, no zero, don't terminate on zero.
  if n<>0 then EXIT;
  if getdims('JPG  image  quality','',Screen.ActiveForm,n,od)=True
     then begin
            if (od[0]<1) or (od[0]>100)
               then begin
                      ShowMessage('error - this setting must be between 1% and 100%');
                      EXIT;
                    end
               else jpg_quality:=Round(od[0]);
          end;
end;
//______________________________________________________________________________
