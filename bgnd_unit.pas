...
type
  ...
  Tbgnd_form = class(TForm)
    ...
    procedure move_up_buttonMouseDown(Sender: TObject;
      Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
    procedure move_up_buttonMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure move_down_buttonMouseDown(Sender: TObject;
      Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
    procedure move_down_buttonMouseUp(Sender: TObject;
      Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
...
const
  ...
  +'||In addition to the geometrical background shapes, a "picture" shape is a rectangle which can contain an image, such as may be created in Windows Paint or similar programs, or obtained from a scanner.'
  +'||Scanned images from maps, published track plans, hand-drawn sketches and tracings can in this way be used as a background guide to your track design.'
  ...
  +'||If you are drawing `0line`1 shapes, you can draw as many lines as needed. To finish drawing lines click the `0STOP`z button on the trackpad, or press the `0ESC`2 key on the keyboard.'
  ...
  +'||Remember to save a new BGS data file containing the new positions of the picture shape outlines and a new image file.'
  ...
  +'||Usually the angle is not known. In this case you can add "marker lines" to the trackpad which Templot0 will use to calculate the required twist angle.'
  ...
  +'||If you do not set a FIXED marker line, Templot0 will assume a horizontal one, or use the current angle of the pegging notch if the PICTURE OPTIONS tab > TWIST USING NOTCH-ANGLE AS FIXED-MARKER tickbox is ticked.'
  ...
  +' You must set a TWIST marker line on the image to be twisted.'
{
  ...
  +' There is seldom any need to scan at resolutions greater than 300 dpi. Memory requirements will be significantly reduced if you scan in black && white only, rather than grey-scale or colour.'
  ...
  +' restart Templot0 and try again. If problems persist, restart Templot0 and do not use this function.'

  +'||If your system fails to twist the bitmap image here in Templot, you may have other graphics software which is able to do it when Templot0 is not running.'
  ...
  +'||Every time an image is twisted, the enclosing picture shape outline is enlarged by the addition of "white triangles" in the corners. If the same image is repeatedly twisted, it will soon become very large.'
  ...
  +'||N.B. Please bear in mind that Templot0 is not fully-fledged graphics imaging software - this "bare-bones" twist function is intended primarily to permit the alignment of scanned track plans.'
...
procedure shapes_current_state;     // set the current state of the form to match the selected shape.
...
begin
  with bgnd_form do begin
    ...
    if (line_radio_button.Checked=True) and (by_drawing_radiobutton.Checked=True)
       then add_shape_button.Caption:='add  shapes'
       else add_shape_button.Caption:='add  shape';

    with bgnd_shapes_listbox do begin

      if Items.count=1
         then count_label.Caption:='1  shape'
         else count_label.Caption:=IntToStr(Items.Count)+'  shapes';
      ...
      if n<0 then n:=0;  // if none make it the top one.
    ...
    with now_shape do begin
      ...
      show_tick_image.Visible:=((hide_bits AND $01)=0);        // byte,  0=normal,  1=hide on trackpad,  2=hide on output,  3=hide both
      ...
      trans_checkbox.Checked:=((show_transparent=True) and (shape_code=-1));   // 0.93.a now in file
      ...
      case shape_code of
                  ...
                  -1: begin                      // picture.
                        ...
                        if Tbgshape(bgnd_form.bgnd_shapes_listbox.Items.Objects[n]).bgnd_shape.picture_is_metafile=True
                           then meta_str:=' (EMF) '
                           else meta_str:='';

                        if Tbgshape(bgnd_form.bgnd_shapes_listbox.Items.Objects[n]).bgimage=nil
                           then code_str:='picture'
                           else begin
                                  with Tbgshape(bgnd_form.bgnd_shapes_listbox.Items.Objects[n]).bgimage.image_shape do begin
                                    code_str:='picture'+meta_str+' :  '+IntToStr(image_width)+'  x  '+IntToStr(image_height)+'  native  dots';
                                  end;//with
                                end;
                   ...
                   1: begin                      // rectangle.
                        ...
                        case shape_style of
                            0: begin shape_str:='clear  '; clear_radio_button.Checked:=True; end;
                            1: begin shape_str:='solid  '; solid_infill_radio_button.Checked:=True; end;
                            2: begin shape_str:='hatched  '; hatched_radio_button.Checked:=True; end;
                        end;//case
                        corner2_button.Enabled:=True;
                   ...
                   2: begin                      // circle.
                        ...
                        if ABS(ABS(p2.x-p1.x)-ABS(p2.y-p1.y))>minfp  // elliptical?   214a
                           then code_str:='ellipse'
                           else code_str:='circle';
      ...
      if shape_code=3
         then rename_menu_entry.Caption:='edit  label  text ...'
         else rename_menu_entry.Caption:='rename  shape ...';

      type_label.Caption:=shape_str+code_str;
      if shape_code<3
         then begin
                p1_label.Caption:=insert_crlf_str('   from :|X1 =  '+round_str(p1.x,2)+' mm'
                                                          +'|Y1 =  '+round_str(p1.y,2)+' mm');

                p2_label.Caption:=insert_crlf_str(  '   to :|X2 =  '+round_str(p2.x,2)+' mm'
                                                          +'|Y2 =  '+round_str(p2.y,2)+' mm');
              end
         else begin                               // label or target
                p1_label.Caption:=insert_crlf_str('     at :|X1 =  '+round_str(p1.x,2)+' mm'
                                                          +'|Y1 =  '+round_str(p1.y,2)+' mm');
    ...
    if bgnd_shapes_listbox.Items.Count>3 then delete_25_button.Enabled:=True
                                         else delete_25_button.Enabled:=False;

    if bgnd_shapes_listbox.Items.Count>1 then delete_50_button.Enabled:=True
                                         else delete_50_button.Enabled:=False;
  end;//with
end;
...
begin
  ...
  if by_drawing_radiobutton.Checked=True
       then add_shape_button.Caption:='add  shapes'
       else add_shape_button.Caption:='add  shape';
...
const
  ...
  +'||Note that unless you use conversion factors the dimension should be entered in mm. To avoid confusion, set the grid spacing in mm before adding background shapes (click the GRID... button).'
  +'||For more information about using conversion factors click the ? HELP button.'
  +'||The pre-set arm length (enter a slash / ) is 10mm.';
  ...
  +'||Note that unless you use conversion factors the dimension should be entered in mm. To avoid confusion, set the grid spacing in mm before adding background shapes (click the GRID... button).'
  +'||For more information about using conversion factors click the ? HELP button.'
  +'||The pre-set diameter (enter a slash / ) is 250mm.'
  +'|||To create an ellipse instead of a circle, first create a circle. Then click the `0DIMENSIONS...`1 button on the `0MODIFY SHAPE``1 tab, to enter the dimensions for the ellipse.';
...
begin
  ...
  if (modifying=True) and (type_code=2)   // allow conversion of circle to ellipse  214a
     then begin
            if ABS(ABS(p2.x-p1.x)-ABS(p2.y-p1.y))>minfp
               then type_code:=99              // already an ellipse
               else begin
                      i:=alert(4,'    circle  or  ellipse ?',
                        ...
                        case i of
                             4: type_code:=99;   // over-ride code 2 below
                             5: EXIT;
                        end;//case
                    end;
          end;
  ...
  if (modifying=False) and (type_code=-1)   // new picture, set default height= 0.75 x width
     then begin
            putdim(x_str,1,'picture :  first  corner  X  dimension  X1 ',p1.x,False,False,False,False);   // neg ok, preset ok, allow zero, don't terminate on zero.
            putdim(y_str,1,'picture :  first  corner  Y  dimension  Y1 ',p1.y,False,False,False,False);   // ditto.
         i:=putdim(w_str,1,'picture :  width ',p2.x-p1.x,True,True,True,False);                           // no neg, no preset, no zero, don't terminate on zero.

            if i<>2 then EXIT;
            if getdims('background  picture  shape',get_str,bgnd_form,i,od)=True
               then begin
                      p1.x:=od[0];
                      p1.y:=od[1];
                      p2.x:=p1.x+ABS(od[2]);
  ...
  if picture_done=False
     then begin
            case type_code of

                 2: begin      // circle changes 214a

                       putdim(x_str,1,'circle  centre  X  dimension',(p1.x+p2.x)/2,False,False,False,False);  // neg ok, preset ok, allow zero, don't terminate on zero.
                       putdim(y_str,1,'circle  centre  Y  dimension',(p1.y+p2.y)/2,False,False,False,False);  // ditto.
                    i:=putdim(circle_str,1,'circle  diameter',ABS(p2.x-p1.x),True,False,True,False);     // no neg, preset ok, no zero, don't terminate on zero.

                      if i<>2 then EXIT;
                      if getdims('background  circle',get_str,bgnd_form,i,od)=True
                         then begin
                                if od[2]=def_req then circle_dia:=250           // reset to default (arbitrary)
                                                 else circle_dia:=ABS(od[2]);
                 ...
                 3: begin
                       putdim(x_str,1,'label  X  dimension  ( top - left ) ',p1.x,False,False,False,False);  // neg ok, preset ok, allow zero, don't terminate on zero.
                    i:=putdim(y_str,1,'label  Y  dimension  ( top - left ) ',p1.y,False,False,False,False);  // ditto.
                      if i<>1 then EXIT;
                      if getdims('background  label',get_str,bgnd_form,i,od)=True
                         then begin
                                p1.x:=od[0];
                                p1.y:=od[1];
                                p2.x:=0;
                 ...
                 4: begin
                       putdim(x_str,1,'target  mark  centre  X  dimension',p1.x,False,False,False,False);  // neg ok, preset ok, allow zero, don't terminate on zero.
                       putdim(y_str,1,'target  mark  centre  Y  dimension',p1.y,False,False,False,False);  // ditto.
                    i:=putdim(target_str,1,'target  arm  lengths',p2.x,True,False,True,False);       // no neg, preset ok, no zero, don't terminate on zero.

                      if i<>2 then EXIT;
                      if getdims('background  target  mark',get_str,bgnd_form,i,od)=True
                         then begin
                                p1.x:=od[0];
                                p1.y:=od[1];
                                if od[2]=def_req then p2.x:=10.0        // default (arbitrary).  // use p2 for the arm length.
                                                 else p2.x:=ABS(od[2]);
                                p2.y:=0;
                                target_arm:=p2.x;                       // so can show the same for the next target.
                              end
                         else EXIT;
                    end;
                      ...
                      case code of
                            -1: begin str1:='picture :  '; str2:='  corner  '; end;
                             0: begin str1:='line :  '; str2:='  end  '; end;
                             1: begin str1:='rectangle :  '; str2:='  corner  '; end;
                             2: begin str1:='ellipse :  '; str2:='  corner  '; end;
                           else begin str1:=''; str2:=''; end;
                      end;//case

                       putdim(x_str,1,str1+'first'+str2+'X  dimension  X1 ',p1.x,False,False,False,False);   // neg ok, preset ok, allow zero, don't terminate on zero.
                       putdim(y_str,1,str1+'first'+str2+'Y  dimension  Y1 ',p1.y,False,False,False,False);   // ditto.
                       putdim(x_str,1,str1+'second'+str2+'X  dimension  X2 ',p2.x,False,False,False,False);  // ditto.
                    i:=putdim(y_str,1,str1+'second'+str2+'Y  dimension  Y2 ',p2.y,False,False,False,False);  // ditto.

                      if i<>3 then EXIT;
                      if getdims('background  '+str1,get_str,bgnd_form,i,od)=True
                         then begin
                                p1.x:=od[0];
                                p1.y:=od[1];
                                p2.x:=od[2];
  ...
  if used1=True then DEC(ring_list_index);   // ignore the copy used, or the ring.
  ...
  if ring_list_index>-1     // and for p2 use the previous copy or the ring itself if it's still there.
     ...
     then begin
            ...
            if p2.x=def_req then p2.X:=screenx/2;   // arbitrary. (pre-sets requested but all data already used)..
...
function get_user_image(dropped,meta,scan_wanted,paste_clicked,reloading:boolean; n:integer; var img_width,img_height:integer):string;
...
begin
  ...
  try
    ...
    if scan_wanted=True
       then begin
              ...
              if do_twain(new_bmp)=True    // get scan
                   ...
                   else was_cancelled:=True;    // scan failed
    ...
    if Clipboard.HasFormat(CF_BITMAP)=True     // 211b paste from clipboard
       then begin
              repeat
                if paste_clicked=True
                   then i:=6                           // paste button clicked

                   else i:=alert(4,'php/402    paste  image  into  picture  shape ?',
                                   'You have previously copied an image.'
                                  +'||Do you want to paste the copied image into this picture shape?',
                                   '','','more  information','no  -  load  image  from  a  file','cancel','yes  -  paste  copied  image',3);

                case i of
                     3: alert_help(0,paste_help_str,'');
    ...
    if was_cancelled=False     // 211b
       then begin
              ...
              if bgnd_form.picture_load_dialog.Execute=True
                 then begin
                        image_file_str:=bgnd_form.picture_load_dialog.FileName;

                        if FileExists(image_file_str)=True
                           then begin
                                  Application.ProcessMessages;   // allow repaints - may take some time.
                                  ...
                                  with Tbgshape(bgnd_form.bgnd_shapes_listbox.Items.Objects[n]) do begin
                                    ...
                                    bgnd_shape.show_transparent:=False;     // init not transparent.
                                  ...
                                  user_load_img_path:=ExtractFilePath(image_file_str); // for next time.

                                  RESULT:='picture : '+Copy(ExtractFileName(image_file_str),1,36);     // for entry in list. 46 chars max
                 ...
                 else was_cancelled:=True  // cancelled reload
...
function get_sk7_image(n:integer; sk7_str:string; var img_width,img_height:integer):string;    // 3-2-01.
...
function get_sk8_image(n:integer; old_bmp_file_str,sk8_str:string; var img_width,img_height:integer):string;    // 3-2-01.
      ...
      if sk8_exists=True     // png format, 0.93.a
         then begin
                ...
                try
                  ...
                  if sk8_exists=True then image_bitmap.Assign(create_png);
...
const
  ...
  +'||Enter the scale of the original scanned map as a unit ratio. For example, a scale of 40ft to one inch of map should be entered as 480.'
  +'||For the well-known OS 25-inch maps, the scale is 1:2500 and should be entered as 2500.';
  ...
  picture_sizing_help_str:string='    `0Sizing  Method`9'
    +'||If you choose `0SCANNED MAP OR PROTOTYPE TRACK PLAN`1 you will be asked for the scanner DPI resolution and the scale of the map in relation to the full-size railway.'
    +' Templot0 will then scale the picture shape correctly to match your drawing.'

    +'||If you choose `0SCANNED MODEL TRACK PLAN`1 you will be asked for the scanner DPI resolution and the scale of the plan in relation to the model.'
    +' Templot0 will then scale the picture shape correctly to match your drawing.'

    +'||For the above two options, it is essential to have previously set your required model gauge and scale.'

    +'||If you do not know the scanner resolution, some trial and error will be needed to get the image to the correct size. A good starting point would be 300 dpi.'

    +'|<HR NOSHADE SIZE=1 COLOR=GRAY>'
    +'|For more information and tutorials about the use of background images, please refer to the <A HREF="go_to_templot_companion.85a">Templot Companion</A> web site.|&nbsp;';
...
begin
  if bgnd_form.bgnd_shapes_listbox.Items.Count<1 then EXIT;  // ???

  n:=bgnd_form.bgnd_shapes_listbox.ItemIndex;

  if (n<0) or (n>(bgnd_form.bgnd_shapes_listbox.Items.Count-1)) then EXIT; // ???
  ...
  dpi_str:='rp.gif  The width-wise and height-wise DPI settings are entered separately. In the vast majority of cases the setting is the same for both, for example from a scanner.'
          ...
          +'|||    `0Scanner  DPI`9'
          +'||Enter the resolution in dots-per-inch (DPI) at which this image was scanned. If you do not know this figure, some trial and error will be needed to get the image to the correct size. A good starting point would be 300 dpi.'
          +'||Refer to the documentation for your scanner for information about the scanner resolution.'

          +'||The width-wise and height-wise DPI settings are entered separately. In the vast majority of cases the setting is the same for both, for example from a scanner.'
          +' The settings can sometimes differ, for example for an image received via a fax machine.'

          +'||Many scanners use the popular ScanGear software:'
          +'||       <img src="'+exe_str+'internal\hlp\scangear_dpi.png">'
          +'||To find or set the DPI resolution:'
          +'||1. click the `0Advanced Mode`z tab.|2. Set or make a note of the `0Output Resolution`z setting.'

          +'||green_panel_begin tree.gif Do not set very high resolutions as this may severely slow down the screen refresh and zooming in Templot0.'
          +'||300 DPI (dots per inch) is a good option on most systems. Use a lower setting on older computers.green_panel_end';

  picture_option:=0; // keep compiler happy

  if auto_fit_msg_pref=False
     then begin
            ...
            repeat
              ...
              i:=alert(2,'        auto - fit  picture  shape',
                      ...
                      'This auto-fit function can automatically adjust the size of this picture shape to fit the image it contains.'
                      +' This is primarily intended for scaling (re-sizing) scanned maps and track plans to use as a background guide.'

                      +'||After the picture shape has been re-sized you will probably want to move it to a new position on the trackpad.'
                      +'||To do that click the `0shift`1 button to move it by mouse action, or the `0shift to...`1 button to set the new position directly.'

                      +'||But first, auto-fit re-sizing:'

                      +'||Do you know the DPI resolution at which the image was scanned?|If not click `0more information`1 below.'
                      +'|`0for example: 300 DPI`n.'

                      +'||Do you know the scale of the scanned map or track plan?'
                      +'|`0for example: 1:2500 for an OS 25" prototype map,'
                      +' or 1:24 for a model layout plan at 1/2" per foot`n.'

                      +'||Have you already set you model gauge and scale?'
                      +'|`0for example: EM`n.'

                      +'||If you answered yes to all of the above Templot0 can automatically set the picture shape to the'
                      +' correct scaled size for your track. Click `0auto-fit`1 below.'

                      +'||If you want to go back and find the information or set the gauge and scale, click `0cancel`1 below, then click the `0auto-fit`1 button again when you are ready.'

                      +'||Alternatively, the height of this picture shape can be adjusted to match the aspect ratio'
                      +' of the image and remove any distortion, leaving the width of the picture shape unchanged. Click `0auto-fit`1 below.'

                      +'||If you want to set a known width for the picture shape, click `0set known width`1 below, or click the `0re-size...`1 button.',

                       '','','more  information','set  known  width','cancel','auto - fit',3);

              auto_fit_msg_pref:=alert_box.preferences_checkbox.Checked;
              alert_box.preferences_checkbox.Hide;

              if i=3 then alert_help(0,dpi_str,'');
  ...
  repeat
    ...
    i:=alert(4,'php/401    sizing  method ?',
             ...
             +'||If you click one of the blue bars Templot0 can size the picture shape automatically, but you will need to enter some information about the scanner DPI setting and the real-world scale of the track plan image.'
             +' Also make sure that you have set your model scale and gauge.'
             +'||Otherwise, click the green bar to adjust only the height of the picture shape to match the image, leaving the width unchanged.'
             +'||For more explanation of these options, click the white `0more information`1 bar.',
              '','more  information','scanned  map  or  prototype  track  plan','scanned  model  layout  plan','cancel','adjust  aspect  ratio  only',2);
    case i of
      2: alert_help(0,picture_sizing_help_str,'');
      3: picture_option:=1;
      4: picture_option:=2;
      5: EXIT;
      6: picture_option:=0;
  ...
  try
    ...
    with Tbgshape(bgnd_form.bgnd_shapes_listbox.Items.Objects[n]) do begin
      ...
      with bgimage.image_shape do begin
        ...
        putdim(dpi_str,0,'width-wise  scanned  dpi',dpi_width,True,True,True,False);   // no neg, no preset, no zero, don't terminate on zero.
        putdim(dpi_str,0,'height-wise  scanned  dpi',dpi_height,True,True,True,False); // no neg, no preset, no zero, don't terminate on zero.

        if picture_option=1 then j:=putdim(map_scale_str,0,'map  scale  ratio  1 :',map_scale,True,True,True,False)                     // no neg, no preset, no zero, don't terminate on zero.
                            else j:=putdim(model_scale_str,0,'model  track  plan  scale  ratio  1 :',model_scale,True,True,True,False); // no neg, no preset, no zero, don't terminate on zero.

        if j<>2 then EXIT;
        if getdims('auto - fit  picture  shape',get_str,bgnd_form,j,od)=False then EXIT;
...
begin
  ...
  if add_option=2        // add at clicked corners.
     then begin
            if (pad_click_X=-1) or (pad_click_Y=-1) or (prev_pad_click_X=-1) or (prev_pad_click_Y=-1)
               then begin
                      alert(6,'php/301    invalid  clicked  shape',
                             ...
                             +'||Click the trackpad in 2 places to mark the corners or ends or enclosing rectangle of the new shape. Then click the `0ADD SHAPE`1 button again.'
                             +'||Or alternatively select the `0BY DRAWING`1 option to draw the outline of the new shape after clicking the `0ADD SHAPE`1 button.'
                             +'||Or select the `0BY ENTERING DIMENSIONS`1 option to enter directly the required dimensions for the new shape.'
                             +'||For more information about adding background shapes click the `0HELP`1 menu items.',
                              '','','','','','continue',0);
                      EXIT;
                    end;
          end;
  ...
  with bgnd_form do begin
    ...
    with new_shape do begin
      ...
      shape_style:=0;   // default transparent.  0=transparent, 1=blank/solid, 2=cross-hatched or dotted
      ...
      if target_radio_button.Checked=True then shape_code:=4;
      if label_radio_button.Checked=True then shape_code:=3;
      if circle_radio_button.Checked=True then shape_code:=2;
      if rect_radio_button.Checked=True then shape_code:=1;
      if picture_radio_button.Checked=True then shape_code:=-1;     //  not used, see add_picture_shape_button

      shape_name:=name_editbox.Text;        // name or label,

      case shape_code of
                    0: if dotted_radio_button.Checked=True then shape_style:=2;

                  1,2: begin
                         if solid_infill_radio_button.Checked=True then shape_style:=1;
                         if hatched_radio_button.Checked=True then shape_style:=2;
                       end;
      end;//case
      ...
      case add_option of
                ...
                2: begin                    // as already clicked
                     ...
                     -1,1,2: begin     // picture, rectangle, circle
                               ...
                               y1:=MIN(mouse_y(pad_click_X,pad_click_Y),mouse_y(prev_pad_click_X,prev_pad_click_Y));

                               x2:=MAX(mouse_x(pad_click_X),mouse_x(prev_pad_click_X));
                               y2:=MAX(mouse_y(pad_click_X,pad_click_Y),mouse_y(prev_pad_click_X,prev_pad_click_Y));
                             end;
      ...
      case add_option of
         ...
         0,2: begin   // drawn rectangle or clicked corners
                ...
                if ( (shape_code=0) and (p1.x=p2.x) and (p1.y=p2.y) ) or ( ( (shape_code=-1) or (shape_code=1) or (shape_code=2) ) and ( (p1.x=p2.x) or (p1.y=p2.y) )  )
                   then begin
           ...
           1: begin      // add_option=1  enter dimensions...
                repeat
                  if get_shape_dims(False,shape_code,p1,p2)=False then EXIT;

                  if ( (shape_code=0) and (p1.x=p2.x) and (p1.y=p2.y) ) or ( ( (shape_code=-1) or (shape_code=1) or (shape_code=2) ) and ( (p1.x=p2.x) or (p1.y=p2.y) )  )
                     then begin
                            repeat
                              i:=alert(6,'    invalid  shape',
                                         '||This shape can not appear because the length of the line or of one of the sides is zero.',
                                         '','','','?  help','cancel  shape','try  again',4);
                              case i of
                                  4: alert_help(0,inv_shape_help,'');
                                  5: EXIT;
                              end;//case
                            until i<>4;
                          end
         ...
         else valid:=True;  // add_option=3  default picture shape dims          not used

      end;//case

      case shape_code of
            0: normalize_line(p1,p2);        // 214a line
       -1,1,2: normalize_rectangle(p1,p2);   // 214a picture, rectangle, circle
      end;//case

      if shape_name=''         // nothing entered
         then begin
                if add_option=0
                   then drawn_str:='drawn '
                   else drawn_str:='';

                case shape_code of

                 -1: shape_name:=drawn_str+'picture';
                  0: shape_name:=drawn_str+'line';
                  1: shape_name:=drawn_str+'rectangle';
                  2: shape_name:=drawn_str+'circle/ellipse';
                  3: shape_name:=drawn_str+'label';
                  4: shape_name:=drawn_str+'target mark';
      ...
      with bgnd_form.bgnd_shapes_listbox do begin
        with Items do begin
          ...
          n:=AddObject(shape_name,Tbgshape.Create);  // create and insert a new entry in the shapes list.
  ...
  if by_drawing_radiobutton.Checked=True   // go do drawing
     then begin
            if line_radio_button.Checked=True                // 214a  line drawing
               then pad_form.mouse_draw_menu_entry.Click
               else drawn_shape_rectangle:=True;
            EXIT;
            end;

  if enter_dims_radiobutton.Checked=True
     then add_code:=1                      // enter dims
     else add_code:=2;                     // as clicked
...
const
  ...
  +'||X dimensions are measured across the screen from the left, and read on the bottom grid scale.'
  +'||Dimensions measured forwards to the right from the origin (0 mark) are positive; dimensions measured backwards to the left from the origin are negative.'
  +'||Unless you use conversion factors the dimension should be entered in mm.'
  ...
  +'||Width dimensions are measured across the screen, representing the distance between the left and right edges of the picture shape.'
  +'||Width dimensions are always positive. Negative or zero width dimensions are invalid.'
  +'||For a picture shape, you can enter the required width initially or leave the suggested width.'
  +' There are subsequent `0AUTO-FIT`1 functions to scale (re-size) a picture shape to the exact size required after the image has been loaded into it.';
...
begin
  ...
  with new_shape do begin
    ...
    putdim(xpic_str,1,'picture :  bottom  left  corner  X  dimension ',p1.x,False,True,False,False);   // neg ok, no preset, allow zero, don't terminate on zero.
    putdim(ypic_str,1,'picture :  bottom  left  corner  Y  dimension ',p1.y,False,True,False,False);   // ditto.
 i:=putdim(wpic_str,1,'picture :  width ',p2.x-p1.x,True,True,True,False);                             // no neg, no preset, no zero, don't terminate on zero.

    if i<>2 then EXIT;
    if getdims('background  picture  shape',get_str,Screen.ActiveForm,i,od)=True
       then begin
              p1.x:=od[0];
              p1.y:=od[1];
              p2.x:=p1.x+ABS(od[2]);
  ...
  if reload_picture_image(False,False,False,False,True)=False    // scanning,pasting,meta,dropped,adjust_aspect
     then begin
            with bgnd_form.bgnd_shapes_listbox.Items do begin
              if (Count<1) or (n<0) or (n>(Count-1)) then EXIT;  // ???
              ...
              if Count<1 then EXIT;                                                 // list now empty.
              if n<Count then bgnd_form.bgnd_shapes_listbox.ItemIndex:=n            // select next line
                         else bgnd_form.bgnd_shapes_listbox.ItemIndex:=Count-1;
            end;//with
          end
     else shapes_saved:=False;      // need a resave.
...
begin
  if bgnd_shapes_listbox.Items.Count>0
     then begin
            case alert(7,'    delete  all  shapes',
                   ...
                   +'||Are you sure you want to delete all the shapes ?',
                    '','','','yes  -  delete  all  shapes','no  -  cancel  delete','save  first,  then  delete  all',0) of
                 4: clear_shapes;
                 6: begin
                      shapes_saved:=False;
                      save_all_menu_entry.Click;
                      if shapes_saved=True then clear_shapes;
                    end;
            end;//case
          end;
end;
...
begin
  if skip_count<1 then EXIT;
  if bgnd_form.bgnd_shapes_listbox.Items.Count<(1+skip_count) then EXIT;

  case alert(7,'    pruning  -  delete  '+str+'  of  all  shapes',
          '||You are about to remove '+str+' of all background shapes from the drawing by deleting alternate shapes from the list.'
         +'||This pruning function is mainly intended as a thinning-out operation after a large DXF file import to make room for additional shapes or to speed up the screen re-draws.'
         +'||If you will need all these shapes again they should first be saved to a file.'
         +'||Are you sure you want to delete '+str+' of all your shapes ?',
          '','','','yes  -  delete  '+str+'  of  shapes','no  -  cancel  delete','save  first,  then  delete  '+str,0) of
       5: EXIT;
       6: begin
            shapes_saved:=False;
            bgnd_form.save_all_menu_entry.Click;
            if shapes_saved=False then EXIT;
          end;
  end;//case
  ...
  with bgnd_form.bgnd_shapes_listbox.Items do begin
    ...
    if Count>0 then bgnd_form.bgnd_shapes_listbox.ItemIndex:=0;  // should be - we started at the second line!!!
  end;//with
...
try
  with bgnd_shapes_listbox do begin
    ...
    with Items do begin
      ...
      shapes_saved:=False;                    // need a resave.
      if Count<1 then EXIT;                   // list now empty.
      if n<Count then ItemIndex:=n            // select next line
...
begin
  ...
  if (Key=VK_DELETE) and (name_editbox.Focused=False)
     then begin
...
begin
  with bgnd_shapes_listbox do begin
    ...
    with now_shape do begin
      ...
      if name_editbox.Text<>'' then shape_name:=name_editbox.Text;        // new name or label.
      ...
      if circle_radio_button.Checked=True then shape_code:=2;
      if rect_radio_button.Checked=True then shape_code:=1;
      if picture_radio_button.Checked=True then shape_code:=-1;
      ...
      if get_shape_dims(True,shape_code,p1,p2)=False then EXIT;       // get the data for this shape. True=modifying

      if ( (shape_code=0) and (p1.x=p2.x) and (p1.y=p2.y) ) or ( ( (shape_code=-1) or (shape_code=1) or (shape_code=2) ) and ( (p1.x=p2.x) or (p1.y=p2.y) )  )
         then repeat
                i:=alert(6,'    invalid  shape',
                           'This shape will not appear because the length of the line or of one of the sides is zero.',
                           '','','','?  help','cancel  modify  -  shape  unchanged','try  again',4);
                case i of
                      4: alert_help(0,inv_shape_help,'');
                      5: EXIT;
                end;//case
      ...
      if (shape_code<>-1) and (old_shape_code=-1)             // was a picture, but not now.
         then begin                                           // so free the image (but not the shape object).
                if Tbgshape(Items.Objects[n]).bgimage<>nil    // ???
                     then begin
                            Tbgshape(Items.Objects[n]).bgimage.image_shape.image_bitmap.Free;      // free the bitmaps.
                            ...
                            Tbgshape(Items.Objects[n]).bgimage:=nil;                               // and free the image object. 3-2-01.
                          end;
              end;
...
begin
  with bgnd_form.bgnd_shapes_listbox.Items do begin
    if (index<0) or (index>(Count-1)) or (Count<1) then EXIT;

    if Tbgshape(Objects[index]).bgnd_shape.shape_code<>1 then EXIT;   // ??? not a rectangle.
    ...
    try
      ...
      with new_shape do begin
        ...
        if bgnd_form.dotted_radio_button.Checked=True
           then shape_style:=2
           else shape_style:=0;     // 0=transparent, 1=blank/solid, 2=cross-hatched or dotted.
      ...
      n:=AddObject(new_shape.shape_name,Tbgshape.Create);  // create and add entry in the shapes list.
      Tbgshape(Objects[n]).bgnd_shape:=new_shape;          // put data in list.
...
begin
  with bgnd_shapes_listbox do begin
    ...
    n:=ItemIndex;
    if n<0 then n:=0;                                      // nothing was selected.
    if n>(Items.Count-1) then n:=Items.Count-1;            // ???

    if Tbgshape(Items.Objects[n]).bgnd_shape.shape_code<>1 then EXIT;   // ??? not a rectangle.

    name_str:=Items.Strings[n];   // name of rectangle

  end;//with

  if alert(7,'      convert  rectangle ?',
             '|'+name_str
            ...
            +'||Are you sure you want to convert this rectangle ?',
             '','','','','no  -  cancel    ','yes  -  convert  '+name_str+'    ',0)=5 then EXIT;
...
begin
  with bgnd_form do begin
    ...
    if modify=True
       then shape_page_control.ActivePage:=modify_tab_sheet   // 214a
       else shape_page_control.ActivePage:=new_tab_sheet;     // 206d
...
begin
  if grid_labels_code_i<>6 then str:='change  to  mm  on  existing  spacings'
                           else str:='';
...
begin
  bgsmru_list.Clear;
  for n:=0 to 5 do bgsmru_list.Add('');    // always 6 items.
...
begin
  ...
  if n<0 then bgsmru_list.Delete(0)     // no, remove oldest from top.
         else bgsmru_list.Delete(n);    // yes, remove it.
...
begin
  alert(5,'     file  error',
        '||'+str
       +'||Sorry, the requested operation on this file has failed. Please check the file and folder names, and the drive letter.'
       +'||If this is a save operation, check the available space on the hard drive or USB memory stick.'
       +' If overwriting an existing file, check that it is not read-only.'
...
begin
  if dropped=True                                         // 214a
     then drop_str:='Dropped file:||`0'+file_str+'`f'
     else drop_str:='';
    ...
    try
      ...
      or (dropped=True)
         then begin
                ...
                if (append=False) and (bgnd_shapes_listbox.Items.Count>0)   // reload clicked, and something already there ?
                   then begin
                          if shapes_saved=False                 // and not saved...
                             then begin
                                    i:=alert(7,'      reload  shapes  -  save  first ?',
                                            drop_str
                                            ...
                                            +'||Or add new shapes to the existing ones instead?',
                                             '','','add  new  shapes  to  existing    ','replace  existing  shapes  without  saving    ','cancel  reload      ','save  existing  shapes  before  reloading      ',0);
                                    case i of
                                            3: append:=True;
                                            5: EXIT;
                                            6: begin
                                                 shapes_saved:=False;
                                                 save_all_menu_entry.Click;
                                                 if shapes_saved=False then EXIT;     // save cancelled.
                                               end;
                                    end;//case
                                  end
                             else begin
                                    i:=alert(7,'      reload  shapes  -  delete  all  first ?',
                                              drop_str
                                              +'||Reloading shapes from a file will replace all of the existing shapes.'
                                              +'||Are you sure you want to do this?'
                                              +'||Or add new shapes to the existing ones instead?',
                                               '','','','add  new  shapes  to  existing  ','cancel  reload      ','reload  and  replace  existing  shapes      ',0);
                                    case i of
                                        4: append:=True;
                                        5: EXIT;
                                    end;//case
                ...
                if (mru=False) and (dropped=False)
                   then begin
                          with fileload_dialog do begin

                            if user_load_shapes_path=''
                               then InitialDir:=exe_str+'SHAPE-FILES\'
                               else Initialdir:=user_load_shapes_path;
                          ...
                          if fileload_dialog.Execute=True                  // get the file name.
                             then bgs_str:=fileload_dialog.FileName
                             else EXIT;
                        end
                   else bgs_str:=file_str;  // file name is from mru list or a dropped file.

              end
         else bgs_str:=file_str;    // file name supplied, e.g. startup parameter.
         ...
         then begin
                ...
                if append=False
                   then begin
                          ...
                          shapes_saved:=True;                                           // list matches file.
                          if close_checkbox.Checked=True then close_button.Click;
                        end
                   else top_str:=' shapes last added from :  ';
...
begin
  ...
  with Tbgshape(bgnd_shapes_listbox.Items.Objects[bgnd_shapes_listbox.ItemIndex]).bgnd_shape do begin

    if (hide_bits AND $01)=$01              // byte,  0=normal,  1=hide on trackpad,  2=hide on output,  3=hide both
       then hide_bits:=hide_bits AND $FE    // turn bit off, don't hide shape
       else hide_bits:=hide_bits OR $01;    // turn bit on, hide shape
  end;//with
...
begin
  shapes_label_font.Assign(get_font('choose  a  new  font  and  text  colour  for  labels',shapes_label_font,True));
  redraw(True);
end;
//___________________________________________________________________________________________
...
begin
  ...
  if Screen.PixelsPerInch>120 then bgnd_shapes_listbox.ItemHeight:=26;  // 214a  for owner-draw

                   // 0.82.a  22-08-06  recent files...

                   // 208d bug fix - can't use exe_str because not yet initialised

  if FileExists(ExtractFilePath(Application.ExeName)+'bgsmru.txt') then bgsmru_list.LoadFromFile(ExtractFilePath(Application.ExeName)+'bgsmru.txt')
                                                                   else empty_bgsmru;

  if bgsmru_list.Count<>6 then empty_bgsmru;   // file tampered with?
...
begin
  with bgnd_shapes_listbox do begin
    if Items.Count<1 then Hide        // list empty, reveal "no shapes" label.
                     else Show;
  end;//with

  if bgnd_no_update=False then shapes_current_state;   // re-showing after click on the form, he might have set the options for the clicked shape.
  bgnd_no_update:=False;                               // but only skip it once.

  bgs_save_hide:=save_hide;                            // so can restore on close.
  save_hide:=False;                                    // otherwise pad redraws on clicking (pad_form.Activate).

  if check_grey_paper=False
     then begin
            cursor_saved:=pad_form.Cursor;
            pad_form.Cursor:=cross_hairs_cursor;  //crCross;       // so can select shape locations.
          end;
...
const
  help_xshift_str:string='     Shift  all  shapes  on  X.'
                  ...
                  +'||To change the dimensions for a single shape only, enter zero here then select it in the list and click the MODIFY SHAPE tab > CHANGE... button.'
                  +'||Shifting shapes can also be done with a mouse action, click the MOUSE ACTIONS ALL tab.';
...
begin
  ...
  if bgnd_form.bgnd_shapes_listbox.Items.Count<1 then EXIT;
     ...
     putdim(help_xshift_str,1,'shift  all  background  shapes  by  X  ',0,False,True,False,False);     // negative ok, no preset, zero ok, don't terminate on zero.
  n:=putdim(help_yshift_str,1,'shift  all  background  shapes  by  Y  ',0,False,True,False,False);   // negative ok, no preset, zero ok, don't terminate on zero.

  if n<>1 then EXIT;
  if getdims('shift  all  background  shapes','',bgnd_form,n,od)=True then shift_all_shapes(od[0],od[1]);
  ...
  with bgnd_form.bgnd_shapes_listbox do begin
    ...
    for n:=0 to (Items.Count-1) do begin
      ...
      with now_shape do begin
        ...
        if (shape_code=-1) and (bgnd_form.lock_to_origin_radiobutton.Checked=False)       // picture   205e
           then begin
...
begin
  with bgnd_form.bgnd_shapes_listbox.Items do begin
    ...
    case rotate_centre_code of
         1: begin xrot:=notchx;     yrot:=notchy;     end;  // rotate shape around notch
         2: begin xrot:=rings[0,0]; yrot:=rings[0,1]; end;  // rotate shape around spacing ring
       else begin xrot:=0;          yrot:=0;          end;  // 3: rotate around grid origin
    end;//case
    ...
    with Tbgshape(Objects[n]).bgnd_shape do begin      // get existing data.
      ...
      case shape_code of
       ...
       1,2: begin                       // rectangle, circle shapes.     (rectangle should have been broken to lines)
...
begin
  with bgnd_form.bgnd_shapes_listbox.Items do begin
    ...
    for n:=0 to (Count-1) do begin
      if Tbgshape(Objects[n]).bgnd_shape.shape_code=1 then rectangles_exist:=True;
    end;//for

    if rectangles_exist=True
       then begin
              if alert(7,'      convert  all  rectangles ?',
                       '|You are about to convert all rectangle shapes, each to 4 separate lines.'
                      +'||This is necessary to permit rotation of the rectangle outlines, but will remove any hatched or solid filling.'
                      +'||The lines will be drawn solid or dotted according to the current setting.'
                      +'||Are you sure you want to convert all rectangles ?',
                       '','','','','no  -  cancel  rotations','yes  -  convert  all  rectangles  and  rotate',0)=5 then EXIT;

              n:=0;
              while n<Count do begin                             // (Count increases by 3 for every rectangle converted.)
                if Tbgshape(Objects[n]).bgnd_shape.shape_code=1  // code 1 = rectangle.
                    then rectangle_to_lines(n)                   // don't increment n, this entry gets deleted.
                    else INC(n);
              end;//while
            end;

    for n:=0 to (Count-1) do begin
      if (Tbgshape(Objects[n]).bgnd_shape.shape_code=-1) and (pictures=True)
         then begin
                twist_angle:=0-rot_k;
         ...
         else rotate_this_shape(n,rot_k);
    end;
...
begin
  ...
  with bgnd_shapes_listbox do begin
    ...
    with now_shape do begin
      ...
      if (shape_code<>1) and (shape_code<>-1)
         then begin
                help(0,not_this_str,'');
                EXIT;
              end;
      ...
      case shape_code of
        ...
        -1: type_str:='picture';
         ...
         1: type_str:='rectangle';

      end;//case


      xshift_to_str:='     `0shift  '+type_str+'  shape  horizontally  to  X`9'
                  +'||Enter an X-dimension in millimetres for the new position of the left edge of this '+type_str+' shape.'
                  +'||X-dimensions are measured horizontally across screen, positive from the left towards the right.'
                  +'||Enter zero 0 if you want to shift this '+type_str+' shape to the left grid origin line.'
                  +'||Leave the X-dimension unchanged if you do not want to shift this '+type_str+' shape horizontally.'
                  +'||green_panel_begin tree.gif  Shapes can also be shifted using the mouse, click the `0SHIFT`1 button on the `0MOUSE ACTIONS:`1 panel.green_panel_end';


      yshift_to_str:='     `0shift  '+type_str+'  shape  vertically  to  Y`9'
                  +'||Enter a Y-dimension in millimetres for the new position of the bottom edge of this '+type_str+' shape.'
                  +'||Y-dimensions are measured vertically on the screen, the positive direction is upwards from the bottom.'
                  +'||Enter zero 0 if you want to shift this '+type_str+' shape to the bottom grid origin line.'
                  +'||Leave the Y-dimension unchanged if you do not want to shift this '+type_str+' shape vertically.'
                  +'||green_panel_begin tree.gif  Shapes can also be shifted using the mouse, click the `0SHIFT`1 button on the `0MOUSE ACTIONS:`1 panel.green_panel_end';
      ...
      putdim(xshift_to_str,1,'shift  '+type_str+'  shape  horizontally  to  X ',p1.x,False,True,False,False);   // negative ok, no preset, zero ok, don't terminate on zero.
  nn:=putdim(yshift_to_str,1,'shift  '+type_str+'  shape  vertically  to  Y ',  p1.y,False,True,False,False);   // negative ok, no preset, zero ok, don't terminate on zero.

      if nn<>1 then EXIT;
      if getdims('shift  background  '+type_str+'  shape  to ...','',bgnd_form,nn,od)=True
         then begin
...
begin
  ...
  with bgnd_shapes_listbox do begin
    ...
    with Tbgshape(Items.Objects[n]).bgnd_shape do begin
      ...
      scale_to_str:='     `0re-size  '+type_str+'  shape  to  new  width`9'
                  +'||Enter a dimension in millimetres for the new width of this '+type_str+' shape.'
                  +'||The height will be adjusted to maintain the existing aspect ratio.'
                  +'||green_panel_begin tree.gif  Shapes can also be re-sized using the mouse, click the `0SIZE`1 button on the `0MOUSE ACTIONS:`1 panel.'
                  +'||Or to change the position, size and aspect ratio of this '+type_str+' shape in one go, click the `0CHANGE DIMENSIONS...`1 button.green_panel_end';

      if shape_code=-1
         then scale_to_str:=scale_to_str
                  +'|green_panel_begintree.gif A picture shape can be locked to the position on the trackpad of the pegging notch or the spacing-ring by selecting'
                  +' the `0when scaling (re-sizing) lock to:`1 options on the `0picture options`1 tab.'
                  +'||This is useful when scaling (re-sizing) a map or track plan image to match the model scale of your track templates.green_panel_end';
  ...
  if shape_width<minfp
     then begin
            help(0,'||Error - this '+type_str+' shape has zero width and cannot be scaled to a new width.'
                  +'||Click the `0CHANGE DIMENSIONS...`1 button to change the width.','');
            EXIT;
          end;

  nn:=putdim(scale_to_str,1,'re-size  '+type_str+'  shape  to  new  width ',shape_width,True,True,True,False);   // no negative, no preset, no zero, don't terminate on zero.

  if nn<>0 then EXIT;
  if getdims('re-size  background  '+type_str+'  shape ...','',bgnd_form,nn,od)=True
     then begin
...
const
  ...
  all_shift_str:string='||green_panel_begin tree.gif  To shift all the shapes in one go by the same amount, click the `0SHIFT ALL BY...`1 button on the `0MODIFY ALL`1 tab.'
                      +'||Shapes can also be shifted by mouse action, click the `0SHIFT`1 button on the `0MOUSE ACTIONS:`1 panel.green_panel_end';
...
begin
  ...
  if bgnd_shapes_listbox.Items.Count<1 then EXIT;
     ...
     putdim(xshift_str+all_shift_str,1,'shift  shape  horizontally  by  X ',0,False,True,False,False);   // negative ok, no preset, zero ok, don't terminate on zero.
  n:=putdim(yshift_str+all_shift_str,1,'shift  shape  vertically  by  Y ',  0,False,True,False,False);   // negative ok, no preset, zero ok, don't terminate on zero.

  if n<>1 then EXIT;
  if getdims('shift  background  shape','',bgnd_form,n,od)=True then shift_this_shape(od[0],od[1],0);
  ...
  draw_bg_shapes(pad_form.Canvas,bgnd_shapes_listbox.ItemIndex,clRed);   // highlight this one in red, directly on the pad.
end;
//____________________________________________________________________________________________
...
const
  rotshap_str:string='     `0rotate  background  shape`9'
    ...
    +'||The centre of rotation can be the grid origin, or the current position of the notch, or the current position of the spacing-ring.'
    +' These options are set on the `0modify all`1 tab > `0rotate all around:`1 panel.'
    +'||The current setting is to rotate this background shape around the';
...
begin
  ...
  if old_count<1 then EXIT;

  j:=bgnd_form.bgnd_shapes_listbox.ItemIndex;

  if j<0 then EXIT;
  if j>(old_count-1) then EXIT;

  if Tbgshape(bgnd_form.bgnd_shapes_listbox.Items.Objects[j]).bgnd_shape.shape_code=-1      // picture shape not done here
     then begin
  ...
  case rotate_centre_code of
      1: rot_cen_str:='||                              <b>notch</b>.';
      2: rot_cen_str:='||                              <b>spacing-ring</b>.';
    else rot_cen_str:='||                              <b>grid origin</b>.';
  end;//case

  case rotate_centre_code of
      1: cen_str:='notch';
      2: cen_str:='spacing-ring';
    else cen_str:='grid origin';
  end;//case
  ...
  if Tbgshape(bgnd_form.bgnd_shapes_listbox.Items.Objects[j]).bgnd_shape.shape_code=1  // rectangle
     then begin
            if alert(7,'      rotate  rectangle  shape',
                     ...
                     +'||A rectangle must be broken into 4 separate lines in order to be rotated. Any existing infill will be discarded.',
                      '','','','','cancel  rotate','OK  break  rectangle  to  4  lines  and  rotate',0)=5 then EXIT;
  ...
  n:=putdim(rotshap_str+rot_cen_str,3,'rotate  background  shape  around  '+cen_str+'  by',rotshap_k*180/Pi,False,True,False,False);   // negative ok, no preset, zero ok, don't terminate on zero.

  if n<>0 then EXIT;
  if getdims('rotate  background  shape','',bgnd_form,n,od)=True
     then begin
...
const
  help_scale_one_str:string='     `0scale  ( re-size )  this  shape`9'
                  ...
                  +'||If the shape is a label or target mark, scaling changes its position. For other shapes the position on the grid is unchanged and only the size of the shape changes.'

                  +'||green_panel_begintree.gif A picture shape can be locked instead to the position on the trackpad of the pegging notch or the spacing-ring by selecting'
                  +' the `0when scaling (re-sizing) lock to:`1 options on the `0picture options`1 tab.'
                  +'||This is useful when scaling (re-sizing) a map or track plan image to match the model scale of your track templates.green_panel_end'

                  +'|To change the position after scaling click the `0SHIFT TO...`1 or `0SHIFT BY...`1 or `0CHANGE DIMENSIONS...`1 buttons on the `0MODIFY SHAPE`1 tab.'
                  +'||If a negative factor is entered, the shape will additionally be rotated 180 degrees. For a label or target mark, this rotation will be around the grid origin.'
                  +'||A zero factor is not valid. To delete the shape click the `0EDIT > DELETE SHAPE`1 menu item.'
                  +'||To change the dimensions for all the shapes, cancel this and then click the `0SCALE ALL BY...`1 button on the `0MODIFY ALL`1 tab.';
var
  n:integer;
  od:Toutdim;
...
begin
  ...
  n:=putdim(help_scale_one_str,4,'scale  this  background  shape  by  ',100,False,True,true,False);     // negative ok, no preset, no zero, don't terminate on zero.

  if n<>0 then EXIT;
  if getdims('scale  background  shape','',bgnd_form,n,od)=True then scale_this_shape(od[0]/100,od[0]/100);
...
const
  help_scale_all_str:string='     Scale  all  shapes.'
                  ...
                  +'||Scaling all shapes also changes their position on the drawing. To adjust the positions after scaling use the buttons on the MODIFY ALL tab.'
                  +'||If a negative factor is entered, the shapes will additionally be rotated 180 degrees around the drawing origin.'
                  +'||A zero factor is not valid. To delete all your shapes click the DELETE ALL button instead.'
                  +'||To change the dimensions for a single shape only, cancel this, select the shape in the list and then click the buttons on the MODIFY SHAPE tab.';
var
  n:integer;
  od:Toutdim;
...
begin
  ...
  n:=putdim(help_scale_all_str,4,'scale  all  background  shapes  by  ',100,False,True,true,False);     // negative ok, no preset, no zero, don't terminate on zero.

  if n<>0 then EXIT;
  if getdims('scale  all  background  shapes','',bgnd_form,n,od)=True then scale_all_shapes(od[0]/100,od[0]/100);
...
begin
  with bgnd_form.bgnd_shapes_listbox.Items do begin
    ...
    if alert(7,'      convert  all  rectangles ?',
             '|You are about to convert all rectangle shapes, each to 4 separate lines.'
            +'||This will make them easier to modify, and will permit rotation of the rectangle outlines, but will remove any hatched or solid filling.'
            +'||The lines will be drawn solid or dotted according to the current setting.'
            +'||Are you sure you want to convert all rectangles ?',
             '','','','','no  -  cancel    ','yes  -  convert  all  rectangles    ',0)=5 then EXIT;
...
const
  help_rotshap_str:string='     rotate  all  shapes'
                        ...
                        +'||The centre of rotation can be the grid origin, or the current position of the notch, or the current position of the spacing-ring.'
                        +' These options are set on the `0modify all`1 tab > `0rotate all around:`1 panel.'
                        +'||The current setting is to rotate all background shapes around the';
...
begin
  if bgnd_shapes_listbox.Items.Count<1 then EXIT;     // no shapes to rotate.
  ...
  n:=putdim(help_rotshap_str+rot_cen_str,3,'rotate  all  shapes  around  '+cen_str+'  by',rotshap_k*180/Pi,False,True,False,False);   // negative ok, no preset, zero ok, don't terminate on zero.

  if n<>0 then EXIT;
  if getdims('rotate  all  background  shapes  around  '+cen_str,'',bgnd_form,n,od)=True
     then begin
            rotshap_k:=od[0]*Pi/180;
            normalize_angle(rotshap_k);
          end
  ...
  rotate_all_shapes(False,True,rotshap_k);  // do the calcs    False = not sync from group rotate   True = include pictures
...
begin
   putdim(shift_str,1,'shift  imported  DXF  X  dimensions  by',input_shift_x,False,True,False,False);   // neg ok, no preset, allow zero, don't terminate on zero.
   putdim(shift_str,1,'shift  imported  DXF  Y  dimensions  by',input_shift_y,False,True,False,False);   // ditto.
i:=putdim(scale_str,4,'scale  all  imported  DXF  dimensions  by',input_scale_factor*100,False,True,True,False);  // neg ok, no preset, no zero, don't etc.
   if i<>2 then EXIT;
   if getdims('modify  DXF  on  import','',bgnd_form,i,od)=True
      then begin
             input_shift_x:=od[0];
             input_shift_y:=od[1];
...
begin
  ...
  with bgnd_form.bgnd_shapes_listbox do begin
    ...
    if Items.Count<1 then EXIT;   // no shapes.

    n:=ItemIndex;

    if n<0 then n:=0;

    if Tbgshape(Items.Objects[n]).bgnd_shape.shape_code<>-1 then EXIT;                  // not a picture shape.

    img_file_name_str:=get_user_image(dropped,meta,scanning,pasting,True,n,dummy1,dummy2);      // get image file and file name for list.
    if img_file_name_str=empty_picture_str
       then EXIT
       else RESULT:=True;
...
begin
  reload_picture_image(False,False,False,False,False);    // scanning,pasting,meta,dropped,adjust_aspect
end;
//______________________________________________________________________________
...
begin
  reload_picture_image(False,True,False,False,False);    // scanning,pasting,meta,dropped,adjust_aspect
end;
//______________________________________________________________________________
...
begin
  reload_picture_image(True,False,False,False,False);    // scanning,pasting,meta,dropped,adjust_aspect
end;
//______________________________________________________________________________
...
begin
  ...
  with bgnd_form.bgnd_shapes_listbox.Items do begin
    ...
    if Count>=count_limit
       then begin
              alert(2,'    dxf  import  -  too  many  shapes',
                      '||You have now reached the limit of  '+IntToStr(count_limit)+'  background shapes. It is not possible to continue importing shapes from:'
                     +'|| '+his_dxf_file_name
    ...
    n:=AddObject(dxf_shape.shape_name,Tbgshape.Create);  // create and add a new line in the shapes list.
...
begin
  ...
  if comma_dp=False then EXIT;  // using dot decimal point

    // convert to a comma for StrTofloat function

  RESULT:=StringReplace(in_str,'.',',',[]);  // 0.94.a  StringReplace(str,'old','new',[rfReplaceAll, rfIgnoreCase]);
end;
//______________________________________________________________________________
...
begin
  ...
  try
    ...
    if k2<k1 then inc_k:=0-0.004        // 0.004 rads = 1/4 degree steps (arbitrary).
             else inc_k:=0.004;
    ...
    with new_shape do begin
      ...
      repeat
        ...
        shape_name:=ExtractFileName(bgnd_form.dxf_file_dialog.FileName)+' A# '+IntToStr(shape_count);
        shape_code:=0;      // 0=line, 1=rectangle, 2=circle, 3=text.
        shape_style:=0;     // 0=transparent, 1=blank/solid, 2=cross-hatched;

        RESULT:=add_dxf_shape(new_shape);

        if RESULT=False then EXIT;       // reached limit
...
begin
  with bgnd_form.dxf_file_dialog do begin                     // set up the open dialog.
    ...
    if bgnd_form.dxf_mm_radio.Checked=True
       then begin
    ...
    if his_dxf_file_name<>'' then InitialDir:=ExtractFilePath(his_dxf_file_name)   // use his previous folder.
                             else InitialDir:=exe_str+'DXF-FILES\';
  ...
  comma_dp:=(Pos(',',test_str)>0);    // True=running on a system with comma decimal point separator
  ...
  try
    try
      ...
      repeat                               // first get synchronised for 2-line groups.
        if Eof(dxf_file)=True then EXIT;
      ...
      repeat
        if read_2_input_lines=True then EXIT;             // end of file
      ...
      100:
        ...
        if value_str='LINE'
           then begin
                  repeat
                    if read_2_input_lines=True then EXIT;   // end of file
                  until (code_str='10') or (code_str='0');  // find x1 value (or not present - next entity found)
                  if code_str='0' then goto 123;
                  x1_str:=value_str;

                  repeat
                    if read_2_input_lines=True then EXIT;   // end of file
                  until (code_str='20') or (code_str='0');  // find y1 value (or not present - next entity found)
                  if code_str='0' then goto 123;
                  y1_str:=value_str;

                  repeat
                    if read_2_input_lines=True then EXIT;   // end of file
                  until (code_str='11') or (code_str='0');  // find x2 value (or not present - next entity found)
                  if code_str='0' then goto 123;
                  x2_str:=value_str;

                  repeat
                    if read_2_input_lines=True then EXIT;   // end of file
                  until (code_str='21') or (code_str='0');  // find y2 value (or not present - next entity found)
                  if code_str='0' then goto 123;
                  y2_str:=value_str;

                  if add_a_line_shape(x1_str,y1_str,x2_str,y2_str)=False then EXIT;  // reached list limit.
                end;//line
        ...
        if value_str='ARC'
           then begin
                  ...
                  repeat
                    if read_2_input_lines=True then EXIT;   // end of file
                  until (code_str='40') or (code_str='0');  // find rad value (or not present - next entity found)
                  if code_str='0' then goto 123;
                  rad_str:=value_str;

                  repeat
                    if read_2_input_lines=True then EXIT;   // end of file
                  until (code_str='50') or (code_str='0');  // find start angle value (or not present - next entity found)
                  if code_str='0' then goto 123;
                  startangle_str:=value_str;

                  repeat
                    if read_2_input_lines=True then EXIT;   // end of file
                  until (code_str='51') or (code_str='0');  // find end angle value (or not present - next entity found)
                  if code_str='0' then goto 123;
                  endangle_str:=value_str;

                  if add_an_arc_shape(x1_str,y1_str,rad_str,startangle_str,endangle_str)=False then EXIT;  // reached list limit.
                end;//arc

        if (value_str='TRACE') or (value_str='SOLID')
           then begin
                  repeat
                    if read_2_input_lines=True then EXIT;   // end of file
                  until (code_str='10') or (code_str='0');  // find x1 value (or not present - next entity found)
                  ...
                  repeat
                    if read_2_input_lines=True then EXIT;   // end of file
                  until (code_str='12') or (code_str='0');  // find x3 value (or not present - next entity found)
                  if code_str='0' then goto 123;
                  x3_str:=value_str;

                  repeat
                    if read_2_input_lines=True then EXIT;   // end of file
                  until (code_str='22') or (code_str='0');  // find y3 value (or not present - next entity found)
                  if code_str='0' then goto 123;
                  y3_str:=value_str;

                  repeat
                    if read_2_input_lines=True then EXIT;   // end of file
                  until (code_str='13') or (code_str='0');  // find x4 value (or not present - next entity found)
                  if code_str='0' then goto 123;
                  x4_str:=value_str;

                  repeat
                    if read_2_input_lines=True then EXIT;   // end of file
                  until (code_str='23') or (code_str='0');  // find y4 value (or not present - next entity found)
                  if code_str='0' then goto 123;
                  y4_str:=value_str;

                  if add_a_line_shape(x1_str,y1_str,x2_str,y2_str)=False then EXIT;   // add 4 lines marking the outline of a trace or solid.
                  if add_a_line_shape(x2_str,y2_str,x4_str,y4_str)=False then EXIT;   // (this sequence arrived at by trial and error).
                  if add_a_line_shape(x4_str,y4_str,x3_str,y3_str)=False then EXIT;
                  if add_a_line_shape(x3_str,y3_str,x1_str,y1_str)=False then EXIT;

                end;//trace or solid

        if value_str='POLYLINE'
           then begin
                    ...
                    if value_str='VERTEX'
                       then begin
                              repeat
                                if read_2_input_lines=True then EXIT;   // end of file
                              until (code_str='10') or (code_str='0');  // find x1 value (or not present - next entity found)
                              if value_str='SEQEND' then goto 100;      // end of polyline, return to outer loop.
                              if code_str='0' then goto 456;            // polyline not finished, ignore this and look for next vertex.

                              if vertices_count>0 then x1_str:=x2_str;  // previous vertex.
                              ...
                              if vertices_count>0 then y1_str:=y2_str;  // previous vertex.
                              y2_str:=value_str;                        // new vertex.

                              if vertices_count>0 then if add_a_line_shape(x1_str,y1_str,x2_str,y2_str)=False then EXIT;  // reached list limit.

                              INC(vertices_count)
                            end;//vertex

                    if (value_str='LINE') or (value_str='POLYLINE') or (value_str='CIRCLE') or (value_str='TEXT') or (value_str='POINT') or (value_str='TRACE') or (value_str='SOLID')
                       then goto 123;
        ...
        if value_str='CIRCLE'
           then begin
                  ...
                  if add_a_circle_shape(x1_str,y1_str,rad_str)=False then EXIT;  // reached list limit.
                end;//circle
       ...
       if value_str='TEXT'
           then begin
                  ...
                  repeat
                    if read_2_input_lines=True then EXIT;   // end of file
                  until (code_str='1') or (code_str='0');   // find text string (or not present - next entity found)
                  if code_str='0' then goto 123;

                  if add_a_label_shape(x1_str,y1_str,value_str)=False then EXIT;  // reached list limit.
                end;//text
        ...
        if value_str='POINT'
           then begin
                  ...
                  if add_a_target_shape(x1_str,y1_str)=False then EXIT;  // reached list limit.
                end;//point
...
const
  ...
  +'||For comfortable working with transparent picture shapes you may want to change the background colour of the trackpad. Click the `0TRACKPAD > TRACKPAD COLOURS > TRACKPAD COLOUR...`1 menu item.'
  +'||For monochrome images (black and white drawings), any trackpad colour other than very dark colours or black can be used.'
  +'||For grey-scale images (black and white photographs), a light pastel colour or white is preferable.'
  +'||For colour images, the only suitable trackpad colour is white.'

  +'||Handy Hints:'
  +'|A similar effect with faster re-draws can be obtained by selecting instead the GENERAL OPTIONS tab > GRID IN FRONT OF SHAPES tickbox.'

  +'||The transparent option has no effect on printed track templates. Click the PICTURE SHAPES tab on the PRINT window for more information about printing picture shapes.'
  +'||The transparent effect applies only to the way the image is displayed in Templot, the original bitmap image file is not modified in any way.'

  +'||Please bear in mind that Templot0 is not fully-fledged graphics imaging software - this "bare-bones" function is intended primarily to permit the alignment of scanned track plans.'
  +' Some degradation of the image quality is likely.';
...
begin
  with bgnd_shapes_listbox do begin
    if Items.Count<1 then EXIT;        // ??  check box should be disabled.
    n:=ItemIndex;
    if (n<0) or (n>(Items.Count-1)) then EXIT;  // ??

    if Tbgshape(Items.Objects[n]).bgnd_shape.shape_code<>-1 then EXIT; // ?? not  picture shape.
  end;//with

  if trans_checkbox.Checked=True
     then begin

            if trans_msg_pref=False
               then begin
                      alert_box.preferences_checkbox.Checked:=False;       //%%%%
                      alert_box.preferences_checkbox.Show;
                      ...
                      if i=5 then trans_checkbox.Checked:=False;  // and continue below.

                    end;
          end;

  Tbgshape(bgnd_shapes_listbox.Items.Objects[n]).bgnd_shape.show_transparent:=trans_checkbox.Checked;   // 0.93.a now in file
...
procedure twist_picture(i:integer; krot:extended; do_container,all,sync:boolean); // rotate bitmap supplied krot clockwise.
...
begin
  ...
  if krot<(0-Pi/2) then krot:=0-Pi/2;      // 90 deg limits
  if krot>(Pi/2) then krot:=Pi/2;

  if (i<0) or (i>bgnd_form.bgnd_shapes_listbox.Items.Count-1) then EXIT;

  with Tbgshape(bgnd_form.bgnd_shapes_listbox.Items.Objects[i]) do begin

    if bgnd_shape.shape_code<>-1 then EXIT;      // not a picture shape.
    ...
    try
      with bgimage.image_shape do begin
        ...
        if sync=False
           ...
           else action_form.action_label.Caption:='twisting  image  '+IntToStr(i)+' ...';
        ...
        outwidth:=ABS(diagonal*COS(inaspectk-ABS(krot)));      // size change is same in both directions of krot.
        outheight:=ABS(diagonal*SIN(inaspectk+ABS(krot)));

        try
          rotated_bitmap.Assign(image_bitmap);        // set same properties, bits/pixel, etc.
          rotated_bitmap.Width:=Round(outwidth);      // ensure output is large enough to contain it.
          ...
          for inrow:=0 to Trunc(inheight-1) do begin
            for incol:=0 to Trunc(inwidth-1) do begin
              ...
              if krot>0 then pout.x:=pout.x+inheight*SIN(krot);   // shift origin across.
              if krot<0 then pout.y:=pout.y+inwidth*SIN(0-krot);  // shift origin down.
            ...
            if sync=False then wait_form.wait_progressbar.StepIt;
            if sync=False then Application.ProcessMessages;

          end;//for

          rotated_bitmap.SaveToFile(exe_str+'_85a_temp.bmp');   // !!! this way corrects a bug with the display of monochrome bitmaps.
          ...
          if do_container=True      // 219a
             then begin
                    ...
                    with bgnd_shape do begin     // re-size the shape to suit.
                      ...
                      if (bgnd_form.twist_as_all_radiobutton.Checked=True) or (all=True)
                         then begin
                                case rotate_centre_code of
                                  1: dotransform(0-krot,notchx,notchy,shape_cen,shape_cen);          // rotate shape centre around notch.
                                  2: dotransform(0-krot,rings[0,0],rings[0,1],shape_cen,shape_cen);  // rotate shape centre around spacing ring.
                                  3: dotransform(0-krot,0,0,shape_cen,shape_cen);                    // rotate shape centre around grid origin.
                                end;// case
                              end;
          ...
          if sync=False then wait_form.Hide;
          Application.ProcessMessages;
          alert(5,'twist  bitmap  error','Sorry, the twist bitmap function failed because of memory or resources limitations on your system.'
      ...
      if sync=False then wait_form.wait_progressbar.Visible:=False;
      if sync=False then wait_form.Hide;
      Application.ProcessMessages;
    end;//try-finally
  end;//with shape
...
const
  ...
  +'||For more information about twisting bitmap images click the button below.';
...
begin
  ...
  if bgnd_form.twist_as_all_radiobutton.Checked=True
     then begin
            case rotate_centre_code of
              1: twist_cen_str:='the  notch';
              2: twist_cen_str:='spacing-ring';
              3: twist_cen_str:='grid  origin';
            end;//case
          end
     else twist_cen_str:='its  centre';

  n:=putdim(help_twbmp_str,3,'twist  bitmap  image  around  '+twist_cen_str+'  by',0,False,True,False,False);   // negative ok, no preset, zero ok, don't terminate on zero.

  if n<>0 then EXIT;
  if getdims('twist  bitmap  image',twist_help_str,bgnd_form,n,od)=True
     then begin
            ...
            twist_picture(n,twist_angle,True,False,False);    // True,False,False = do picture rectangle too, this one only, not sync
          end
     else EXIT;
end;
//______________________________________________________________________________________
...
begin
  ...
  while k>deg90 do k:=k-deg90;           // range +/- 90 degs.
  while k<(0-deg90) do k:=k+deg90;
end;
//______________________________________________________________________________________
...
begin
  if marker_angle_twist_is_defined=True
     then begin
            if use_notch_fixed_marker_checkbox.Checked=False
               then begin
                      ...
                      tw_str:='use  marker  lines  ( twist  angle : '+round_str(0-twist_angle*180/Pi,2)+' degrees )';
                    end
               else begin
                      twist_angle:=marker_angle_twist-notch_angle;
                      minimize_angle(twist_angle);
                      tw_str:='use marker line and notch (twist angle : '+round_str(0-twist_angle*180/Pi,2)+' degrees)';
                    end;
          end
     else tw_str:='( no  twist  marker  line  has  been  set )';
  ...
  with bgnd_shapes_listbox do begin
    ...
    with Items do begin
      ...
      if (Count<1) or (n<0) or (n>(Count-1)) then EXIT;
      if Tbgshape(Objects[n]).bgnd_shape.shape_code<>-1 then EXIT;      // not a picture shape.
    end;//with
  end;//with

  repeat
    i:=alert(4,'    twist  picture  shape',
                'This function will twist (rotate) the image contained in this picture shape. It may take several seconds to complete.'
               +'||You can choose to enter a twist angle in degrees, or let Templot0 use the calculated twist angle shown below to align your marker lines.'
               +'||(You may want to make a note of this angle and use it in other graphics software to rotate this or other bitmap images.)'
               +'||Before continuing you may want to save a BGS file, so that you can return to the untwisted original if necessary.',
                '','','important  information','enter  twist  angle','cancel',tw_str,3);
    case i of
           3: alert_help(0,twist_help_str,'');
           ...
           6: begin
                if marker_angle_twist_is_defined=False
                   then begin
                          if alert(6,'    no  marker  line',
                                     'No twist marker line has been set.',
                                     '','','','more  information','cancel','',4)=5 then EXIT;
...
begin
  with bgnd_shapes_listbox do begin
    ...
    with Tbgshape(Items.Objects[n]).bgnd_shape do begin
      ...
      if shape_code<>0 then EXIT;  // ??? not a line - button should be disabled.
      ...
      try
        ...
        if hyp<minfp then marker_angle_fixed:=0                         // single point.
                     else marker_angle_fixed:=ARCSIN((p2.y-p1.y)/hyp);
      except
  ...
  with bgnd_shapes_listbox do begin
    ...
    with Tbgshape(Items.Objects[n]).bgnd_shape do begin
      ...
      try
        ...
        hyp:=SQRT(SQR(p2.x-p1.x)+SQR(p2.y-p1.y));
        if hyp<minfp then marker_angle_twist:=0                         // single point.
                     else marker_angle_twist:=ARCSIN((p2.y-p1.y)/hyp);
...
begin
  shapes_current_state; //marker_line_fixed_button.Enabled:= NOT use_notch_fixed_marker_checkbox.Checked;
end;
//_____________________________________________________________________________________
...
begin
  if rotate_grid_radiobutton.Checked=True then rotate_centre_code:=3;     // default
end;
...
begin
  if rotate_notch_radiobutton.Checked=True then rotate_centre_code:=1;
end;
...
begin
  if rotate_ring_radiobutton.Checked=True then rotate_centre_code:=2;
end;
//_____________________________________________________________________________________
...
begin
  if bgsmru_list.Count<>6 then EXIT;  // ??? error
  ...
  with bgnd_form do recent_popup_menu.PopUp(Left+ClientWidth div 10,Top+ClientHeight div 5);    // 0.82.a
end;
...
begin
  if bgsmru_list.Strings[5]<>'' then load_shapes(bgsmru_list.Strings[5],False,True,False);
end;
...
begin
  if bgsmru_list.Strings[4]<>'' then load_shapes(bgsmru_list.Strings[4],False,True,False);
end;
//_____________________________
...
begin
  if bgsmru_list.Strings[3]<>'' then load_shapes(bgsmru_list.Strings[3],False,True,False);
end;
//_____________________________
...
begin
  if bgsmru_list.Strings[2]<>'' then load_shapes(bgsmru_list.Strings[2],False,True,False);
end;
//_____________________________
...
begin
  if bgsmru_list.Strings[1]<>'' then load_shapes(bgsmru_list.Strings[1],False,True,False);
end;
//_____________________________
...
begin
  if bgsmru_list.Strings[0]<>'' then load_shapes(bgsmru_list.Strings[0],False,True,False);
end;
...
const
  ...
  banding_help_str:string='php/810    `0straighten  picture  shape  -  banding`9'
   ...
   +'||green_panel_begintree.gif If the control template is plain track you can quickly see which side is TS and which is MS by inserting a turnout. This will have no effect on the straightening function.'
   +'||It is normally best to arrange the control template as a LH (left-hand) template running from left to right across the image with the `0CTRL-0`2 position on the left.'
   +'||In which case the TS (turnout) side band will be above the control template and the MS (main) side band will be below it.green_panel_end';
  ...
  with Tbgshape(bgnd_form.bgnd_shapes_listbox.Items.Objects[i]) do begin
    ...
    with this_shape do begin
      ...
      repeat
        ...
        j:=alert(4,'php/810   apply  banding  limits ?',
                   ...
                   '','','more  information','yes  -  enter  band  dimensions','cancel','no  -  straighten  entire  image',3);
        case j of
              3: alert_help(0,banding_help_str,'');
              4: begin
                   putdim(banding_help_str,1,'band  depth  TS  side',ts_limit_mm,True,True,False,False);    // no neg, no preset, zero ok, don't terminate on zero.
                n:=putdim(banding_help_str,1,'band  depth  MS  side',ms_limit_mm,True,True,False,False);    // ditto.

                   if n<>1 then run_error(159);

                   if getdims('straighten  picture  shape  along  the  control  template  -  banding  limits','',bgnd_form,n,od)=False then EXIT;
      ...
      if alert(7,'php/810 straighten  picture  shape  along  the  control  template',
            ...
            +'||Before proceeding you may want to save your current background shapes so that you can revert to them if the straightening results are not as expected or intended .'
            +'||Cancel this and click the `0save all as...`1 button to save your background shapes.',
             '','','','','cancel','continue  -  straighten  picture  shape',0)=5 then EXIT;
    ...
    try
      with bgimage.image_shape do begin
        ...
        if (inwidth_dots<1) or (inheight_dots<1) then EXIT;
        ...
        repeat
          ...
          repeat
            ...
            if get_col_row(x_on_pad,y_on_pad,incol,inrow)=True
               then begin
                      if max_y<(ys_on_control-g/2) then max_y:=ys_on_control-g/2;       // from centre-line
                      if min_y>(ys_on_control-g/2) then min_y:=ys_on_control-g/2;
                    end;

            ys_on_control:=ys_on_control+ys_step;

          until (ys_on_control>old_diagonal_mm) or ( (ts_limit_mm>0) and (ys_on_control>(g/2+ts_limit_mm)) );
          ...
          until (ys_on_control<(0-old_diagonal_mm)) or ( (ms_limit_mm>0) and (ys_on_control<(g/2-ms_limit_mm)) );
        ...
        if (max_y=0-maxfp) or (min_y=maxfp)
           then begin
                  alert(6,'php/810    control  template  in  wrong  position',
                          '`0straighten image along control template`9'
                         +'||rp.gif The control template is in the wrong position. No part of the control template is currently over the selected picture shape.'
                         +'||In order to straighten the image along the control template, all or part of the control template main-road centre-line must be positioned over the image.'
                         +'||Please check that you have selected the correct picture shape in the list.'
                         +'||Otherwise please click the `0more information online`1 button above for more information and explanation.',
                          '','','','','cancel','',0);
                  EXIT;
                end;
        ...
        try
          ...
          destination_bitmap.Width:=outwidth_dots;        // ensure output is large enough to contain it.
          ...
          repeat
            ...
            repeat
              ...
              if get_col_row(x_on_pad,y_on_pad,incol,inrow)=True    // on the image
                 then begin
                        if NOT ( (incol<0) or (incol>(inwidth_dots-1)) or (inrow<0) or (inrow>(inheight_dots-1)) )
                           then begin
                                  pixel_colour:=image_bitmap.Canvas.Pixels[incol,inrow];
                                  ...
                                  with destination_bitmap.Canvas do begin
                                    ...
                                    outrow:=outheight_dots-1-Round((ys_on_control-g/2-min_y)*picture_scale_height);  // top-down
          ...
          destination_bitmap.SaveToFile(exe_str+'_85a_temp.bmp');   // !!! this way corrects a bug with the display of monochrome bitmaps.
          ...
          docurving(True,True,turnoutx,g/2,x_end_mm,ycurved,dummy_k,dummy_r);    // CTRL-9 on pad (to get direction from peg)

          with this_shape do begin     // re-size the shape to suit.

            if x_end_mm>x_org_mm then  p2.x:=p1.x+new_shapewidth_mm   // p1 unchanged x, new p2
                                 else  p1.x:=p2.x-new_shapewidth_mm;  // p2 unchanged x, new p1
          ...
          alert(5,'straighten  bitmap  error','Sorry, the straighten bitmap function failed because of memory or resources limitations on your system.'
                 +'||Try closing all other applications, temporarily deleting other picture shapes, or re-scanning smaller images or at a lower scan resolution (dpi setting).',
                  '','','','','cancel','',0);
        end;//try-except
...
procedure wrap_picture(datum_offset_mm,x_factor,y_factor:extended); // curve bitmap along datum line to align with the control template.
      ...
      with bgimage.image_shape do begin
        ...
        for inrow:=0 to Trunc(inheight-1) do begin
          for incol:=0 to Trunc(inwidth-1) do begin
             ...
             ys:=g/2+(old_shapeheight-inrow/inheight*old_shapeheight-datum_offset_mm)*y_factor*hand_i;

               // get xc,yc,tn,rn ...

             docurving(True,True,xs,ys,xc,yc,tn,rn);    // calc curving to control template, and call any transforms wanted.

             yc_on_pad:=yc*hand_i+y_datum;

             if xc>max_extent_x then max_extent_x:=xc;
             if yc_on_pad>max_extent_y then max_extent_y:=yc_on_pad;

             if xc<min_extent_x then min_extent_x:=xc;
             if yc_on_pad<min_extent_y then min_extent_y:=yc_on_pad;
        ...
        try
          destination_bitmap.Assign(image_bitmap);        // set same properties, bits/pixel, etc.
          destination_bitmap.Width:=Round(outwidth);      // ensure output is large enough to contain it.
          ...
          for inrow:=0 to Trunc(inheight-1) do begin
            for incol:=0 to Trunc(inwidth-1) do begin
              ...
              with destination_bitmap.Canvas do begin
                outcol:=Trunc((xc-min_extent_x)*picture_scale_width);     // slow method - makes better job of filling all pixels.
                outrow:=destination_bitmap_height-Trunc((yc_on_pad-min_extent_y)*picture_scale_height{-0.5});
                Pen.Color:=pixel_colour;
                MoveTo(outcol,outrow);
                LineTo(outcol+1,outrow);        // most of this will get overwritten later.
                LineTo(outcol+1,outrow+1);
          ...
          alert(5,'wrap  bitmap  error','Sorry, the wrap bitmap function failed because of memory or resources limitations on your system.'
                 +'||Try closing all other applications, temporarily deleting other picture shapes, or re-scanning smaller images or at a lower scan resolution (dpi setting).',
                  '','','','','cancel','',0);
        end;//try-except
...
begin
  if straighten_picture=False then EXIT;   // failed or cancelled
...
const
  wrapping_help_str:string='php/810    `0wrapping  line  offset`9'
   +'||Enter a dimension in mm from the bottom edge of the picture shape to the position of a horizontal reference line through the image which will be used to wrap the image along the control template.'
   +'||green_panel_begin tree.gif If you have previously straightened this image using the `0straighten along the control template`1 function you should normally leave the dimension showing unchanged - click `0enter`1.green_panel_end';

  wrap_shrinkx_help_str:string='php/810    `0length - wise  shrink / stretch  factor`9'
   +'||While the image is being wrapped along the control template it can be shrunk or stretched length-wise. Enter the % change required. For example entering 75 will reduce the image length to 75% of its unwrapped length.'
   +'||This can be useful in fitting prototype track plans in a restricted model space.'
   +'||Enter 100 to leave the image at its original size (100%).';

  wrap_shrinky_help_str:string='php/810    `0width - wise  shrink / stretch  factor`9'
   +'||While the image is being wrapped along the control template it can be shrunk or stretched width-wise (i.e. in image height). Enter the % change required.'
   +' For example entering 80 will reduce the image width (height) to 80% of its unwrapped width (height).'
   +'||This can be useful in fitting prototype track plans in a restricted model space.'
   +'||Enter 100 to leave the image at its original size (100%).'
   +'||rp.gif If any width-wise shrinking or stretching is done, after wrapping the offset in mm to the reference line will have been modified accordingly. Bear this in mind when checking the finished dimensions.';
...
begin
  ...
  if (i<0) or (i>bgnd_shapes_listbox.Items.Count-1) then EXIT;
  ...
  with Tbgshape(bgnd_shapes_listbox.Items.Objects[i]) do begin
    ...
    if (datum_offset_mm<0) or (datum_offset_mm>10000)  // must be old BGS file before 205e - 10000mm arbitrary sensible limit
       then begin
              datum_offset_mm:=0;
  ...
  y_factor:=1.0;
     ...
     putdim(wrapping_help_str,1,'wrapping  line  offset  from  bottom  of  image',datum_offset_mm,False,True,False,False);   // neg ok, no preset, zero ok, don't terminate on zero.
     putdim(wrap_shrinkx_help_str,4,'length - wise  shrink / stretch  factor',(x_factor*100),True,True,True,False);                // no neg, no preset, no zero, don't terminate on zero.
  n:=putdim(wrap_shrinky_help_str,4,'width - wise  shrink / stretch  factor', (y_factor*100),True,True,True,False);                // ditto.

  if n<>2 then run_error(159);
  if getdims('wrap  picture  shape  along  the  control  template','',bgnd_form,n,od)=False then EXIT;
  ...
  if alert(7,'php/810 wrap  picture  shape  along  the  control  template',
            ...
            +'||Before proceeding you may want to save your current background shapes so that you can revert to them if the wrapping results are not as expected or intended .'
            +'||Cancel this and click the `0save all as...`1 button to save your background shapes.',
             '','','','','cancel','continue  -  wrap  picture  shape',0)=5 then EXIT;

  Tbgshape(bgnd_shapes_listbox.Items.Objects[i]).bgnd_shape.wrap_offset:=Round(datum_offset_mm*100);  // 205e save offset in file  in 1/100mm integer
...
begin
  ...
  with bgnd_form.bgnd_shapes_listbox do begin
    ...
    if (n<1) or (n>(Items.Count-1)) or (Items.Count<2) then EXIT;

    t:=TopIndex;                          // index of top displayed item.
    if (t<0) or (t>(Items.Count-1)) then EXIT;  // ???
...
begin
  ...
  with bgnd_form.bgnd_shapes_listbox do begin
    ...
    if (n>(Items.Count-2)) or (n<0) or (Items.Count<2) then EXIT;
    ...
    Visible:=False;  // avoid flicker.
    if (n>=t) and (n<(t+(Height div ItemHeight))) then TopIndex:=t;  // keep same lines showing if poss.
    Visible:=True;
...
procedure Tbgnd_form.move_up_buttonMouseDown(Sender:TObject; Button:TMouseButton; Shift:TShiftState; X,Y:Integer);
...
begin
  ...
  if move_up_button.Tag=1 then bgnd_move_up_button_click;
end;
//______________________________________________________________________________

procedure Tbgnd_form.move_up_buttonMouseUp(Sender:TObject; Button:TMouseButton; Shift:TShiftState; X,Y:Integer);
...
procedure Tbgnd_form.move_down_buttonMouseDown(Sender:TObject; Button:TMouseButton; Shift:TShiftState; X,Y:Integer);

              // do first move here, then via timer if held down.
begin
  move_down_button.Tag:=1;
  Application.ProcessMessages;                        // to show button down or quick back up.
  if move_down_button.Tag=1 then bgnd_move_down_button_click;
end;
//______________________________________________________________________________

procedure Tbgnd_form.move_down_buttonMouseUp(Sender:TObject; Button:TMouseButton; Shift:TShiftState; X,Y:Integer);
...
begin
  ...
  with bgnd_form.bgnd_shapes_listbox do begin
    ...
    with Tbgshape(Items.Objects[n]).bgnd_shape do begin
      ...
      if xmax<p1.x then xmax:=p1.x;
      if xmin>p1.x then xmin:=p1.x;

      if ymax<p1.y then ymax:=p1.y;
      if ymin>p1.y then ymin:=p1.y;

      if shape_code<3
         then begin
                if xmax<p2.x then xmax:=p2.x;
                if xmin>p2.x then xmin:=p2.x;

                if ymax<p2.y then ymax:=p2.y;
                if ymin>p2.y then ymin:=p2.y;
              end;
    end;//with
  ...
  if screenx<((ymax-ymin)*margin_factor*wl_factor) then screenx:=(ymax-ymin)*margin_factor*wl_factor;
  ...
  if wl_factor>minfp then zoom_offsety:=ymin-(screenx/wl_factor-(ymax-ymin))/2;
...
begin
  if line_radio_button.Checked=True
     then add_shape_button.Caption:='add  shapes'
     else add_shape_button.Caption:='add  shape';
end;
//______________________________________________________________________________
...
begin
  with canv do begin
    ...
    for i:=0 to bgnd_form.bgnd_shapes_listbox.Items.Count-1 do begin
      ...
      with Tbgshape(bgnd_form.bgnd_shapes_listbox.Items.Objects[i]).bgnd_shape do begin     // next shape.

        if shape_code<>-1 then CONTINUE;   // not a picture shape
...
begin
  ...
  with bgnd_shapes_listbox do begin
    ...
    with Items do begin
      ...
      if (Count<1) or (j<0) or (j>(Count-1)) then EXIT;    // ??? menu should be disabled

      with Tbgshape(Objects[j]).bgnd_shape do begin

        if shape_code<>1 then EXIT; // not a rectangle ??? menu should be disabled
      ...
      for n:=0 to Count-1 do begin
        ...
        with Tbgshape(Objects[n]).bgnd_shape do begin
          ...
          if p2.x<xmin then CONTINUE;
          if p1.x>xmax then CONTINUE;

          if p2.y<ymin then CONTINUE;
          if p1.y>ymax then CONTINUE;
          ...
          with Tbgshape(Objects[n]).bgimage.image_shape do begin
            ...
            if max_xdots_per_mm<xdots_per_mm then max_xdots_per_mm:=xdots_per_mm;

            ydots_per_mm:=ABS(image_height/(p2.y-p1.y));
            if max_ydots_per_mm<ydots_per_mm then max_ydots_per_mm:=ydots_per_mm;
          end;//with
      ...
      if pic_found=False
         then begin
                if alert(6,'    crop / combine  picture  shapes',
                   ...
                   then companion_help('crop_combine.php');
                EXIT;
              end;
      ...
      draw_bg_shapes(pad_form.Canvas,j,clRed);  // show new bitmap and highlight in red, directly on the pad.
      ...
      if alert(4,'    delete  cropped / combined  picture  shapes ?',
                 ...
                 '','','','','no  thanks','yes  please  -  delete  originals',0)=6
         then begin
                ...
                repeat
                  ...
                  for n:=0 to Count-1 do begin
                    ...
                    if Strings[n]='_ _ _ _ _' then CONTINUE;   // this is the cropped one
...
begin
  if shapes_help_label.Font.Color<>clRed then shapes_help_label.Font.Color:=clRed;
end;
//______________________________________________________________________________
...
begin
  if shapes_help_label.Font.Color<>clBlue then shapes_help_label.Font.Color:=clBlue;
end;
//______________________________________________________________________________
...
begin
  with bgnd_shapes_listbox do begin
    ...
    with now_shape do begin
      ...
      if (shape_code<>4) and (old_shape_code=4) then p2.x:=0;  // old was target_arm length

      case shape_code of

         -1,1,2: normalize_rectangle(p1,p2);            // new picture, rectangle, circle
              0: normalize_line(p1,p2);                 // new line
              3: begin p2.x:=0; p2.y:=0; end;           // new label
              4: begin p2.x:=target_arm; p2.y:=0; end;  // new target
...
begin
     ...
     then begin
            ...
            repeat
              i:=alert(2,'php/401    background  images',
                        ...
                        +'||After which you will be able to fill it by selecting an image file, or by pasting a copied image, or by directly scanning the image.'
                        +'|<HR NOSHADE SIZE=1 COLOR=GRAY>'
                        +'Whichever method you choose, after the picture shape has been created you can adjust its size and position to your requirements.'
                        +'||Or Templot0 can adjust it automatically for you.'
                        +'||For more information and tutorial videos click the white bar.',
                         '','','more  information','I  want  to  drop  an  image  file  on  the  trackpad','cancel','continue',3);

              if i=3 then alert_help(0,pic_shapes_help_str,'');
  ...
  with new_shape do begin
    ...
    if shape_name='' then shape_name:=empty_picture_str;

    hide_bits:=0;  // normal visibility
    option_bits:=0;     // byte;

    p1.x:=mouse_x(Screen.Width*2 DIV 3)-Random(Round(screenx/5));                          // arbitrary  random mod in case multiple overlaid shapes
    p1.y:=mouse_y(Screen.Width*2 DIV 3,Screen.Height DIV 2)-Random(Round(screeny/5));      // arbitrary
  ...
  if alert_box.drop_image.Visible=True
     then reload_picture_image(False,False,False,True,True) // dropped    // scanning,pasting,meta,dropped,adjust_aspect
     else begin
            case i of
              ...
              3: begin  // paste
                   if Clipboard.HasFormat(CF_BITMAP)=False
                      ...
                      else reload_picture_image(False,True,False,False,True);   // scanning,pasting,meta,dropped,adjust_aspect
                 end;

              4: if reload_picture_image(False,False,True,False,True)=False then EXIT;  // metafile   // scanning,pasting,meta,dropped,adjust_aspect

              //5: EXIT;

              6: if reload_picture_image(False,False,False,False,True)=False then EXIT; // file    // scanning,pasting,meta,dropped,adjust_aspect
  ...
  if yellow_msg_pref=False
     then begin
            ...
            if help(0,'         `0Image  Loaded`9||An '+picture_buttons_str,'don''t  show  this  again')=1 then yellow_msg_pref:=True;
          end;
...
begin
  ...
  with Tbgshape(bgnd_shapes_listbox.Items.Objects[bgnd_shapes_listbox.ItemIndex]).bgnd_shape do begin
    ...
    if show_tick_image.Visible=False
       then hide_bits:=hide_bits OR $01    // toggle bit on, hide shape
       else hide_bits:=hide_bits AND $FE;  // toggle bit off, show shape
...
begin
  ...
  with Tbgshape(bgnd_shapes_listbox.Items.Objects[bgnd_shapes_listbox.ItemIndex]).bgnd_shape do begin
    ...
    if show_output_tick_image.Visible=False
       then hide_bits:=hide_bits OR $02    // toggle bit on, hide shape
       else hide_bits:=hide_bits AND $FD;  // toggle bit off, show shape
  ...
  with (Control as TListBox).Canvas do begin
    ...
    try
      with Tbgshape((Control as TListBox).Items.Objects[Index]).bgnd_shape do begin
        ...
        if (hide_bits AND $02)<>0         // 214a  byte,  0=normal,  1=hide on trackpad,  2=hide on output,  3=hide both
           then mark_str:=mark_str+'• ';  // hidden on output

        if (hide_bits AND $03)<>0   // 214a  byte,  0=normal,  1=hide on trackpad,  2=hide on output,  3=hide both
           then begin
                  if text_colour=clWhite
...
const
  map_str:string='    `0calculate size of map screenshot`9'
                +'||This calculator function requires that the map screenshot was captured with the page zoom in the browser set for dot-for-dot image display.'
                +'||This may be less than the 100% page zoom setting if your Windows screen dpi setting is higher than 96dpi.'
                +'||If the page zoom was not dot-for-dot the calculation result will be incorrect.';
var
  n:integer;
  od:Toutdim;
...
begin
  ...
  lat_degs:=54.0;
     ...
     putdim('This information is shown below the shapes list for the selected picture shape.',0,'width  of  map  image  in  pixels',map_pixels,True,True,True,False);            // no neg, no preset, no zero, don't terminate on zero.
     putdim('This information is part of the the map URL.',0,'zoom  level  of  captured  image',zoom_level,True,True,True,False);             // no neg, no preset, no zero, don't terminate on zero.
  n:=putdim('This information is part of the the map URL.',3,'latitude  at  centre  of  map  in  degrees',lat_degs,False,True,False,False);   // neg ok, no preset, zero ok, don't terminate on zero.

  if n<>2 then EXIT;

  if getdims('calculate  map  size',map_str,bgnd_form,n,od)=True
     then begin
