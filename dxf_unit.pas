...
var
  ...
  rail_foot_z:extended=0-2.75;   // mm default rail base 6" at 5.5 mm scale bleow zero datum (rail top).
...
begin
  if dxf_form.scaled_checkbox.Checked=True
     then pz:=out_factor                        // scale in accordance with current print size.
     else pz:=1.0;

  if dxf_form.inches_radio_button.Checked=True
     then out_str:=FormatFloat('0.0000',d*pz/25.4)+'|'
     else out_str:=FormatFloat('0.00',d*pz)+'|';

  // 0.94.a bug fix for non English speaking contries. DXF format uses dot for decimal point regardless of regional settings.

  RESULT:=StringReplace(out_str,',','.',[]);  // 0.94.a  StringReplace(str,'old','new',[rfReplaceAll, rfIgnoreCase]);
end;
//___________________________________________________________________________________________

function dxf_3dface(x1,y1,z1,x2,y2,z2,x3,y3,z3,x4,y4,z4:extended; layer:integer):string;    // make up a four-cornered 3-D face.
...
begin
  ...
  case layer of
         ...
         10: RESULT:=IntToStr(solid_bgnd_combo.ItemIndex)+'|';
         ...
         14: RESULT:=IntToStr(railsides_3d_colour)+'|';
         15: RESULT:=IntToStr(railtop_3d_colour)+'|';
         16: RESULT:=IntToStr(chair_3d_colour)+'|';
         17: RESULT:=IntToStr(timber_3d_colour)+'|';
        else RESULT:='7|';        // black.
  end;//case
  end;//with
end;
...
begin
  ...
  with dxf_form do begin
    ...
    if (group_option_button.Checked=True) and (any_selected=0)
       then begin
    ...
    with save_dialog do begin                          // set up the save dialog.
      ...
      if user_save_file_name<>'' then InitialDir:=ExtractFilePath(user_save_file_name)   // use his previous folder.
                                 else InitialDir:=exe_str+'DXF-FILES\';                  // or the default one.

      Filter:='DXF files (*.dxf)|*.dxf';
      Filename:=remove_invalid_str(Copy(Trim(box_project_title_str),1,20)+FormatDateTime(' yy_mm_dd hhmm ss',Date+Time))+f_str+'.dxf';
...
const
  chat_str:string='This DXF generator is fairly minimal - the object is simply to get the drawing into your CAD software.'

 +' What you do with it after that is up to you. The file contains no header information to change your CAD setup.'
 +'||( Handy hint - make a note of your DXF file name and date in the memo text for the first template in the box. You will then have a reference'
 +' to it when you reload this box file at some future date.)'

 +'||Templot0''s drawing space is  +/- 1.0E7 mm ( that''s 20 kilometres or 12 miles square ! ) and drawing is done'
 +' to the nearest 0.01 mm within it. Your CAD program may need this information to load the file correctly.'

 +'||Although it is possible to import a DXF file as BACKGROUND SHAPES, there is currently no means of importing individual templates from a DXF file'
 +' and never will be as far as the normal template generating functions are concerned - Templot0 cannot work backwards from a collection of drawn lines to recreate'
 +' the original template specification which produced them.'
 +'||p.s. Never say "never" !';
...
const
 ...
 +'||N.B. This export function is not Templot0''s normal data file saving function. To save your work in the normal way for later reloading, click the SAVE ALL... or SAVE GROUP... buttons on the STORAGE BOX,'
 +' or click the FILES > SAVE ALL TEMPLATES... menu item on the trackpad window.'

 +'||This DXF export function is primarily intended as a means of transferring Templot0''s drawings to other software, or to specialist copying services for printing in large format.'

 +'||Exported DXF files can only be re-imported into Templot0 as part of the BACKGROUND SHAPES functions, which do not permit the templates to be further copied, adjusted or aligned.'
 ...
 +'||Templot0 will generate a DXF file from the current background drawing using these colours and styles for each layer.'
 ...
 +'||If the 3-D boxes are ticked, some 3-dimensional data will be included in the DXF file for the rail height and timber thickness (wire-frame or solid-rendered format).'
 +' Bear in mind that this will very significantly increase the DXF file size and the time taken for your CAD software to load the data. There is nothing to be gained by this if your CAD software does not have 3-D capability.'
 +' Do not use the 3-D options if this exported DXF file is intended to be re-imported into Templot0''s Background Shapes.'

 +'||The rail and timber 3-D dimensions are set via the REAL > RAILS > RAIL SECTION DATA..., REAL > CHAIRS/BASEPLATES > CHAIR/BASEPLATE DATA... and REAL > TIMBERING > TIMBERING DATA... menu items,'
 +' and can differ for each template. The rail top is set as the zero datum Z-dimension, so that templates with differing rail section heights will be correctly aligned.'
 +'||When the 3-D options are selected, timber centre-lines, 8ft6in timber end marks and timber numbering are omitted from the file automatically. You may wish to omit other features by setting their colours to NONE.'
 +'||N.B. For the timbers to be correctly drawn in 3-D, it is necessary to have de-selected OUTLINE EXTENSION MARKS in the GENERATOR > GENERATOR SETTNGS > menu, and then clicked REBUILD ALL BACKGROUND.'
 +' ( And TIMBER INFILL should be selected, otherwise the timbers will be drawn hollow.)'
 ...
 +'||Unlike Templot0''s own box file format, the DXF file contains only the template drawings. The information texts'
 ...
 +'||Remember that DXF files are much larger than Templot0''s own box data files. A complete track layout which requires say 150KB in Templot0 format can be 3MB or more as a DXF file. Don''t accumulate more of them than you need,'
 +' and remember to omit unwanted features to save file space. It is better to save your work as Templot0 files and generate DXFs only as needed.'
...
begin
  if help(0,help_str,'DXF  chat')=1 then chat_click;
end;
//________________________________________________________________________________________
  ...
  fixed_diamond_ends:boolean;
                              ...
                              begin
                                if colour(layer)<>'0|'       // does he want this in the file ?
                                   then begin
                                          x1:=move_to.x/100; y1:=move_to.y/100-text_height;   // (convert Templot0 top-left to DXF bottom-left.)
                              ...
                              begin
                                if colour(layer)<>'0|'       // does he want this in the file ?
                                   then begin
                                          ...
                                          if (_3d=False) or ( (code<>3) and (code<>33) and (code<>93) )  // not 3-D timber edges.
                                             then begin
                                                    ...
                                                    if dxf_form.timb_full_radiobutton.Checked=True     // (flat tops done elswhere.)
                                                       then begin
                                                              ...
                                                              if wire_frame=True
                                                                 then begin
                                                                        Write(dxf_file,dxf_line(x1, y1, timb_top_z, x2, y2, timb_top_z, layer));  // top edge.
                                                                        Write(dxf_file,dxf_line(x2, y2, timb_top_z, x2, y2, timb_bot_z, layer));  // then down corner
                                                                        Write(dxf_file,dxf_line(x2, y2, timb_bot_z, x1, y1, timb_bot_z, layer));  // bottom edge.
                                                                        Write(dxf_file,dxf_line(x1, y1, timb_bot_z, x1, y1, timb_top_z, layer));  // then back up to top.
                                                                      end
                                                                 else Write(dxf_file,dxf_3dface(x1,y1,timb_top_z, x2,y2,timb_top_z, x2,y2,timb_bot_z, x1,y1,timb_bot_z, 17));
                                                            end;
                                                  end;
                                        end;
                              end;
                                ...
                                with now_keep do begin
                                  if (bgnd_endmarks_yn[aq1,aq1end]=True) and (bgnd_endmarks_yn[aq2,aq2end]=True)
                                     then begin
                                            ...
                                            Write(dxf_file,dxf_line(x1, y1, 0, x2, y2, 0, layer));  // at rail top.

                                            if _3d=True   // add 3-D effect...
                                               then begin
                                                      Write(dxf_file,dxf_line(x2, y2,           0, x2, y2, rail_foot_z, layer));  // then down corner to foot
                                                      Write(dxf_file,dxf_line(x2, y2, rail_foot_z, x1, y1, rail_foot_z, layer));  // end of foot.
                                                      Write(dxf_file,dxf_line(x1, y1, rail_foot_z, x1, y1,           0, layer));  // then back up corner to top.

                                                      if wire_frame=False  // add solid infill for rail-end...
                                                         then begin
                                                                Write(dxf_file,dxf_3dface(x1,y1,0, x2,y2,0, x2,y2,rail_foot_z, x1,y1,rail_foot_z, 14));
                                                              end;
                              ...
                              begin
                                with now_keep do begin
                                  ...
                                  for nk:=1 to array_max do begin
                                    ...
                                    Write(dxf_file,dxf_line(x1, y1, 0, x2, y2, 0, layer));   // along rail top
                                    ...
                                    if do_3d=True               // add 3-D effect...
                                       then begin
                                              ...
                                              if wire_frame=True
                                                 then Write(dxf_file,dxf_line(x2, y2,           0, x2, y2, rail_foot_z, layer));  // then down to foot

                                              Write(        dxf_file,dxf_line(x2, y2, rail_foot_z, x1, y1, rail_foot_z, layer));                          // back along foot.

                                              if wire_frame=True
                                                 then Write(dxf_file,dxf_line(x1, y1, rail_foot_z, x1, y1,           0, layer));    // then back up to top.
                                              ...
                                              if array_max_outer>0         // valid outer rail edge.
                                                 then begin
                                                        ...
                                                        x_outer1:=move_to_outer.x/100; y_outer1:=move_to_outer.y/100;
                                                        x_outer2:=line_to_outer.x/100; y_outer2:=line_to_outer.y/100;


                                                        if wire_frame=True
                                                           then Write(dxf_file,  dxf_line(x1,y1,0, x_outer1,y_outer1,0, layer))  // wire across the top.
                                                           else Write(dxf_file,dxf_3dface(x1,y1,0, x_outer1,y_outer1,0, x_outer2,y_outer2,0, x2,y2,0, 15));  // solid top face of rail (4 corners of segment).
...
begin
  ...
  for n:=0 to (keeps_list.Count-1) do begin
    ...
    with Ttemplate(keeps_list.Objects[n]) do begin   // to next template.
      ...
      if (dxf_form.group_option_button.Checked=True) and (group_selected=False)  // ignore this one.
         then CONTINUE;

      with template_info.keep_dims.box_dims1.proto_info do begin      // 3-D data for this template...
    ...
    with now_keep do begin
      ...
      -4,-1,0,8,9,10: CONTINUE;    // timber selector, fixing peg, blank entries, peg arms, plain track start marks.  not in DXF.

                   1: layer:=6;    // guide marks.

                 2,7: layer:=7;    // radial end marks.  transition marks.

             3,33,93: layer:=3;    // timber outlines.
        ...
        if ((code=203) or (code=233) or (code=293)) and (i<array_max)      // timber infill
           and (_3d=True) and (wire_frame=False) and (colour(layer)<>'0|')
           then begin
                  x1:=move_to.x/100;
                  ...
                  Write(dxf_file,dxf_3dface(x1,y1,timb_top_z, x2,y2,timb_top_z, x3,y3,timb_top_z, x4,y4,timb_top_z, 17));
     ...
     then begin
            ...
            for i:=0 to maxbg-1 do begin
              ...
              with now_shape do begin
                ...
                if ((shape_code=0) and (shape_style=2)) or (shape_code=-1)
                   then layer:=11    // for dotted lines or pictures.
                   else layer:=10;   // for the rest.

                if colour(layer)='0|' then CONTINUE;        // he doesn't want it.
                ...
                if shape_code<>4    // not a target mark
                   then begin
                          ...
                          case shape_code of
                                       0: Write(dxf_file,dxf_line(x1, y1, 0, x2, y2, 0, layer));  // line.
                                    ...
                                    -1,1: begin  // make rectangle or picture.
                                       ...
                                       3: begin    // label shape
                                            s:='  0|TEXT|  8|'+layer_str[layer]
                                              +' 10|'+make_dim(x1)
                                              +' 20|'+make_dim(y1)
                                              +' 40|'+make_dim(font_height)  // text height for labels
...
begin
  if (Sender is TComboBox) then TComboBox(Sender).Color:=clWindow;
  top_panel.Hide;
  bottom_panel.Hide;
end;
//_________________________________________________________________________________________
...
begin
  ...
  case i of
         ...
         2: timber_3d_colour:=54;   // stone = olive.
         3: timber_3d_colour:=33;   // dark brown.
         4: timber_3d_colour:=31;   // light brown.
         5: EXIT;
         6: timber_3d_colour:=8;    // grey.
  end;//case
