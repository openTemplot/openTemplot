...
const
  ...
  +'||If the inner ring size is set to a small dimension or zero (click the MAKE TARGET button to make it 6" scale diameter), the spacing-ring and any copies of it become small target marks which are useful as general markers.'
  ...
  +' which can be changed to the cross-hairs symbol for accuracy by clicking the MOUSE CROSS-HAIRS button.'
  ...
  +' Each drawn line becomes a separate shape in the background shapes list, and can be deleted or modified as required.'
  ...
  +'||Remember to save the shapes before quitting Templot0 (they are not included in the storage box data files).';
...
uses
  ...
  { OT-FIRST dtp_unit, dtp_settings_unit,} export_unit, { OT-FIRST pdf_unit,} entry_sheet, alert_unit,
...
begin
  ...
  if (pad_form.show_shapes_menu_entry.Checked=True) and (bgnd_form.bgnd_shapes_listbox.Items.Count>0)
     then begin
            with canv do begin
              ...
              for i:=first_index to max_index do begin
                ...
                if ( (corner1_mod=1) or (corner2_mod=1) or (oneshape_shift_mod=1) or (oneshape_scale_mod=1) )
                   and (i=bgnd_form.bgnd_shapes_listbox.Itemindex)
                       then Pen.Color:=clRed                  // he's adjusting a single shape (but not all of them).
                       else Pen.Color:=colour;
                try
                  Pen.Style:=psSolid;              // changes later for dotted lines.

                                   // mods 0.73.a 23-9-01...
                  Pen.Width:=1;
                  if bgnd_form.pad_shapes_linewidth_2_radiobutton.Checked=True then Pen.Width:=2;
                  if bgnd_form.pad_shapes_linewidth_3_radiobutton.Checked=True then Pen.Width:=3;

                  with Tbgshape(bgnd_form.bgnd_shapes_listbox.Items.Objects[i]).bgnd_shape do begin     // next shape.

                    if (hide_bits AND $01)<>0 then CONTINUE;   // 214a  byte,  0=normal,  1=hide on trackpad,  2=hide on output,  3=hide both
                    ...
                    if shape_code<>4    // not a target mark
                       then begin
                              ...
                              case shape_style of
                                          0: begin
                                               Brush.Color:=paper_colour;
                                               Brush.Style:=bsClear;      // transparent (not used for elipse). (also line shapes).
                                             end;
                                          1: begin
                                               Brush.Color:=timber_infill_colour;   // 0.93.a was paper_colour
                                               Brush.Style:=bsSolid;      // blank.
                                             end;
                                          2: begin
                                               if shape_code=0
                                                  then begin
                                                         draw_dotted_line(canv,move_to,line_to);   // bug workaround!!!
                                                         CONTINUE;
                                                       end
                                                  else begin
                              ...
                              if shape_code=3      // label rectangle..
                                 then begin
                                        ...
                                        line_to.Y:=move_to.Y+TextHeight(shape_name)+6;          // 0.93.a was +3
                                        Brush.Color:=paper_colour;
                                        Brush.Style:=bsSolid;                    // blank rectangle box for label.

                                        if ( NOT ( ( (corner1_mod=1) or (corner2_mod=1) or (oneshape_shift_mod=1) or (oneshape_scale_mod=1) )
                                           and (i=bgnd_form.bgnd_shapes_listbox.Itemindex) ) )

                                               and (index<0) then Pen.Color:=Font.Color;   // if only one, use highlight colour instead.
                                      end;
                                 ...
                                 then begin
                                        case shape_code of
                                             ...
                                             1,3: Rectangle(move_to.X, move_to.Y, line_to.X, line_to.Y);
                                               2: begin
                                                    if shape_style=0 then arc_ellipse(canv, move_to.X, move_to.Y, line_to.X, line_to.Y)   // transparent. Bug-fix.
                                                                     else Ellipse(move_to.X, move_to.Y, line_to.X, line_to.Y);
                                                  end;
                                        end;//case

                                        if shape_code=3 then TextOut(move_to.X+2,move_to.Y+1,' '+shape_name);   // 0.93.a was X+1  // insert label text in rectangle box.
                                      end;
                            end
                              ...
                              x1:=((p1.x-arm)*100+re_org_x)*sx;  // p2.x is the arm length each way.
            ...
            with canv do begin
              ...
              if paper_bunching=False
                 ...
                 then begin
                        ...
                        max_x:=xmax+bunch_jump_i;                                  // need extra lines to fill the bunch.
                        if bunch_shear_i=0 then line_endx:=max_x
                                           else line_endx:=bunch_start;  // but no horizontal lines if there is a shear.
              ...
              while Round(screenx/(grid_spacex*scrub))>(xmax/25) do INC(scrub);    // number of grid lines more than 1/25th of screen pixels ?.

              Pen.Color:=Font.Color;   // zero line in font colour.

              move_to.X:=Round(ex);         move_to.Y:=Round(by-gy);     // do bottom margin line before any others,
              line_to.X:=Round(line_endx);  line_to.Y:=Round(by-gy);     // so that we don't rule through the labels.

              if (check_limit(False,False,move_to)=True) and (check_limit(False,False,line_to)=True)
                 then begin MoveTo(move_to.X, move_to.Y); LineTo(line_to.X, line_to.Y); end;
...
//  draw vertical grid lines...
              ...
              repeat
                if gridco=0 then Pen.Color:=Font.Color   // zero line in margin colour.
                            else Pen.Color:=grid_colour;

                move_to.X:=Round(m-gx);  move_to.Y:=0;
                line_to.X:=Round(m-gx);  line_to.Y:=Round(by);

                grid_now:=0;                  // keep compiler happy.

                if (paper_bunching=False) or (move_to.X<(bunch_start-bunch_gap)) or (move_to.X>(bunch_start+bunch_gap*2+bunch_jump_i))    // no vertical lines anywhere near the bunch gap.
                   then begin

                          if (check_limit(True,False,move_to)=True) and (check_limit(True,False,line_to)=True)
                             then begin MoveTo(move_to.X, move_to.Y); LineTo(line_to.X, line_to.Y); end;

                          case grid_labels_code_i of
                                          1: grid_now:=grid_spacex*gridco/304.8; //  labels in feet.
                                          2: grid_now:=grid_spacex*gridco/25.4;  //  labels in inches.
                                          3: grid_now:=grid_spacex*gridco/scale; //  labels in prototype feet.
                                          4: grid_now:=grid_spacex*gridco/10;    //  labels in cm.
                                          6: grid_now:=grid_spacex*gridco;       //  labels in mm.
                                        else run_error(222);
                          end;//case

                          move_to.X:=Round(m-6-gx);  move_to.Y:=Round(by+1);

                          if check_limit(True,False,move_to)=True
                             then TextOut(move_to.X,move_to.Y,FormatFloat('0.###',grid_now)); //  add bottom margin labels.
                        end;
              ...
              while Round(screeny/(grid_spacey*scrub))>(ymax/25) do INC(scrub);    // number of grid lines more than 1/25th of screen pixels ?.
...
// and draw horizontal grid lines...
              ...
              move_to.X:=Round(ex/8);  move_to.Y:=Round(by-gy)-2+Font.Height div 2;
              if check_limit(True,False,move_to)=True then TextOut(move_to.X,move_to.Y,'0');  //  but not the label for it.

              repeat
                move_to.X:=Round(ex);         move_to.Y:=Round(m-gy);
                line_to.X:=Round(line_endx);  line_to.Y:=Round(m-gy);

                if move_to.Y>Round(by) then line_to.X:=Round(0-gx);  // don't overwrite bottom labels.

                if (check_limit(False,False,move_to)=True) and (check_limit(False,False,line_to)=True) then begin MoveTo(move_to.X, move_to.Y); LineTo(line_to.X, line_to.Y); end;

                case grid_labels_code_i of
                                1: grid_now:=grid_spacey*gridco/304.8; //  labels in feet.
                                2: grid_now:=grid_spacey*gridco/25.4;  //  labels in inches.
                                3: grid_now:=grid_spacey*gridco/scale; //  labels in prototype feet.
                                4: grid_now:=grid_spacey*gridco/10;    //  labels in cm.
                                6: grid_now:=grid_spacey*gridco;       //  labels in mm.
                              else run_error(222);
                end;//case

                move_to.X:=Round(ex/8);  move_to.Y:=Round(m-gy)-2+Font.Height div 2;
                if check_limit(False,False,move_to)=True
                   then TextOut(move_to.X,move_to.Y,FormatFloat('0.###',grid_now));  //  add left margin labels.

                INC(gridco,scrub);
                m:=m-gridy*scrub;
              until m<gy;

              if zoom_offsety=0 then TextOut(Round(ex/8),Round(by)-2+Font.Height div 2,'0');   // add origin label.

              green_dot_r:=5; // 0.91.b was xmax/200;  // radius of green datum dot (pixels).
              if green_dot_r>Round(scale*fx) then green_dot_r:=Round(scale*fx); // but not more than 12" scale,
              if green_dot_r<3 then green_dot_r:=3;                             // or less than 3 pixels radius.

              green_dot_x:=ex+(re_org_x*sx)-gx;    // re-org is in 1/100th mm.
              if green_dot_x<(ex-minfp)
                 then begin
                        green_dot_x:=ex;                                     // don't let datum dot disappear off-stage left.
                        if check_dark_paper=False then Brush.Color:=clBlack  // but make it black.
                                                  else Brush.Color:=clGray;
                      end
                 else Brush.Color:=$0000C000;    // dot normally green (was red!).
              ...
              move_to.X:=Round(green_dot_x - green_dot_r);  move_to.Y:=Round(green_dot_y - green_dot_r);
              line_to.X:=Round(green_dot_x + green_dot_r);  line_to.Y:=Round(green_dot_y + green_dot_r);
              ...
              if m>(pad_form.ClientWidth-TextWidth(grid_str)) then m:=pad_form.ClientWidth-TextWidth(grid_str);
              if m<0 then m:=0;
...
begin
  ...
  with pad_form do begin
    ...
    if (show_reminders_menu_entry.Checked=False) or (keeps_list.Count<1) or (show_bgnd_keeps_menu_entry.Checked=False)
       then begin
    ...
    for n:=0 to (keeps_list.Count-1) do begin
      ...
      with Ttemplate(keeps_list.Objects[n]) do begin
        ...
        if bg_copied=False then CONTINUE;  // not a background template.

        if (group_selected=True) and (hide_group_templates_menu_entry.Checked=True) then CONTINUE;

        with template_info.keep_dims.box_dims1.align_info do begin

          if (reminder_str='') or (reminder_flag=False) then CONTINUE;
          ...
          if check_limit(True,True,move_to)=True
             then begin
                    with canv do begin
                      ...
                      Ellipse(move_to.X-8,move_to.Y-8,move_to.X+8,move_to.Y+8);      // 8 dots radius arbitrary
          ...
          if memo_count>5 then CONTINUE;

            //  if one of first 5 reminders, also add callout ...

          case memo_count of

              1: memo:=reminder_memo1;
              2: memo:=reminder_memo2;
              3: memo:=reminder_memo3;
              4: memo:=reminder_memo4;
            else memo:=reminder_memo5;
    ...
    if memo_count<5 then reminder_memo5.Visible:=False;
    if memo_count<4 then reminder_memo4.Visible:=False;
    if memo_count<3 then reminder_memo3.Visible:=False;
    if memo_count<2 then reminder_memo2.Visible:=False;
    if memo_count<1 then reminder_memo1.Visible:=False;

  end;//with pad_form
end;
//______________________________________________________________________________

procedure draw_rings(canv:TCanvas; copies,dummy_vehicle:boolean);   //  draw the spacing-ring or copies.
...
begin
  if show_spacing_rings=True
     then begin
            if copies=True
               then begin
                      if ring_index<1 then EXIT;   // no copies to draw
                      imin:=1;
                      ...
                      and (grid_form.attach_ring_checkbox.Checked=True)
                      and (grid_form.show_dummy_vehicles_radio_button.Checked=True)
                          then EXIT;   // will be drawn by the control template (with dummy_vehicle=True)
            ...
            for i:=imin to imax do begin       // ring or copies
              ...
              with canv do begin
                ...
                if i=0 then Pen.Color:=ring_colour
                       else Pen.Color:=ring_copy_colour;

                move_to.X:=pad_ring_outerX1;  move_to.Y:=pad_ring_outerY1;
                line_to.X:=pad_ring_outerX2;  line_to.Y:=pad_ring_outerY2;

                if check_limits(move_to, line_to)=True then arc_ellipse(canv, move_to.X, move_to.Y, line_to.X, line_to.Y);

                move_to.X:=pad_ringX1;  move_to.Y:=pad_ringY1;
                line_to.X:=pad_ringX2;  line_to.Y:=pad_ringY2;
                ...
                move_to.X:=ringx-ring_dim;  move_to.Y:=ringy;
                line_to.X:=ringx+ring_dim;  line_to.Y:=ringy;

                if check_limits(move_to, line_to)=True then begin MoveTo(move_to.X, move_to.Y); LineTo(line_to.X, line_to.Y); end;

                move_to.X:=ringx;  move_to.Y:=ringy-ring_dim;
                line_to.X:=ringx;  line_to.Y:=ringy+ring_dim;
...
begin
  ...
  with Ttemplate(keeps_list.Objects[bgk]) do begin
    ...
    if bg_copied=False then EXIT;  // not a background template.

    if (group_selected=True) and (pad_form.hide_group_templates_menu_entry.Checked=True) then EXIT;
    ...
    try
      ...
      with now_keep do begin
        ...
        for aq:=25 downto 0 do begin    // save time by searching centre-lines first, ignore FB foot lines.
...
procedure draw_background_templates(canv:TCanvas; group_code:integer; highlight_index:integer; highlight_on:boolean; highlight_colour:TColor);  //  all the background keeps.
...
var
  ...
  last_bgnd_index:integer;  // 219a
                              ...
                              procedure draw_bgnd_rail(pen_solid:boolean);   // 0.93.a pen_solid added
                              ...
                              begin
                                with now_keep do begin
                                  ...
                                  move_to.X:=Round(ex-gx+xint*sx); move_to.Y:=Round(by-gy+yint*sy);

                                  if shift_keeps_mod=1 then shift_keep_moveto(bgk,canv);  // mouse action shift keeps, sets pen colour if selected.

                                  if twist_keeps_mod=1 then twist_keep_moveto(bgk,canv);  // mouse action twist keeps.

                                  with canv do begin

                                    if pen_solid=True then Pen.Style:=psSolid
                                                      else Pen.Style:=psDot; // 0.93.a show dotted on screen if platform edges hidden on output.
                                    ...
                                    for nk:=1 to array_max do begin
                                      ...
                                      line_to.X:=Round(ex-gx+xint*sx); line_to.Y:=Round(by-gy+yint*sy);
                                      if shift_keeps_mod=1 then shift_keep_lineto(bgk);  // mouse action shift keeps.
                                      if twist_keeps_mod=1 then twist_keep_lineto(bgk);  // mouse action twist keeps.
...
begin
  if (keeps_list.Count<1) or (pad_form.show_bgnd_keeps_menu_entry.Checked=False) then EXIT;

  last_bgnd_index:=highest_bgnd_template;  // get last bgnd template in list  219a

           // 218d set limit rectangle ...

  if (pad_form.show_bgnd_keeps_in_rect_menu_entry.Checked=True) { OT-FIRST and (file_viewer_form.Showing=False)} and (output_rectangle_width>0) and (output_rectangle_height>0)
     then begin
            draw_export_rectangle_flag:=True;

            X_left:=Round(MIN(output_rectangle_x1,output_rectangle_x2)*100);                // export rectangle  ...
            ...
            X_left:=Round((zoom_offsetx-g*2)*100);                 // screen area instead ...       g over-limits arbitrary
  ...
  for keeps_index:=0 to (keeps_list.Count-1) do begin
    Ttemplate(keeps_list.Objects[keeps_index]).bgnd_is_in_rect:=is_bgnd_in_rect(keeps_index, X_left,X_right,Y_top,Y_bottom);   // temp flag
  end;//next
  ...
  with canv do begin
    ...
    for bgk:=0 to (keeps_list.Count-1) do begin
      ...
      if (highlight_index<>-1) and (bgk<>highlight_index) then CONTINUE;  // don't need to draw any others if one is for highlighting.

      with Ttemplate(keeps_list.Objects[bgk]) do begin

        if bgnd_is_in_rect=False then CONTINUE;     // 218d no part of template in screen area

        if bg_copied=False then CONTINUE;  // not a background template.

        if (group_selected=True) and (pad_form.hide_group_templates_menu_entry.Checked=True) then CONTINUE;  // 209c
        ...
        try
          ...
          with now_keep do begin
            ...
            if (keep_selected=True) and (group_code=-1) then CONTINUE;    // group templates not wanted.
            if (keep_selected=False) and (group_code=1) then CONTINUE;    // only group templates wanted.
            ...
            with template_info.keep_dims.box_dims1 do begin
              ...
              case marker_colours_pad of
                   ...
                   2,3: if use_pad_marker_colour=True
                           then begin
                                  marker_colour:=pad_marker_colour;
                                  using_marker_colour:=True;
                                end;

                     4: if use_print_mapping_colour=True
                           then begin
            ...
            for i:=0 to array_max do begin
              ...
              with bgkeeps_form do begin
                ...
                case code of
                       -5: Pen.Color:=clBlack;    // label position - colour ignored.

   -4,0,501..508,600..605,700..703: CONTINUE;    // ignore timber selector mark, blank lines. // 0.94.a ignore check-rail labels
                                                 // 206b  600..605, 700..703 ignore long marks and switch/xing labels on trackpad

                    -3,-2: if marks_checkbox.Checked=True
                              then begin
                                     if using_marker_colour=False then Pen.Color:=bgkeep_mark_colour  // curving rad centres.
                                                                  else Pen.Color:=marker_colour;
                                   end
                              else CONTINUE;

                   -1,8,9: if peg_checkbox.Checked=True
                              then Pen.Color:=bgkeep_peg_colour     // fixing peg, peg arms.
                              else CONTINUE;

                 1,2,7,10: if marks_checkbox.Checked=True
                              then begin
                                     if using_marker_colour=False then Pen.Color:=bgkeep_mark_colour     // guide marks, radial ends, transition ends.
                                                                  else Pen.Color:=marker_colour;
                                   end
                              else CONTINUE;

                  3,33,93: if timber_outlines_checkbox.Checked=True
                              then begin
                                     if using_marker_colour=False then Pen.Color:=bgkeep_timber_colour  // timber outlines.
                                                                  else Pen.Color:=marker_colour;
                                   end
                              else CONTINUE;

               4,14,44,54: if timber_centres_checkbox.Checked=True
                              then begin
                                     if using_marker_colour=False then Pen.Color:=bgkeep_timber_colour  // timber centre-lines only on pad.
                                                                  else Pen.Color:=marker_colour;
                                   end
                              else CONTINUE;

                  5,55,95: if reduced_ends_checkbox.Checked=True  // timber reduced ends.
                              then begin
                                     if using_marker_colour=False then Pen.Color:=bgkeep_timber_colour
                                                                  else Pen.Color:=marker_colour;
                                   end
                              else CONTINUE;

                        6: if joints_checkbox.Checked=True
                              then begin
                                     if using_marker_colour=False then Pen.Color:=bgkeep_mark_colour    // rail joints.
                                                                  else Pen.Color:=marker_colour;
                                   end
                              else CONTINUE;

                       99: if timber_numbering_checkbox.Checked=False then CONTINUE;  // text.

              203,233,293: if (timber_infill_checkbox.Checked=True) and ( (screenx<200*scale) or (bgpad_timb_infill_style>2) )  // infill on pad if solid/blank fill or large enough to see hatching.
                              then Pen.Color:=paper_colour //use paper colour to avoid line thickening caused by rounding, when the outline overwrites.
              ...
              if code=99    // code=99, timber numbering.
                 then begin
                        ...
                        numb_str:=extract_tbnumber_str(tbnum_str);   // get next timber numbering string from the acummulated string.
                        if numb_str='' then CONTINUE;                // no string available??
                        ...
                        if check_limit(True,True,move_to)=True
                           then begin
                                  ...
                                  TextOut(move_to.X-(TextWidth(numb_str) div 2),move_to.Y-(TextHeight(numb_str) div 2),numb_str);
              ...
              if (keep_selected=True) and (pad_form.show_group_menu_entry.Checked=True) then Pen.Color:=selection_colour; // draw selected group in this colour.

              if (bgk=highlight_index) and (highlight_on=True) then Pen.Color:=highlight_colour;    // this one to be highlighted.
              ...
              if code>0
                 then begin
                        ...
                        infill3.X:=Round(check_int3x);  infill3.Y:=Round(check_int3y);
                        infill4.X:=Round(check_int4x);  infill4.Y:=Round(check_int4y);
                        ...
                        if (code=203) or (code=233) or (code=293)   // timber infill...
                           then begin
                                  ...
                                  if (check_limits(infill_points[0],infill_points[1])=True) and (check_limits(infill_points[2],infill_points[3])=True)
                                     then begin
                                            ...
                                            case bgpad_timb_infill_style of
                                                            0: CONTINUE;
                                                            1: Brush.Style:=bsBDiagonal;     // hatched. Backward diagonal for background templates.
                                  ...
                                  if (bgk=last_bgnd_index) and (Pen.Color=bgkeep_timber_colour) and (bgkeeps_form.bold_timber_outlines_checkbox.Checked=True)     // 219a
                                     then Pen.Width:=3
                                     else Pen.Width:=1;
                        ...
                        if code=-1              // code -1, draw bgnd fixing peg...
                           then begin
                                  peg_dim:=10;  // 0.91.b was Screen.Width div 100; // 100 arbitrary.
                                  if peg_dim>Round(scale*fx) then peg_dim:=Round(scale*fx); // but not more than 2ft scale.
                                  ...
                                  move_to.X:=bg_pegx-peg_dim*2;  move_to.Y:=bg_pegy;
                                  line_to.X:=bg_pegx+peg_dim*2;  line_to.Y:=bg_pegy;
                                  ...
                                  move_to.X:=bg_pegx;  move_to.Y:=bg_pegy-peg_dim*2;
                                  line_to.X:=bg_pegx;  line_to.Y:=bg_pegy+peg_dim*2;
                        ...
                        if (code=-2) or (code=-3)              // draw bgnd curving rad centres...
                           then begin
                                  radcen_dim:=4; // 0.91.b was Screen.Width div 250; // 250 arbitrary.  (smaller than for the current).
                                  if radcen_dim>Round(scale*3*fx) then radcen_dim:=Round(scale*3*fx); // but not more than 3ft scale.
                        ...
                        if code=-5        // save the label position - actual draw later.
                           then begin
                                  ...
                                  if check_limit(True,True,move_to)=True
                                     then begin
                                            text_begin_X:=move_to.X;        // save position for the name label.
          ...
          with now_keep do begin
            ...
            with template_info.keep_dims.box_dims1 do begin
              ...
              case marker_colours_pad of
                   ...
                   1,3: if use_pad_marker_colour=True
                           then begin
                                  marker_colour:=pad_marker_colour;
                                  using_marker_colour:=True;
                                end;
            ...
            if bgkeeps_form.centres_checkbox.Checked=True
               then begin
                      if (using_marker_colour=False) or (marker_colours_pad<3)
                         then begin
                                    // 212a  dummy template = centre-lines as background shapes...

                                if template_info.keep_dims.box_dims1.align_info.dummy_template_flag=True
                                   then Pen.Color:=shapes_colour
                                   else Pen.Color:=bgkeep_mark_colour;
                              end
                         else Pen.Color:=marker_colour;

                      if (keep_selected=True) and (pad_form.show_group_menu_entry.Checked=True) // he's clicking selections.
                         then Pen.Color:=selection_colour;                                      // draw selected keeps in this colour.
                      ...
                      for aq:=24 to 25 do draw_bgnd_rail(True);
            ...
            if using_marker_colour=False
               then Pen.Color:=bgkeep_rail_colour
               else Pen.Color:=marker_colour;

            if (keep_selected=True) and (pad_form.show_group_menu_entry.Checked=True) //and  (click_bgnd_select=True)  // he's clicking selections.
               then Pen.Color:=selection_colour;                                      // draw selected keeps in this colour.
            ...
            with template_info.keep_dims.box_dims1 do begin
              ...
              if bgkeeps_form.gauge_faces_checkbox.Checked=True
                 then begin
                        for aq:=0 to 7 do begin                                         // main rails gauge faces
                          if (turnout_info1.plain_track_flag=False) or (aq=0) or (aq=3) // stock rails only, if plain track
                             then draw_bgnd_rail(True);
                        end;//next aq
              ...
              if bgkeeps_form.outer_edges_checkbox.Checked=True
                 then begin
                        for aq:=8 to 15 do begin                     // main rails outer edges
                          if (turnout_info1.plain_track_flag=False) or (aq=8) or (aq=11)
                             then draw_bgnd_rail(True);
                        end;//next aq
              ...
              if (bgkeeps_form.gauge_faces_checkbox.Checked=True)
                 ...
                 then begin
                        ...
                        if fixed_diamond_ends=True
                           then begin
                                  mark_end(bgk,canv, 1,0,9,0,True);         // planed faced of point rails for a fixed-diamond.
                                  mark_end(bgk,canv, 2,0,10,0,True);

                                  mark_end(bgk,canv, 26,1,27,1,True);     // MS K-crossing check rails.
                                  mark_end(bgk,canv, 28,1,29,1,True);     // DS K-crossing check rails.
                                end;
              ...
              with platform_trackbed_info do begin
                ...
                if (adjacent_edges_keep=True) and (bgkeeps_form.platforms_checkbox.Checked=True)   // 0.93.a platform edges
                   then begin
                          ...
                          if (using_marker_colour=False) or (marker_colours_pad<3)
                             then Pen.Color:=bgkeep_platform_colour                // 0.93.a
                             else Pen.Color:=marker_colour;
                          ...
                          draw_bgnd_rail(draw_ts_platform_rear_edge_keep);   // draw solid or dotted
                ...
                if (adjacent_edges_keep=True) and (bgkeeps_form.trackbed_edges_checkbox.Checked=True)   // trackbed edges
                   then begin
                          aq:=18;
            ...
            with template_info.keep_dims.box_dims1.platform_trackbed_info do begin    // 0.93.a ...
              ...
              if (bgkeeps_form.platforms_checkbox.Checked=True) and (adjacent_edges_keep=True)  // platforms wanted?
                 then begin
                             // 0.93.a draw platform ends ...

                        if draw_ts_platform_keep=True
                           then begin
                                  mark_end(bgk,canv, 16,0,17,0,draw_ts_platform_start_edge_keep);
                                  mark_end(bgk,canv, 16,1,17,1,draw_ts_platform_end_edge_keep);
                                end;

                        if draw_ms_platform_keep=True
                           then begin
    ...
    number_yes:=(bgkeeps_form.template_number_checkbox.Checked) and (hide_name_labels=False);    // 0.82.b
    name_yes:=(bgkeeps_form.template_name_checkbox.Checked) and (hide_name_labels=False);        // 0.82.b
    id_yes:=(bgkeeps_form.template_id_checkbox.Checked) and (hide_name_labels=False);            // 208a
          ...
          with now_keep do begin
            ...
            if keep_selected=True        // but this one is selected.
               then begin
                      ...
                      if pad_form.transparent_names_menu_entry.Checked=True then Brush.Style:=bsClear   // this one not selected - normal label.
                                                                            else Brush.Style:=bsSolid;
                    end;


            the_name_str:=Trim(template_info.keep_dims.box_dims1.reference_string);           // 208a

            if (the_name_str<>'') and (id_yes=True) then the_name_str:=the_name_str+'-';      // 208a

            full_label_string:=' '+IntToStr(bgk+1)+': '+the_name_str+template_info.keep_dims.box_dims1.id_number_str+' ';    // 208a ID added

            requested_label_string:='';

            if (number_yes=True) and (name_yes=True) and (id_yes=True)
               then requested_label_string:=full_label_string
               else begin
                      if id_yes=True
                         then begin
                                if (number_yes=True)  and (name_yes=False) then requested_label_string:=' '+IntToStr(bgk+1)+': '+template_info.keep_dims.box_dims1.id_number_str+' ';    // 208a ID added
                                if (number_yes=False) and (name_yes=True)  then requested_label_string:=' '+the_name_str+template_info.keep_dims.box_dims1.id_number_str+' ';            // 208a ID added
                                if (number_yes=False) and (name_yes=False) then requested_label_string:=' '+template_info.keep_dims.box_dims1.id_number_str+' ';                         // 208a ID added
                              end
                         else begin
                                if (number_yes=True)  and (name_yes=False) then requested_label_string:=' '+IntToStr(bgk+1)+' ';                                                         // 208a ID added
                                if (number_yes=False) and (name_yes=True)  then requested_label_string:=' '+the_name_str+' ';                                                            // 208a ID added
                                if (number_yes=False) and (name_yes=False) then requested_label_string:='';                                                                              // 208a ID added
                                if (number_yes=True)  and (name_yes=True)  then requested_label_string:=' '+IntToStr(bgk+1)+': '+the_name_str+' ';                                       // 208a ID added
                              end;
                    end;


            max_label_height:=Round(ABS(20*scale*fx));   // arbitrary - scale labels to 20ft high max.

            if pad_form.names_scaled_menu_entry.Checked=True
               then repeat
                      if ABS(TextHeight('A'))<max_label_height
                         then BREAK                                    // n.b. scale is per current pad, not necessarily the same as the bgnd template.
                         else Font.Size:=Font.Size-1;
                    until Font.Size<=1;                                // 1pt minimum font size.

            if (check_limit(True,True,move_to)=True)
               and ( (Font.Size>2) or (click_bgnd_select=True) or (shift_click=True) or (keep_selected=True) or ((GetKeyState(VK_CAPITAL) AND 1)<>0) )
               then begin
                      if requested_label_string='' then showing_label_string:=full_label_string
                                                   else showing_label_string:=requested_label_string;

                      text_end_X:=text_begin_X + TextWidth(showing_label_string);   // for boxed over and mouse hover...
                      text_end_Y:=text_begin_Y + TextHeight(showing_label_string);
                      text_font_height:=Font.Height;

                      if (paper_bunching=False) or (text_begin_X<(bunch_start-bunch_gap)) or (text_begin_X>(bunch_start+bunch_gap*2))    // no name labels anywhere near the bunch gap.
                         then begin
                                if (requested_label_string<>'') and (text_begin_X<>min_draw_int) and (text_begin_Y<>min_draw_int)
                                   then begin
                                          if pad_form.boxed_over_names_menu_entry.Checked=True
                                             then begin
                                                    ...
                                                    Rectangle(text_begin_X-4, text_begin_Y-4, text_end_X+4, text_end_Y+4);
                                                    Rectangle(text_begin_X-3, text_begin_Y-3, text_end_X+3, text_end_Y+3);

                                                    Pen.Color:=Font.Color;
                                                    Rectangle(text_begin_X-2, text_begin_Y-2, text_end_X+2, text_end_Y+2);
                                                  end;

                                          TextOut(text_begin_X, text_begin_Y, requested_label_string);
                                        end;//if requested.
                              end;//if not bunching.
...
begin
  ...
  if pad_form.show_notch_menu_entry.Checked=False then EXIT;   // 0.93.a

  with canv do begin
    notch_dim:=10; // 0.91.b was (pad_form.ClientWidth div 100);  // 100 arbitrary - matches size of peg.
    if notch_dim>Round(scale*fx) then notch_dim:=Round(scale*fx); // but not more than 2ft scale.
    ...
    pad_notch_arm_rightx:=Round(Round((notchx+peg_arm_length*COS(notch_angle))*100)*sx+ex-gx);  // notch arm length is same as peg arm length.
    ...
    if check_limits(move_to, line_to)=True       // arms ok, check the square..
       then begin
              move_to.X:=pad_notchx-notch_dim;  move_to.Y:=pad_notchy-notch_dim;
              line_to.X:=pad_notchx+notch_dim;  line_to.Y:=pad_notchy+notch_dim;

              if check_limits(move_to, line_to)=True   // square ok, draw the notch...
                 then begin
                              // first the square ...

                        if paper_colour<>clWhite then Brush.Color:=clWhite   // square with white centre.
                                                 else Brush.Color:=clLime;   // square with lime centre.
                        Brush.Style:=bsSolid;

                        if paper_colour<>clBlack then Pen.Color:=clBlack     // black border.
                                                 else Pen.Color:=clGray;     // grey border.
                        ...
                        move_to.X:=pad_notchx;  move_to.Y:=pad_notchy;
                        line_to.X:=pad_notch_arm_rightx; line_to.Y:=pad_notch_arm_righty;
                        MoveTo(move_to.X, move_to.Y); LineTo(line_to.X, line_to.Y);

                        line_to.X:=pad_notch_arm_leftx; line_to.Y:=pad_notch_arm_lefty;
                        MoveTo(move_to.X, move_to.Y); LineTo(line_to.X, line_to.Y);
                        ...
                        move_to.X:=pad_notchx;  move_to.Y:=pad_notchy-notch_dim;
                        line_to.X:=pad_notchx;  line_to.Y:=pad_notchy+notch_dim;
                        MoveTo(move_to.X, move_to.Y); LineTo(line_to.X, line_to.Y);       // add vertical hair-line.

                        move_to.X:=pad_notchx-notch_dim;  move_to.Y:=pad_notchy;
                        line_to.X:=pad_notchx+notch_dim;  line_to.Y:=pad_notchy;
                        MoveTo(move_to.X, move_to.Y); LineTo(line_to.X, line_to.Y);       // add horizontal hair-line.
                      end;
            end;
  end;//with canvas
end;
...
begin
  case mode of
    ...
    0,9,10: begin                   // normal redraw on draw-bitmap, no mouse action.
              ...
              draw_all_on_canvas(offdraw_bmp.Canvas);   // (gocalc does the copy to the pad after adding the mouse labels).
            end;

       1,2: draw_all_on_canvas(pad_form.Canvas);     // no wiping, current is erased instead.

       3,4: begin
              if mode=4 then wipe_fill_drop;         // draw as much as will remain fixed on the backdrop.
              copy_drop_to_canvas(pad_form.Canvas);  // copy the drop-bitmap to the pad.
              finish_on_canvas(pad_form.Canvas);     // draw any items not on the backdrop directly on the pad.
            end;
       ...
       7,8: begin
              if mode=8 then wipe_fill_drop;            // draw as much as will remain fixed on the backdrop.
              copy_drop_to_canvas(offdraw_bmp.Canvas);  // copy the drop-bitmap to the draw-bitmap.
              finish_on_canvas(offdraw_bmp.Canvas);     // draw any items not on the backdrop on the draw bitmap.
            end;                                        // (gocalc does the copy to the pad after adding the mouse labels).
...
procedure draw_all_on_canvas(canv:TCanvas);   // normal draw, use the specified canvas for everything.

begin

  if bgnd_form.trackpad_grid_in_front_checkbox.Checked=True then draw_bg_shapes(canv,-1,shapes_colour);   // mod 3-2-01 draw all background shapes before the grid.
  ...
  if bgnd_form.trackpad_grid_in_front_checkbox.Checked=False then draw_bg_shapes(canv,-1,shapes_colour);  // draw all background shapes over grid.

  draw_dummy_vehicle_outline_envelope_as_polygon(canv);  // 215c

  draw_background_templates(canv, 0, -1,False,clBlack);  //  all the background keeps (clBlack is dummy, not used).
...
begin
  with backdrop_bmp.Canvas do begin        // clear the backdrop for starters.
    ...
    draw_dtp_items_on_pad(backdrop_bmp.Canvas);    // 0.93.a  add sketchbook items
              ...
              Brush.Color:=guide_colour;                                           //  top-right bitmap indicator.
              FillRect(Rect(pad_form.ClientWidth-3, 0, pad_form.ClientWidth, 3));  //  (visible only when maximized).
              Brush.Color:=paper_colour;                                           //  restore for next use.

  end;//with

  if bgnd_form.trackpad_grid_in_front_checkbox.Checked=True then draw_bg_shapes(backdrop_bmp.Canvas,-1,shapes_colour);   // mod 3-2-01 draw any background shapes before grid.
  ...
  if bgnd_form.trackpad_grid_in_front_checkbox.Checked=False then draw_bg_shapes(backdrop_bmp.Canvas,-1,shapes_colour);  // draw any background shapes over grid.
  ...
  if (shift_keeps_mod=1) or (twist_keeps_mod=1)  // moving group.
     then group_code:=-1;                        // so non-group templates only.

  if (mouse_modify>-1) and (notch_linked_to_current=True) and (group_notch_linked=True)  // mouse action moving linked group?
     then group_code:=-1;                                                                // so non-group templates only.

  draw_background_templates(backdrop_bmp.Canvas, group_code,-1,False,clBlack);  //  clBlack is dummy, not used.

  draw_reminders(backdrop_bmp.Canvas);

  draw_rings(backdrop_bmp.Canvas,True,False);                                   //  draw the spacing-ring copies.

  if (ring_mod<>1) and (ringdia_mod<>1) then draw_rings(backdrop_bmp.Canvas,False,False);   //  draw the ring itself.
  if (ruler1_mod<>1) and (ruler2_mod<>1) then draw_ruler(backdrop_bmp.Canvas);              //  and the ruler.

  if (notch_mod=1) or ( (mouse_modify>-1) and (notch_linked_to_current=True) ) then EXIT; //  continue on the other canvas.
  draw_notch(backdrop_bmp.Canvas);                                                        //  draw the pegging notch.
...
procedure finish_on_canvas(canv:TCanvas); // draw any items not on the backdrop on the specified canvas.

begin
  if (shift_keeps_mod=1)
     or (twist_keeps_mod=1)
     or ( (mouse_modify>-1) and (notch_linked_to_current=True) and (group_notch_linked=True) )   // then these are not on backdrop...
        then draw_background_templates(canv, 1,-1,False,clBlack);  //  draw the group templates (clBlack is dummy, not used).

  if (ring_mod=1) or (ringdia_mod=1) then draw_rings(canv,False,False);      //  draw the ring itself.

  if (notch_mod=1) or ( (mouse_modify>-1) and (notch_linked_to_current=True) )
     then draw_notch(canv);                       //  draw the pegging notch.

  if (ruler1_mod=1) or (ruler2_mod=1) then draw_ruler(canv);  //  and the ruler.
end;
//___________________________________________________________________________________________
...
begin
  if paper_bunching=True then EXIT;     // scalebar would cross the bunch gap.

  scx:=1;                    // keep compiler happy.
  inc:=1;                    // ditto.
  case scale_bar_i of
               0: EXIT;                 //  scalebar not wanted.
  ...
  scony:=ymax-10;
  if scony>Round(by-18) then scony:=Round(by-18);   // y for top of scale bar, max 18 above sheet edge.
  ...
  with canv do begin  // canvas
      ...
      for y:=(scony+1) to (scony+3) do
        begin
          black:=0;
                                       //  10 pairs of short intervals
          for incb:=1 to 10 do
            begin
              move_to.X:=Round(sbex+black*scx);  move_to.Y:=y;                                // black/white scale 10 mm or 0.5" intervals.

              Pen.Color:=sb_colour1;
              line_to.X:=Round(sbex+(black+10)*scx+1);  line_to.Y:=y;   // +1 is a bodge to make black/white look equal intervals.
              ...
              line_to.X:=Round(sbex+(black+20)*scx);  line_to.Y:=y;
              if check_limits(move_to, line_to)=True then begin MoveTo(move_to.X, move_to.Y); LineTo(line_to.X, line_to.Y); end;
        ...
        begin
          ...
          for incb:=1 to 2 do
            begin
              ...
              Pen.Color:=sb_colour1;
              line_to.X:=Round(sbex+(black+10*inc)*scx+1);  line_to.Y:=y;   // +1 is a bodge to make black/white look equal intervals.
              if check_limits(move_to, line_to)=True then begin MoveTo(move_to.X, move_to.Y); LineTo(line_to.X, line_to.Y); end;

              move_to:=line_to;

              Pen.Color:=sb_colour2;
              line_to.X:=Round(sbex+(black+20*inc)*scx);  line_to.Y:=y;                     // only do half of last pair (for inches).
              if check_limits(move_to, line_to)=True then begin MoveTo(move_to.X, move_to.Y); LineTo(line_to.X, line_to.Y); end;
           ...
           if inc=4                   //   need another half pair if inches.
              then begin
                     move_to.X:=Round(sbex+black*scx);  move_to.Y:=y;
                     Pen.Color:=sb_colour1;
                     line_to.X:=Round(sbex+(black+10*inc)*scx);  line_to.Y:=y;
                     if check_limits(move_to, line_to)=True then begin MoveTo(move_to.X, move_to.Y); LineTo(line_to.X, line_to.Y); end;
      ...
      move_to.X:=Round(sbex);          move_to.Y:=scony+4;
      line_to.X:=Round(sbex+200*scx);  line_to.Y:=scony+4;    // middle rule
      if check_limits(move_to, line_to)=True then begin MoveTo(move_to.X, move_to.Y); LineTo(line_to.X, line_to.Y); end;

      move_to.X:=Round(sbex);          move_to.Y:=scony+8;
      line_to.X:=Round(sbex+200*scx);  line_to.Y:=scony+8;    // lower rule
      if check_limits(move_to, line_to)=True then begin MoveTo(move_to.X, move_to.Y); LineTo(line_to.X, line_to.Y); end;
      ...
      move_to.X:=Round(sbex+200*scx+4); move_to.Y:=scony+2+(Font.Height div 2);
      if check_limit(False,False,move_to)=True
         then TextOut(move_to.X,move_to.Y,sb_str);  // label end of bar. (font height is negative).
...
begin
  if show_ruler_tool=False then EXIT;   // not wanted.
  if paper_bunching=True then EXIT;     // ruler might cross the bunch gap.
  ...
  if ABS(ruler_xlength)<minfp
     then begin
            ...
            if ruler_xlength<0 then ruler_k:=Pi-ruler_k;
          end;
  ...
  case ruler_units of
                 0: begin      // as grid
                      case grid_labels_code_i of
                          1: rule_str:='feet';     //  labels in feet.
                          2: rule_str:='inches';   //  labels in inches.
                          3: rule_str:='proto-feet'; //  labels in prototype feet.
                          4: rule_str:='cm';       //  labels in cm.
                          6: rule_str:='mm';       //  labels in mm.
                        else rule_str:='mm';       //  was run_error(223);
                      end;//case
                    end;
                 1: rule_str:='mm';
                 2: rule_str:='inches';
  ...
  case ruler_units of
                0: begin      // as grid
                     case grid_labels_code_i of
                          1: rule_factor:=1/304.8; //  labels in feet.
                          2: rule_factor:=1/25.4;  //  labels in inches.
                          3: rule_factor:=1/scale; //  labels in prototype feet.
                          4: rule_factor:=1/10;    //  labels in cm.
                          6: rule_factor:=1.0;     //  labels in mm.
                        else rule_factor:=1.0;     //  mm, was run_error(222);
                     end;//case
                   end;
                1: rule_factor:=1.0;       // mm
                2: rule_factor:=1/25.4;    // inches
              else rule_factor:=1.0;       // mm
  end;//case
  ...
  with canv do begin  // canvas
    ...
    if len_done>first_len then end_start_len:=len_done-ruler_div
                          else end_start_len:=first_len-ruler_div;

    if (sb_colour1=clRed) or (sb_colour2=clRed)
       then Brush.Color:=grid_colour
    ...
    yy1:=ruler_starty+ruler_halfbar_width*2*COS(ruler_k);    // 2 arbitrary (mark length each side of centre).
    ...
    repeat        // mark divisions..
      ...
      if len_done<(ruler_len-ruler_div)  // not the last one, so not under the full length text.
         then begin
                ...
                if check_limit(False,False,move_to)=True
                   then TextOut(move_to.X,move_to.Y,FormatFloat('0.###',len_done*rule_factor)); // label divisions.
              end;
    ...
    if check_limit(False,False,move_to)=True
       then TextOut(move_to.X,move_to.Y,FormatFloat('0.###',ruler_len*rule_factor)+' '+rule_str); // label end mark.
...
procedure copy_drop_to_canvas(canv:TCanvas);     // copy the backdrop-bitmap to the specified canvas.
...
begin
  ...
  with offdraw_bmp.Canvas do begin
    ...
    if add_sketchboard_items=True then draw_dtp_items_on_pad(offdraw_bmp.Canvas);    // 0.93.a  add sketchbook items

    Brush.Color:=timber_colour;                                         //  top-right bitmap indicator.
    FillRect(Rect(pad_form.ClientWidth-3, 0, pad_form.ClientWidth, 3)); //  (visible only when maximized).
    Brush.Color:=paper_colour;                                          //  restore for next use.
  end;//with
end;
...
function hover_mousedown(click_x, click_y, limit:extended):integer;   // to highlight a bgnd keep if any at this clicked location.
...
begin
  ...
  if pad_form.hide_bgnd_keeps_menu_entry.Checked=True then EXIT;  // 209c bug-fix  can't click on invisible template
  ...
  for bgk:=0 to (keeps_list.Count-1) do begin
    ...
    with Ttemplate(keeps_list.Objects[bgk]) do begin
      ...
      if (group_selected=True) and (pad_form.hide_group_templates_menu_entry.Checked=True) then CONTINUE;  // 209c  can't click on invisible template
...
begin
  ...
  x_label.Top:=measuring_panel.Height div 10;   // attempt to keep these labels central in the panel.
...
begin
  ring_copy_colour:=get_colour('choose  a  new  colour  for  the  spacing - ring  copies',ring_copy_colour);
end;
//___________________________________________________________________________________________
...
begin
  ...
  with new_shape do begin
    ...
    with bgnd_form.bgnd_shapes_listbox.Items do begin
        n:=AddObject(new_shape.shape_name,Tbgshape.Create);  // create and add a new line in the shapes list.
...
begin
  if ring_index>(ring_count_c-1)
     then begin
            ShowMessage('Ring copies - limit reached.'+#13+#13+'To make another copy you must first delete one or more existing copies.');
            EXIT;
          end;
...
begin
  ...
  with new_shape do begin
    ...
    if bgnd_form.dotted_radio_button.Checked=True
       then shape_style:=2
       else shape_style:=0;     // 0=transparent, 1=blank, 2=cross-hatched or dotted.
...
begin
  ...
  if show_spacing_rings=True then show_rings_radio_button.Checked:=True
                             else hide_rings_radio_button.Checked:=True;
...
begin
  if ring_has_been_shown=False then grid_form.jump_to_centre_button.Click;    // 0.98.a move to centre on first showing
...
begin
  ...
  if (Top<pad_form.ClientHeight*8 div 15) and ((Top+Height)>pad_form.ClientHeight*7 div 15)
     and (Left<pad_form.ClientWidth*8 div 15) and ((Left+Width)>pad_form.ClientWidth*7 div 15)
     then Top:=10;
...
const
  ...
  helpdv_clr_str:string='php/920    `0Dummy  Vehicle  Clearance`9'
                       ...
                       +'||These marker lines can be adjusted using the `0dummy vehicle clearance`1 mouse action to measure the actual clearance available from other objects.'
                       +'||For more information about using the dummy vehicle click <A HREF="online_ref920.85a">more information online</A>.';
...
begin
  with cdvi do begin
    ...
    putdim(helpdv_len_str,2, 'vehicle  body  length  ( full-size  inches )',           dv_length,True,False,False,False);    // no neg, preset ok, zero ok, don't terminate on zero.
    putdim(helpdv_wide_str,2,'vehicle  body  width  ( full-size  inches )',            dv_width,True,False,False,False);     // no neg, preset ok, zero ok, don't terminate on zero.
    putdim(helpdv_wb_str,2,  'wheelbase  or  bogie  centres  ( full-size  inches )',   dv_wheelbase,True,False,False,False); // no neg, preset ok, zero ok, don't terminate on zero.
    putdim(helpdv_clr_str,2, 'body  side  clearance  allowance  ( full-size  inches )',dv_clearance,True,False,False,False); // no neg, preset ok, zero ok, don't terminate on zero.
 n:=putdim(helpdv_pos_str,1, 'vehicle  position  on  template',                   dv_start*inscale,False,False,False,False); // neg ok, preset ok, zero ok, don't terminate on zero.

    if n<>4 then EXIT;

    if getdims('dummy  vehicle  dimensions',helpdv_str,grid_form,n,od)=True
       then begin
              dv_length:=od[0];
              if dv_length=def_req then dv_length:=780;        // body length 780" = 65ft

              dv_width:=od[1];
              if dv_width=def_req then dv_width:=111;          // body width 111" = 9ft-3in

              dv_wheelbase:=od[2];
              if dv_wheelbase=def_req then dv_wheelbase:=558;  // wheelbase / bogie centres 558" = 46ft-6in

              dv_clearance:=od[3];
              if dv_clearance=def_req then dv_clearance:=6;    // 6" clearance each side

              if od[4]=def_req then dv_start:=111              // to first axle/bogie-pin from CTRL-0  111" = 9ft-3in
                               else dv_start:=od[4]/inscale;
                 ...
                 then begin
                        if alert(4,'php/920    delete  all  existing  copies ?',
                                  ...
                                  +' This may produce confusing results when testing clearances, if you have now changed the current dummy vehicle dimensions.',
                                   '','','','','no  -  leave  existing  dummy  vehicle  copies','yes  -  delete  all  existing  dummy  vehicle  copies',0)=6
                           then dummy_vehicle_clear_copies_button.Click;
                        end;
...
begin
  if dv_copies_index<0 then EXIT;    // -1 = no copies
  DEC(dv_copies_index);
  if dv_copies_index<0
     then begin
            dummy_vehicle_delete_copy_button.Enabled:=False;
            dummy_vehicle_clear_copies_button.Enabled:=False;
          end;
...
begin
  ...
  if dv_copies_index>(dv_copies_c-1)
     then begin
            ShowMessage('Dummy vehicle copies - limit reached.'+#13+#13+'To make another copy you must first delete one or more existing copies,'
...
begin
  ...
  if grid_form.show_dummy_vehicles_radio_button.Checked=False then EXIT;   // turned off

  if dv_copies_index<0 then EXIT;  // no copies exist
  ...
  with on_canvas do begin
    ...
    for n:=0 to dv_copies_index do begin
      ...
      with dv_copies[n] do begin
        ...
        Polygon([dvc_mm_to_pixels(b1),dvc_mm_to_pixels(b2),dvc_mm_to_pixels(b3),dvc_mm_to_pixels(b4)]);
...
begin
  if attach_ring_checkbox.Checked=True
     then begin
            ...
            gocalc(2,0);   // force re-draw even though control template hasn't changed (to show attached ring).
...
begin
  ...
  if dv_envelopes_index>dv_envelopes_c
     then begin
            ShowMessage('Unable to create more than '+IntToStr(dv_envelopes_c+1)+' outline envelopes.'
               +#13+#13+'Delete one or more existing enelopes first.');
  ...
  with dv_envelopes[dv_envelopes_index] do begin
    ...
    with cdvi do begin     // vehicle dimensions
      ...
      repeat      // fill the envelope...
        ...
        draw_dummy_vehicle_on_control_template(True,grid_form.Canvas);  // True=do calcs only, canvas not used
...
begin
  ...
  for n:=1 to dv_envelopes_index do dv_envelopes[n-1]:=dv_envelopes[n];     // move all remaining up one
...
begin
  ...
  if dv_envelopes_index<0 then EXIT;  // no envelopes exist
  ...
  with on_canvas do begin
    ...
    for i:=0 to dv_envelopes_index do begin
      ...
      with dv_envelopes[i] do begin
        ...
        if dv_outlines_limit<0 then CONTINUE;  // ??? no outlines exist at this index
        ...
        for n:=0 to dv_outlines_limit do begin
          ...
          with dv_outlines[n] do begin
            ...
            Polygon([dvo_mm_to_pixels(c1),dvo_mm_to_pixels(c2),dvo_mm_to_pixels(c3),dvo_mm_to_pixels(c4)]);
        ...
        for n:=0 to dv_outlines_limit do begin
          ...
          with dv_outlines[n] do begin
            ...
            Polygon([dvo_mm_to_pixels(b1),dvo_mm_to_pixels(b2),dvo_mm_to_pixels(b3),dvo_mm_to_pixels(b4)]);
        ...
        if grid_form.blank_ends_checkbox.Checked=True
           then begin
                  Pen.Width:=3;                // ensure overdraw ends
                  Pen.Color:=paper_colour;
                  Brush.Color:=paper_colour;

                  Polygon([dvo_mm_to_pixels(dv_outlines[0].o1),dvo_mm_to_pixels(dv_outlines[0].o2),dvo_mm_to_pixels(dv_outlines[0].o3),dvo_mm_to_pixels(dv_outlines[0].o4)]);

                  Polygon([dvo_mm_to_pixels(dv_outlines[dv_outlines_limit].o1),dvo_mm_to_pixels(dv_outlines[dv_outlines_limit].o2),dvo_mm_to_pixels(dv_outlines[dv_outlines_limit].o3),dvo_mm_to_pixels(dv_outlines[dv_outlines_limit].o4)]);
                end;
      end;//with
    end;//next i
  end;//with canvas
