...
type
  ...
  Tfile_viewer_form = class(TForm)
    ...
    procedure html_file_viewerHotSpotClick(Sender: TObject; const SRC: ThtString;
      var Handled: Boolean);
...
begin
  if (not Assigned(Item)) or (not Selected) then begin
    html_preview.LoadFromString('');
  end;
...
begin
  ...
  if tags_str = '' then
    list_str := 'No name prefix tags have been used in this file.'
  else
    list_str := 'The following prefix tags are used in this file:||' + tags_str;
...
begin
  ...
  if box_file_list.Count > 0 then begin
    for n := 0 to box_file_list.Count - 1 do begin
      if Assigned(box_file_list.Objects[n]) then begin
...
const
  readme_help_str: string =
    'green_panel_begin tree.gif To add reference notes to your storage box or data file, store a template with the name `0read me`3.'
...
begin
  if none = True then
    none_str := '  `0<B>&middot;</B> There are no read-me reference notes for this file.`8||'
  else
    none_str := '';

  if help(-7, 'php/950' + none_str + readme_help_str, 'more  about  the  file  viewer') = 1 then
    file_viewer_form.help_button.Click;
end;
//______________________________________________________________________________
...
const
  fv_help_str: string = 'php/950      `0.box  file  viewer`9' +
    '||Select the disk drive and folder from which you want to browse your .box template files, and then click the|`0show files`1 button.' + '||A screenshot image will be created from each file, and displayed in a scrollable list.' + '||You can jump quickly down the list one file at a time by pressing or holding down the `0SPACE`2 bar. You can jump quickly back up the list by pressing or holding down `0SHIFT+SPACE`2.' + ' You can jump to the top or bottom of the list by pressing the `0HOME`2 or `0END`2 keys.' + '||green_panel_begintree.gif The screenshots will reflect your current trackpad settings for colours, generator settings etc.' + '||They will be created much more quickly if you change to `0skeleton&nbsp;settings`1 in the `0generator`1 menu before using the viewer.green_panel_end' + '|If the screenshots are taking too long to create, press the `0ESC`2 key to cancel and then split the files into several smaller folders.' + '||You can choose whether to include the template name labels in the screenshots by ticking or unticking the|`0template name labels`1 tickbox.' + '||To load a file into Templot click the `0load this file`1 link below the relevant screenshot. Loading a file closes the file viewer.' + '||You can choose whether the screenshot images should also be clickable to load a file by ticking or unticking the|`0images clickable`1 tickbox. If an image is clickable it will be outlined in red when the mouse pointer moves over it.' + '||If you click the `0add from this file`1 link the templates from the selected file will be added to any existing templates in your storage box and on the trackpad.' + '||Click the `0tags`1 link to see a list of the template name prefix tags used in the selected file. If the link is greyed out it means that there are no tags used in the selected file.' + '||Click the `0read me`1 link to see the `0read me`3 notes for the selected file. If the link is greyed out it means that there are no notes for the selected file. For more information about `0read me`3 notes, click the bar below.' + '||If you would prefer the list to be created instantly without waiting for you to click the `0show files`1 button, tick the `0instant show files`1 tickbox.' + '||For more information about using the file viewer please click <A HREF="online_ref950.85a">more information online</A>.';

begin
  if help(0, fv_help_str, 'more  about  ''read  me'' notes') = 1 then
    no_readme_help(False);
...
begin
  ...
  then begin
    ...
    if FindFirst(dir_str + '*.box3', 0, search_record_box) = 0 then begin
      Inc(file_count);

      while FindNext(search_record_box) = 0 do
        Inc(file_count);
    end;
    ...
    top_str :=
      '<TABLE ALIGN="CENTER" WIDTH="100%" BORDER="0" CELLSPACING="0" CELLPADDING="4" STYLE="FONT-SIZE:15PX; FONT-WEIGHT:BOLD;">'
      + '<TR><TD NOWRAP STYLE="COLOR:#9900FF; FONT-SIZE:18PX; FONT-WEIGHT:BOLD; FONT-STYLE:ITALIC;"> &nbsp; '
      +
      IntToStr(file_count) + ' &nbsp;.box &nbsp;files &nbsp;in &nbsp;this &nbsp;folder</TD></TR>'
      + '<TR><TD> &nbsp; &nbsp; ' + dir_str + '</TD></TR>'
      + '<TR><TD ALIGN="RIGHT"><HR NOSHADE STYLE="COLOR:GRAY; HEIGHT:6PX;"></TD></TR></TABLE>';
...
begin
  ...
  if (n < box_file_list.Count) then begin
    temp := box_file_list.Objects[n];
    if Assigned(temp) then begin
      if (temp is Tbitmap) then begin
        Bitmap := TBitmap(temp);
      end;
    end;
  end;
...
begin
  if url = 'fv_reload.85a' then begin
    fv_reload_file();
    Handled := True;
    EXIT;
  end;

  if url = 'fv_add.85a' then begin
    fv_add_file();
    Handled := True;
    EXIT;
  end;

  if url = 'fv_tags.85a' then begin
    fv_tags();
    Handled := True;
    EXIT;
  end;

  if url = 'fv_readme.85a' then begin
    fv_read_me();
    Handled := True;
    EXIT;
  end;

  if url = 'fv_no_readme.85a' then begin
    no_readme_help(True);
    Handled := True;
    EXIT;
  end;   // 208d

  if url = 'fv_help.85a' then begin
    help_button.Click;
    Handled := True;
    EXIT;
  end;   // 209a

  if url = 'fv_instant.85a' then begin
...
begin
  ...
  if not OpenDocument(dir_str) then
...
procedure Tfile_viewer_form.html_file_viewerMouseMove(Sender: TObject;
...
const
  html_header_str: string =
    '<HTML><HEAD><TITLE>Templot file viewer</TITLE></HEAD><BODY><TABLE ALIGN="CENTER" WIDTH="100%" BORDER="0" CELLSPACING="0" CELLPADDING="4" STYLE="FONT-SIZE:15PX; FONT-WEIGHT:BOLD;">';
  html_footer_str: string = '</TABLE></BODY></HTML>';
...
var
  ...
  begin
    box_file_list.Add(search_record.Name);
    box_size_list.Add(IntToStr(Round(search_record.Size / 1024)) + ' KB');
    box_time_list.Add(DateToStr(FileDateToDateTime(search_record.Time)) +
      ' &nbsp; ' + TimeToStr(FileDateToDateTime(search_record.Time)));
  end;
  //////////////////////////////////////////////////////////////////
...
begin
  ...
  if FindFirst(file_name, 0, search_record_box) = 0 then begin
    add_file_to_box_lists(search_record_box);
  end;
  ...
  if box_file_list.Count > 0 then begin
    ...
    DeleteFile(oldbox_str);        // delete any previous file.
    save_box(0, eSB_SaveAll, eSO_Normal, oldbox_str);    // save existing contents for restore later.
    ...
    for n := 0 to box_file_list.Count - 1 do begin
      ...
      if Pos('.box3', LowerCase(file_name)) = 0 then begin
        file_name := file_name + '.box3';  // in case his system hides extensions.
      end;

      if load_storage_box(eLB_FileViewer, file_name, append, hl) = True then begin
      ...
      if tag_list.Count > 0 then begin
        for i := 0 to tag_list.Count - 1 do begin
          tag_str := tag_str + ' &nbsp; &nbsp; ' + tag_list.Strings[i] + '|';
      ...
      try
        ...
        show_modal_message('Sorry, an error occurred in creating image:' + #13 +
          #13 + IntToStr(n + 1) + ' : ' + box_file_list.Strings[n]);
        create_bitmap.Free;
      end;//try
    ...
    if load_storage_box(eLB_FileViewer, oldbox_str, append, hl) = True then begin
      if keeps_list.Count > 0 then begin
        if (loaded_version < 93) and (hl > -1) and (hl < keeps_list.Count) then
          mint_final_or_copy_control(hl);
        // if something loaded mint from highest bgnd if he so wants.
    ...
    for n := 0 to readme_list.Count - 1 do begin
      ...
      next_str := next_str + '</TD></TR>' + '<TR><TD COLSPAN="2">' +
        box_file_list.Strings[n] + '</TD>' + '<TD NOWRAP ALIGN="RIGHT">' +
        box_size_list.Strings[n] + ' &nbsp; &nbsp;' + box_time_list.Strings[n] + '</TD></TR>'
        + '<TR><TD NOWRAP>&nbsp;<A HREF="fv_reload.85a">load this file</A>'
        + ' &nbsp;&nbsp;<SPAN STYLE="FONT-WEIGHT:NORMAL;"><A HREF="fv_add.85a">add from this file</A>'
        + ' &nbsp;&nbsp;' + fv_tag_str +
        ' &nbsp;&nbsp;' + readme_str + '</SPAN></TD>' + '<TD></TD>'
        + '<TD NOWRAP ALIGN="RIGHT" STYLE="FONT-WEIGHT:NORMAL;">background templates: '
        +
        num_bgnd_list.Strings[n] + '&nbsp;' + '<BR>unused templates: ' +
        num_unused_list.Strings[n] + '&nbsp;' + '<BR>library templates: ' +
        num_lib_list.Strings[n] + '&nbsp;</TD></TR>';
...
begin
  ...
  if keep_form_was_showing = True then
...
begin
  if instant_show_checkbox.Checked = True then
    refresh_button.Click; // 208g
end;
//______________________________________________________________________________
