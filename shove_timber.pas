...
var
  ...
  procedure shove_buttons(able:boolean; shove_code : Tshove_code; index:integer);    // enable/disable timber shove buttons.
  ...
  with shove_timber_form do begin
    ...
    if able=True
       then begin
              ...
              if shove_code = svcOmit then Caption:=Caption+'  ( omitted )';
    ...
    if (index>-1) and (index<current_shove_list.Count)
       then begin
              with current_shove_list[index].shove_data do begin

                       // this timber either omitted or both selected AND has been shoved?

                enable_restore := (shove_code = svcOmit)
                             OR ( (
                                        (sv_x<>0)       // xtb modifier.
    ...
    if shove_code = svcOmit then able:=False;    // don't enable these if it's omitted.
           ...
           i:=putdim(help_str,1,'widen / narrow  timber  by  ( +/- per  side )',sv_w,False,True,False,False); // neg ok, no preset, 0 ok, don't terminate on zero.
              ...
              if getdims('shoving  timber  '+current_shove_str,'',pad_form,i,od)=True
                 then begin
                        ...
                        sv_code := svcShove;                 // might previously have been be omitted.
                        shove_buttons(True, svcShove, n);
                        show_and_redraw(True,True);
                      end;
            end;//with
...
begin
  ...
  for f:=0 to from_list.Count-1 do
    ...
    t.timber_string := from_list[f].timber_string;
    t.shove_data := from_list[f].shove_data;
    to_list.Add(t);
    end;//next
...
begin
  ...
  if n>=0                                    // valid slot.
     then begin
            with current_shove_list[n].shove_data do begin
              if sv_code = svcEmpty               // new slot.
                 then begin
                        sv_code := svcShove;                                        // flag to shove this timber.
    ...
    try
      ...
      if n>=0                          // valid slot.
         then begin
                ...
                current_shove_list[n].timber_string := num_str;  // and add it to list.
              end
         else CONTINUE;
    except
      CONTINUE;
