...
interface
  ...
  type
    ...
    procedure centre_line_offset_options_as_stored_menu_entryClick(
      Sender: TObject);
    procedure centre_line_offset_options_as_control_menu_entryClick(
      Sender: TObject);
...
var
  ...
  function rebuild_it(n,move_bgnd_peg_to_code,move_bgnd_peg_to_peg_rail:integer; move_bgnd_peg:boolean):boolean;     // rebuild template n.   // mods 205c
...
{ out 206b
    ...
    with bgnd_keep do begin
      for i:=0 to 4 do list_bgnd_marks[i]:=nil;     // init all background data array pointers to nil.
...
begin
  ...
  with Ttemplate(keeps_list.Objects[n]) do begin
    ...
    template_info.keep_dims.box_dims1.bgnd_code_077:=0; // bgnd_flag:=False;  // flag it is not now a background keep.
  ...
  n,list_index:integer;
                        ...
                        begin
                          ...
                          with okd do begin
                            ...
                            if old_keep_dims1.box_dims1.turnout_info1.plain_track_flag=True   // plain track template
                               then RESULT:= NOT pt_all
                               else if old_keep_dims2.turnout_info2.semi_diamond_flag=True    // half-diamond template
    ...
    with old_keep_dims1.box_dims1 do begin
      ...
      if templot_version<>program_version              // template not saved by this version.
         then begin
                ...
                if templot_version<33     // slew mods not in earlier versions than 0.33...
                   then begin
                          ...
                          with align_info do begin
                            tanh_kmax:=2;       {:double;}  {spare_int1:integer;}   // stretch factor for mode 2 slews.
                                                            {spare_int2:integer;}   // !!! double used because only 8 bytes available in existing file format (2 integers).
                            slew_type:=1;       {:byte;}    {spare_flag3:boolean;}  // !!! byte used because only 1 byte available in existing file format 1-11-99.
                          end;//with
                        end;

                if templot_version<35 then old_keep_dims2.turnout_info2.start_draw_x:=0;   {spare_float1:extended;}   // startx  not in earlier than version 0.35

                if templot_version<38
                   then begin
                          mod_text_x:=0;      // position modifiers for labels.
                          mod_text_y:=0;
                        end;

                if templot_version<52 then old_keep_dims2.turnout_info2.equalizing_fixed_flag:=False;  {spare_flag1:boolean;}   // equalizing style 1-4-00 not before 0.52.
                ...
                if templot_version<65 then old_keep_dims2.turnout_info2.no_timbering_flag:=False;  {spare_flag2:boolean;}   // 8-9-00 not before 0.65.

                if templot_version<67 then old_keep_dims2.turnout_info2.plain_track_info.pt_spacing_name_str:='  plain track rail length settings as loaded';  // 17-1-01.

                if templot_version<68
                   then begin
                          with old_keep_dims2.turnout_info2.plain_track_info do begin
                            user_pegx:=0;    // user-defined peg data (here to use former spare floats in file)
                            user_pegy:=0;
                            user_pegk:=0;
                ...
                if templot_version<71
                   then begin
                          ...
                          with proto_info do begin
                            ...
                            old_tb_pi:=102;    // 8ft-6in sleepers default (used internally for gauge changes - no meaning in file). (was 108in - 9ft pre 0.93.a)

                                      // convert old check rail lengths loaded..
                                      // to new check and wing dimensioning : v:0.71.a 24-5-01...

                            ck_ms_working1_pi:=old_cklongs_pi-old_winglongs_pi;   // full-size inches - size 1 MS check rail working length (back from "A").
                            ck_ms_working2_pi:=old_cklongm_pi-old_winglongs_pi;   // full-size inches - size 2 MS check rail working length (back from "A").
                            ck_ms_working3_pi:=old_cklongxl_pi-old_winglongl_pi;  // full-size inches - size 3 MS check rail working length (back from "A").

                            ck_ms_ext1_pi:=old_winglongs_pi;  // full-size inches - size 1 MS check rail extension length (forward from "A").
                            ck_ms_ext2_pi:=old_winglongl_pi;  // full-size inches - size 2 MS check rail extension length (forward from "A").

                            ck_ts_working_mod_pi:=0;   // full-size inches - TS check rail working length modifier.
                            ck_ts_ext_mod_pi:=0;       // full-size inches - TS check rail extension length modifier.

                            wing_ms_reach1_pi:=old_winglongs_pi;  // full-size inches - size 1 MS wing rail length.
                            wing_ms_reach2_pi:=old_winglongl_pi;  // full-size inches - size 2 MS wing rail length.
                            wing_ts_reach_mod_pi:=0;              // full-size inches - TS wing rail reach length modifier.
                          end;//with
                          ...
                          with old_keep_dims2.turnout_info2.switch_info do begin
                            ...
                            case old_size of   // was called size
                                       ...
                                       0..5: list_index:=get_switch_list_index(1,old_size+1);  // straight switches (6).
                                          6: list_index:=-1;                                   // custom switch in file.
                                      7..12: list_index:=get_switch_list_index(2,old_size-6);  // REA switches A-F (6).
                                     13..20: list_index:=get_switch_list_index(4,old_size-12); // GWR old_type heel switches. 9ft - 20ft (8).
                                     21..24: list_index:=get_switch_list_index(3,old_size-20); // GWR curved switches B-D + 30ft straight switch (4).
                                        else list_index:=-1;            //  ???
                            end;//case

                            if list_index<0   // custom switch or switch not found in listbox...
                               then begin
                                      planing_radius:=0;          // planing radius for double-curved switch.
                                      joggle_depth:=0.375;        // 3/8" depth of joggle. 0.71.a 13-4-01.
                                      joggle_length:=6;           // 6" length of joggle in front of toe (+ve). 0.71.a 13-4-01.
                                      joggled_stock_rail:=False;  // no joggle default.
                                    end
                               else begin
                                      with Tswitch(switch_select_form.switch_selector_listbox.Items.Objects[list_index]) do begin
                                        planing_radius:=list_switch_info.planing_radius;    // planing radius for double-curved switch.
                                        joggle_depth:=list_switch_info.joggle_depth;        // depth of joggle. 0.71.a 13-4-01.
                                        joggle_length:=list_switch_info.joggle_length;      // length of joggle in front of toe (+ve). 0.71.a 13-4-01.
                ...
                if templot_version<72 then old_keep_dims2.turnout_info2.angled_on_flag:=False;  // 29-7-01.
                ...
                if templot_version<74
                   then begin
                          ...
                          with proto_info do begin
                            ...
                            seat_thick_pi:=1.750;             // bullhead chair seating thickness (for 3D in DXF). (F-S inches).
                ...
                if templot_version<76
                   then begin
                          ...
                          old_keep_dims2.turnout_info2.plain_track_info.rail_joints_code:=0;   // 0=normal, 1=staggered, -1=none (cwr).
                          ...
                          old_keep_dims2.turnout_info2.plain_track_info.pt_tb_rolling_percent:=0;  // timber rolling;
                        end;
                ...
                if templot_version<94    // 0.94.a mods...    11-08-11
                   then begin
                          ...
                          with check_diffs do begin         // check-rail mouse modifiers
                            ...
                            end_diff_mk.len_diff:=old_keep_dims2.turnout_info2.crossing_info.k_check_mod_ms;
                            end_diff_mk.flr_diff:=0;
                            end_diff_mk.gap_diff:=0;
                            end_diff_mk.type_diff:=0; // byte

                            end_diff_dk.len_diff:=old_keep_dims2.turnout_info2.crossing_info.k_check_mod_ds;
                ...
                if templot_version<208    // 208a mods...    28-04-2013
                   then begin
                          ...
                          old_keep_dims2.turnout_info2.smallest_radius_stored:=max_rad;   // not available before 208a -- not loaded to the control, always calculated fresh

                          old_keep_dims2.turnout_info2.dpx_stored:=0;    // not available before 208a -- not loaded to the control, always calculated fresh
                          old_keep_dims2.turnout_info2.ipx_stored:=0;    // not available before 208a -- not loaded to the control, always calculated fresh
                          old_keep_dims2.turnout_info2.fpx_stored:=0;    // not available before 208a -- not loaded to the control, always calculated fresh
                ...
                if templot_version<209    // 209a mods...    23-04-2014
                   then begin
                          ...
                          old_keep_dims2.turnout_info2.turnout_road_endx_infile:=turnout_info1.turnout_length-turnout_info1.origin_to_toe;    //  mm default to overall length (from CTRL-1)
                ...
                if templot_version<213    // 213a mods...    13-10-2015
                   then begin
                          ...
                          old_keep_dims2.turnout_info2.diamond_switch_timbering_flag:=False; // not a dummy Y-turnout
                        end;
                ...
                if templot_version<215    // 215a mods...    14-AUG-2017
                   then begin
                          ...
                          old_keep_dims2.turnout_info2.crossing_info.curviform_timbering_keep:=False;   // 215a
                ...
                if templot_version<217    // 217a mods...    4-12-2017
                   then begin
                          ...
                          old_keep_dims2.turnout_info2.crossing_info.main_road_endx_infile:=turnout_info1.turnout_length-turnout_info1.origin_to_toe;    //  mm default to overall length (from CTRL-1)

                          old_keep_dims2.turnout_info2.crossing_info.main_road_code:=0;  // normal main-side exit
...
var
  ...
  save_bw:boolean; // 0.93.a
                               ...
                               begin
                                 ...
                                 DeleteFile(box_str);
                                 if rolling_backup=0 then file_error(box_str);
                               end;
                               /////////////////////////////////////////////////////////////
...
begin
  ...
  if (which_ones=0) and (turnoutx>0) and ({check_if_abandoned=-1}abandon_calcs=False) // check not zero-length
     then begin
  ...
  try
    ...
    try
      ...
      try
        ...
        try
          ...
          for i:=0 to keeps_list.Count-1 do begin     // first write the template data.
            ...
            Ttemplate(keeps_list.Objects[i]).template_info.keep_dims.box_dims1.file_format_code:=1;   // OT format      // put format in file
            ...
            case which_ones of
                -1: if i<>this_one then CONTINUE;
                 ...
                 3: if Ttemplate(keeps_list.Objects[i]).group_selected=False                                then CONTINUE;  // group only, ignore unselected.
            ...
            BlockWrite(box_file, next_ti.keep_dims, SizeOf(Tkeep_dims), number_written); // write all the data.

            if number_written<>SizeOf(Tkeep_dims)
          ...
          for i:=0 to keeps_list.Count-1 do begin
            ...
            BlockWrite(box_file, block_ident, SizeOf(Tblock_ident), number_written);  // the data block ident.
            if number_written<>SizeOf(Tblock_ident)
               then begin file_write_error; RESULT:=False; EXIT; end;

                           // now the shove data segment itself..

            BlockWrite(box_file, shove_count, SizeOf(integer), number_written);      // first the count of shoved timbers.
            if number_written<>SizeOf(integer)
               then begin file_write_error; RESULT:=False; EXIT; end;
      ...
      if (which_ones<>-1) and (rolling_backup=0) and (save_str='')    // normal save of any templates..
...
const
  ...
  +'||rp.gif If you run two instances of Templot0 concurrently (not recommended for Windows 95/98/ME) from the same `0\TEMPLOT\`2 folder,'
              ...
              var
                ...
                block_ident:Tblock_ident;
                                              ...
                                              begin
                                                ...
                                                if load_backup=False then file_error(box_str);
                                              end;
                                              /////////////////////////////////

                                              function load_shove_block(t_index,seg_len:integer):boolean;   // for a single template.
                                                ...
                                                if shove_count>0        // now get the data for each shoved timber...
                                                   then begin
                                                          ...
                                                          with Ttemplate(keeps_list.Objects[t_index]).template_info.keep_shove_list do begin
                                                            ...
                                                            if Count<>0 then EXIT;   // !!! shove list should be empty (created in init_ttemplate).
                ...
                try
                  ...
                  try
                    ...
                    repeat
                      ...
                      BlockRead(box_file, old_next_data, SizeOf(Told_keep_data), number_read);    // read bytes.


                      s:=old_next_data.old_keep_dims1.box_dims1.box_ident;

                      if (number_read<>SizeOf(Told_keep_data)) or ( (s<>('N '+IntToStr(n-old_count))) and (s<>('NX'+IntToStr(n-old_count))) )  // error reading, or this is not a template.
                         then begin
                                read_file_error;
                                EXIT;
                              end;

                      if version_mismatch(old_next_data)=True then resave_needed:=True;  // check for version mismatch.
                      ...
                      copy_template_info_from_to(True,this_ti,Ttemplate(keeps_list.Objects[n]).template_info);  // True = free the shove list.
                    ...
                    if Eof(box_file)=True
                       then begin
                              try CloseFile(box_file); except on EInOutError do end;  // close file if it's open.
                              EXIT;                                                   // there was no text in the file
                            end;
                  ...
                  if (_071_format=True) and (Eof(box_file)=False)
                     then begin
...
// 16 byte Tblock_ident comprising all zeroes (segment length=0).
                            ...
                            repeat     // get all the data blocks
                              ...
                              case block_ident.block_code of
                                 ...
                                 10: if load_shove_block(n,block_ident.segment_length)=False then EXIT;  // code 10 = shove data block for this template.
  ...
  try // 208c
    ...
    try
      ...
      try
        ...
        BlockRead(test_box_file, old_next_data.old_keep_dims1, SizeOf(Tkeep_dims1), number_read);    // read bytes.
        if number_read<>SizeOf(Tkeep_dims1)
           then begin
                  try CloseFile(test_box_file); except on EInOutError do end;  // close file if it's open.
                  if load_backup=False then file_error(box_str);
                  EXIT;
                end;
      ...
      with old_next_data.old_keep_dims1.box_dims1 do begin
        ...
        if Copy(s,1,1)='N'              // it's in new file format (v:0.48 on, 17-2-00)
           then begin
                  ...
                  if templot_version>70  // mods for v:0.71.a  (shove-timber data blocks appended to file). 2-5-01.
                     then _071_format:=True
                     else _071_format:=False;

                  if load_backup=True
                     then begin

                            if templot_version>62 then restored_save_done:=box_save_done;     // mods 23-6-00 for version 0.63

                            if startup_restore_pref=1 then EXIT;   //%%%%   0=ask, 1=don't restore, 2=restore without asking

                            if (auto_restore_on_startup=False) or (ask_restore_on_startup=False)  // if both True??? - must have been abnormal termination, so reload without asking.
                               then begin

                                      if startup_restore_pref=0       //%%%%   0=ask, 1=don't restore, 2=restore without asking
                                         then begin
                                                if auto_restore_on_startup=False           // these two only read from the first keep in the file..
                                                   then begin
                                                          if ask_restore_on_startup=True   // he wanted to be asked first.
                                                             then begin
                  ...
                  if load_new_format=False then EXIT;    // go get file in new format.
                end;
...
begin
         ...
         then begin
                ...
                if close_on_reload_menu_entry.Checked=True
                   then begin
                          ...
                          if mint_from=True then mint_new_current(0);  // 0.93.a test added //mint from last reloaded.
                          EXIT;              // do't do the statusbar_click, mint clears the current name.
                        end
                   else begin
                          current_name_str:=ti.keep_dims.box_dims1.reference_string;
                          if mint_from=True then mint_new_current(0);  // 0.93.a test added  // mint from last reloaded (now current).
                          EXIT;                     // do't do the statusbar_click, mint clears the current name.
                        end;
              end;
  ...
  if ((Pos('[',current_name_str)>0) or (Pos(']',current_name_str)>0))
     then begin
            if retain_prefix_msg_pref=False
               then begin
...
with keep_form do begin
  ...
  if keeps_list.Count>0
     then begin
                      ...
                      for n:=0 to keeps_list.Count-1 do begin
                        with Ttemplate(keeps_list.Objects[n]).template_info.keep_dims do begin
                          ...
                          with box_dims1 do begin
                            ...
                            if name_str<>''
                               then k:=keepform_listbox.Items.Add(name_str)     // 208a mods
                               else begin
            ...
            if (keep_form.Active=True) and (list_panel.Visible=False) then keep_draw(n);     // draw the keep if the box is open and drawing visible.

            if keep_form.Active=True then highlight_bgkeep(n);   // and highlight the peg on the current keep on the background.
            ...
            info_str:={insert_cr_str( out 0.91.b}'   '+group_str+'  '+IntToStr(list_position+1)+'  '+bg_str+'   '+ref_str
                                   +'||  '+gauge_panel.Caption
                                   +'||--------------------------------------------------------------'
    ...
    with keep_form.keep_image.Canvas do begin
          ...
          if keep_form.fit_radio_button.Checked=True   // scale to fit..
             then begin
                    ...
                    box_width:=xmax/sx;                 // (1/100th mm) in case he locks the scaling.
                  end
                                    // use fixed box width:
             else begin
                    sx:=xmax/box_width;
        ...
        for bary:=(scony+4) to (scony+5) do
          ...
          for i:=0 to (mark_index-1) do    // (mark_index is always the next free slot)
             ...
             5,8,9,55,95,203,233,293,493: CONTINUE;        // ignore timber reduced ends, peg arms, timber infill, chairs
...
begin
  ...
  if size_updown.Position>size_updown.Tag                          // ! position goes up, size goes down.
     then begin
            ...
            with keepform_listbox do ItemHeight:=Round(ItemHeight*9/10);  // owner-draw listbox.
          end;

  if size_updown.Position<size_updown.Tag
     then begin
            ScaleBy(10,9);                                                 // scale the form contents up.
            with keepform_listbox do ItemHeight:=Round(ItemHeight*10/9);
          end;
...
begin
  ...
  if (keep_form.Active=True) and (control_template_on_save=False)                // the box was opened unchanged.
     then begin
            ...
            if control_template_on_save=True             // 0.93.a
               then begin
                      ...
                      if {check_if_abandoned<>-1} abandon_calcs=True then EXIT;
                    end
               else begin
  ...
  try
    ...
    //with info_form do begin
      si:='';
      if info_text_list{info_memo.Lines}.Count<1
         then si:='    blank|'     // ???
         else begin
                for i:=0 to (info_text_list{info_memo.Lines}.Count-1) do si:=si+info_text_list{info_memo.Lines}.Strings[i]+'|'; // info text goes in keeps_list.
                si:=si+'total timbering length on this template = '+round_str(total_template_timber_length,2);  //0.97.a
              end;
    ...
    if library_template=True
       then new_ti.keep_dims.box_dims1.bgnd_code_077:=-1   // make it a library template.
       else new_ti.keep_dims.box_dims1.bgnd_code_077:=0;   // unused, not yet on background.
    new_ti.keep_dims.box_dims1.pre077_bgnd_flag:=False;    // in case reloaded in older version than 0.77.a
    ...
    if control_template_on_save=True   // 208c
       then with new_ti.keep_dims.turnout_info2 do template_type_str:=template_type_str+'c';  // max 6 chars
    ...
    if control_template_on_save=False           // 0.93.a ...
       then begin
              ...
              if (keep_form.Active=True) and (keep_form.list_panel.Visible=False) then keep_draw(n);    // draw the control template in the keeps box.
            end;
...
begin
  ...
  current_state(-1);
  if (keep_form.Active=True) and (list_panel.Showing=True) and (keeps_list.Count>0) then keepform_listbox.SetFocus;
end;
//__________________________________________________________________________________________
...
begin
  ...
  html_str:='<HTML><HEAD>'
           ...
           +'The Templot0 storage box for :</TD></TR>'
...
const
  ...
  +'||Templot0 maintains a continuous copy on file of your storage box contents and background drawing, and can restore them automatically at the start of the next working session.'
  ...
  +'||The FILES > RESTORE PREVIOUS menu item restores the box contents and background drawing to the condition they were in when you quit the previous Templot0 session.'
  ...
  +'||The FILES > RESTORE PRIOR-PREVIOUS menu item works similarly, but restores the box contents and background drawing to the condition they were in when you quit the Templot0 session prior to the previous one, i.e. the data from two sessions back.'
  ...
  +'||RESTORE ON STARTUP: These options permit you to customise the way the automatic restore on startup function works. Changes you make here will take effect the next time you start a Templot0 session, they have no effect on the current session.'
  +'||If AUTO RESTORE PREVIOUS DATA is selected, the next time you start a Templot0 session your storage box and background drawing will be automatically re-instated to the condition in which you leave it when you quit this session.'
  ...
  +'||Templot0 occasionally needs to use the control template itself in some storage box operations, so you may see some unexplained changes take place on the trackpad behind the storage box.'
  ...
  +'||If you are using Windows NT or 2000 or XP, it can be useful to run two instances of Templot0 concurrently.'
  +' It is important to run the second instance as a separate copy of Templot0 from a different folder, otherwise the automatic restore functions will not work as intended.'
  ...
  temp:extended;
                     ...
                     begin
                       with new_bgk do begin
                         for n:=0 to 4 do if list_bgnd_marks[n]<>nil then intarray_free(list_bgnd_marks[n]);
...
begin
  ...
  with new_bgk do begin
    for i:=0 to 4 do list_bgnd_marks[i]:=nil;          // init all array pointers to nil in case of error exit check.
    for aq:=0 to aq_max_c do begin
      list_bgnd_rails[aq,0]:=nil;
      list_bgnd_rails[aq,1]:=nil;
    end;//for
  ...
  //                label_loc_Y:=label_point.Y+Round(y_datum*100);
                  ...
                  for i:=0 to (mark_index-1) do begin              // (mark_index is always the next free slot)
                    ...
                    if ptr^.code=99 //then timber_numbers_string:=timber_numbers_string+ptr^.str+Chr($1B);  // add on the next timber numbering string, + separator.
                       then timber_numbers_string:=timb_numbers_str;  // copy from control template
                  end;//for
            ...
            if update_info=True
               then begin
                      si:='';
                      //with info_form do begin
                        if info_text_list{info_memo.Lines}.Count>0
                           then begin
                                  for i:=0 to (info_text_list{info_memo.Lines}.Count-1) do si:=si+info_text_list{info_memo.Lines}.Strings[i]+'|'; // info text goes in keeps_list.
                                  si:=si+'total timbering length on this template = '+round_str(total_template_timber_length,2);  //0.97.a
                                end;
...
begin
  if (index<0) or (index>(keeps_list.Count-1)) //]]] or (bgkeeps_form.bgkeeps_listbox.Items.Count<1)
     then EXIT;
...
begin
  ...
  if keeps_list.Count<1 then begin ShowMessage('Your storage box is now empty.'); EXIT; end;

  if keep_count<>keeps_list.Count then begin ShowMessage(IntToStr(keep_count-keeps_list.Count)+' unused templates were deleted.'); EXIT; end;
...
begin
  ...
  if keep_count<>keeps_list.Count then begin ShowMessage(IntToStr(keep_count-keeps_list.Count)+' library templates were deleted.'); EXIT; end;
...
begin
  ...
  if keep_count<>keeps_list.Count then begin ShowMessage(IntToStr(keep_count-keeps_list.Count)+' T-55 templates were deleted.'); EXIT; end;
...
begin
  ...
  if searched_index=-1
     then begin
            ShowMessage('Searching for '''+search_str+'''.'+#13+#13+'There are no more matching templates in the storage box.');
...
begin
  ...
  for n:=0 to (keeps_list.Count-1) do begin
    ...
    if Ttemplate(keeps_list.Objects[n]).group_selected=True
       then begin
              ShowMessage('A new group was created, matching '''+search_str+''' in the template name or ID number.');
              EXIT;
            end;
...
begin
  pad_form.show_group_templates_menu_entry.Checked:=True;  // 209c for next group in case hidden - radio item
...
begin
  ...
  html_str:='<HTML><HEAD>'
   ...
   +'The Templot0 storage box for: <SPAN STYLE="font-weight:bold;">'+box_project_title_str+'</SPAN></TD></TR>'
...
begin
  ...
  try
    ...
    copy_keep(save_current);                // reset for pad redraws after copy to background calcs.
...
begin
  ...
  if Screen.PixelsPerInch>120 then keepform_listbox.ItemHeight:=26;  // 214a
            ...
            if keeps_list.Count>0
               then begin
                      i:=alert(7,'    restore  previous  box  contents',
                                 'You are about to restore your storage box contents and background drawing to the state they were in when you quit the previous Templot0 session.'
            ...
            if keeps_list.Count>0
               then begin
                      i:=alert(7,'    restore  prior - previous  box  contents',
                                 'You are about to restore your storage box contents and background drawing to the state they were in when you quit the Templot0 session PRIOR to the PREVIOUS one, i.e. from two sessions back.'
                                +'||This will replace your existing box contents and drawing. If you will need these again you should save them first.',
...
begin
     ...
     then begin
            ...
            if alert(7,'      delete  selected  group ?',
                    ...
                    +'||Are you sure you want to delete this group ?',
                     '','','','','no  -  cancel  delete','yes  -  delete  selected  group  of  templates',0)=5 then EXIT;
          end;
  ...
  if keep_count<>keeps_list.Count then begin ShowMessage(IntToStr(keep_count-keeps_list.Count)+' group templates were deleted.'); EXIT; end;
...
begin
  ...
  if alert(7,'    normalize  all  templates',
             'The normalize function is used to bring templates which have been loaded from earlier versions of Templot0 into line with the current version.'
...
begin
  ...
  with Ttemplate(keeps_list.Objects[list_position]).template_info.keep_dims.box_dims1 do begin
    ...
    if bgnd_code_077<>0 then EXIT                // already library or on bgnd???  menu should be disabled.
                        else bgnd_code_077:=-1;  // library template.
  end;//with
  current_state(-1);
end;
//______________________________________________________________________________________
...
begin
  ...
  +'<P><TABLE STYLE="FONT-SIZE:15px; FONT-WEIGHT:BOLD;"><TR><TD WIDTH="66%">TOTAL LENGTH: The total length of all background templates is currently:</TD>'
  ...
  msg_str:=msg_str+'<TR><TD>GROUP LENGTH: The total length of all group-selected templates is currently:</TD>'
  ...
  msg_str:=msg_str+'<TR><TD>TOTAL TIMBERING: The total length of timbering on all background templates is currently:</TD>'
  +'<TD>&nbsp; &nbsp;</TD><TD>'+round_str(total_bgnd_timbering_length,2)+' mm<BR>(approx '+round_str(total_bgnd_timbering_length/304.8,0)+' ft)</TD></TR>';

  msg_str:=msg_str+'<TR><TD COLSPAN="3" ALIGN="CENTER">&nbsp;</TD></TR>';

  if classic_templot=False
     then begin
            msg_str:=msg_str+'<TR><TD>Including the current control template, the total timbering length is:</TD>'
            +'<TD>&nbsp; &nbsp;</TD><TD>'+round_str(total_bgnd_timbering_length+total_template_timber_length,2)+' mm<BR>(approx '+round_str((total_bgnd_timbering_length+total_template_timber_length)/304.8,0)+' ft)</TD></TR>';

            msg_str:=msg_str+'<TR><TD COLSPAN="3" ALIGN="CENTER">&nbsp;</TD></TR>';
          end;

  msg_str:=msg_str+'<TR><TD>GROUP TIMBERING: The total length of timbering on all group-selected templates is currently:</TD>'
  +'<TD>&nbsp; &nbsp;</TD><TD>'+round_str(total_group_timbering_length,2)+' mm<BR>(approx '+round_str(total_group_timbering_length/304.8,0)+' ft)</TD></TR>';
  ...
  if help(-350,msg_str,'normalize  templates')=1
      then begin
             normalize_templates;
             msg_str:='';
             goto 123;
...
begin
  ...
  for n:=0 to keeps_list.Count-1 do begin
    ...
    with Ttemplate(keeps_list.Objects[n]) do group_selected:=(Pos(tag_str,template_info.keep_dims.box_dims1.reference_string)<>0);
